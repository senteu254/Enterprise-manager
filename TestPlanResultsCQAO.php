...
if (isset($_POST['submit'])) {
	...
	$result = DB_query("SELECT count(sampleid) FROM qrsampleremarks
						WHERE sampleid = '".$SelectedSampleID."' AND approver_level=3",	$db);
	$myrow = DB_fetch_row($result);
	if($myrow[0]>0) {
		$sql = "UPDATE qrsampleremarks SET remark='" . $_POST['remark1'] . "'
				WHERE sampleid = '".$SelectedSampleID."' AND approver_level=3";
...
$FromDate = FormatDateForSQL($_POST['FromDate']);
		...
		if (isset($LotNumber) AND $LotNumber != '') {
			...
			$SQL = "SELECT qasamples.sampleid,
				...
				$SQL = "SELECT qasamples.sampleid,
						...
						AND sampledate <='".$ToDate."' AND approver_level=2";
		...
		if (DB_num_rows($SampleResult) > 0) {
			...
			while ($myrow = DB_fetch_array($SampleResult)) {
				...
				$ModifySampleID = $RootPath . '/TestPlanResultsCQAO.php?SelectedSampleID=' . $myrow['sampleid'];
...
echo '<div class="centre"><a href="' . $RootPath . '/TestPlanResultsCQAO.php">' . _('Back to Samples') . '</a></div>';
