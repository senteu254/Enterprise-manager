...
$sql = "UPDATE qanonconformingproducts SET process_level='" . 1 . "', mc_setter='". $_POST['setter'] ."'
					WHERE id = '". $SelectedUser . "'";
	$result = DB_query($sql);
}else{
$error = 1;
prnMsg( _('Machine Setter can not be empty'), 'error' );
}
}else{
$sql = "UPDATE qanonconformingproducts SET process_level='" . 2 . "'
					WHERE id = '". $SelectedUser . "'";
	$result = DB_query($sql);
}
if($error == 0){
$sql = "INSERT INTO qanonconformingremarks (refid,remarks,approver,approvertitle,approvername)
					VALUE('". $SelectedUser ."','". $_POST['remarks'] ."',2,'CORRECTIVE ACTION TO BE TAKEN TO PREVENT RECURRENCE(M/C SETTER)','". $_SESSION['UsersRealName'] ."')";
	...
	echo '<table style="width: 350px;
						...
						margin-left: -150px;">
			<tr>
				<th>' . _('Please Leave your Remarks') . ':</th>
			</tr>';
			echo '<form method="post" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" id="form">';
			echo '<tr>
			<td><input class="user" type="checkbox" name="user" /> Forward to another M/C Setter?
  ...
  echo '</select></div>
			...
			<tr>
				<td><input type="submit" name="Submit" value="' . _('Submit') . '" />
				</form>';
...
echo '<a href="' . $RootPath . '/QASetterNon_ConformanceForm.php">' . _('Back to Main Menu') . '</a>';
...
if (!isset($SelectedUser)) {
	$sql = "SELECT *
				FROM qanonconformingproducts where process_level >=1 ORDER BY id DESC";
	$result = DB_query($sql);
