...
$sql = "UPDATE qanonconformingproducts SET process_level='" . 4 . "'
					WHERE id = '". $SelectedUser . "'";

		$result = DB_query($sql);
$sql = "INSERT INTO qanonconformingremarks (refid,remarks,approver,approvertitle,approvername)
					VALUE('". $SelectedUser ."','". $_POST['remarks'] ."',4,'PM REMARKS','". $_SESSION['UsersRealName'] ."')";
...
echo '<a href="' . $RootPath . '/QAPMNon_ConformanceForm.php">' . _('Back to Main Menu') . '</a>';
...
if (!isset($SelectedUser)) {
	$sql = "SELECT *
				FROM qanonconformingproducts where process_level >=3 ORDER BY id DESC";
	$result = DB_query($sql);
