...
if (isset($_GET['ReceivePO']) AND $_GET['ReceivePO']!=''){
	...
	if (! in_array($_SESSION['PageSecurityArray']['LSO_ServiceDone.php'], $_SESSION['AllowedPageSecurityTokens'])){
		prnMsg(_('Your permissions do not allow receiving of goods. Automatic receiving of Service orders is restricted to those only users who are authorised to receive goods/services'),'error');
	} else {
		...
		/*Now the SQL to do the update to the PurchOrderDetails */
					...
					$SQL = "UPDATE lsorderdetails SET quantityrecd = quantityrecd + '" . $OrderLine->ReceiveQty . "',
					...
					$SQL = "INSERT INTO lsogrns (grnbatch,
											podetailitem,
											itemcode,
											itemdescription,
											deliverydate,
				...
				$SQL = "SELECT grnbatch,
								grnno,
								lsorderdetails.orderno,
								lsorderdetails.unitprice,
								lsogrns.itemcode,
								...
								lsogrns.quantityinv,
								lsogrns.stdcostunit,
								lsorderdetails.glcode,
								lsorderdetails.shiptref,
								lsorderdetails.jobref,
								lsorderdetails.podetailitem,
								lsorderdetails.assetid,
								stockmaster.decimalplaces
							FROM lsogrns INNER JOIN lsorderdetails
							ON  lsogrns.podetailitem=lsorderdetails.podetailitem
						LEFT JOIN stockmaster ON lsogrns.itemcode=stockmaster.stockid
						WHERE lsogrns.supplierid ='" . $_SESSION['SuppTrans']->SupplierID . "'
						AND lsorderdetails.orderno = '" . intval($_GET['ReceivePO']) . "'
						AND lsogrns.qtyrecd - grns.quantityinv > 0
						ORDER BY lsogrns.grnno";
...
if (!isset($_POST['PostInvoice'])){
	...
	if (isset($_POST['GRNS'])
		AND $_POST['GRNS'] == _('Service Orders')){
		/*This ensures that any changes in the page are stored in the session before calling the grn page */
		echo '<meta http-equiv="Refresh" content="0; url=' . $RootPath . '/LSO_SuppInvGRNs.php">';
	...
	echo '<br />
		<div class="centre">
			<input type="submit" name="GRNS" value="' . _('Service Orders') . '" />
	...
	$TotalGLValue = 0;
		...
		if($_POST['taxtype_' . $OrderLine->LineNumber . '']=='Excl'){ //tax exclusive
		...
		$_SESSION['SuppTrans']->OvAmount1 = ($TotalGRNValue + $TotalGLValue + $TotalAssetValue + $TotalShiptValue + $TotalContractsValue);
		$_SESSION['SuppTrans']->OvAmount = (($_SESSION['SuppTrans']->OvAmount1*100)/116);
		}else{
		$_SESSION['SuppTrans']->OvAmount = ($TotalGRNValue + $TotalGLValue + $TotalAssetValue + $TotalShiptValue + $TotalContractsValue);
		}
		}
	...
	echo '<td></td><td></td><td class="number"><strong>Inclusive</strong><input checked="checked" type="radio" ';?><?=$_POST['taxtype_' . $Tax->TaxCalculationOrder . ''] == 'Inc' ? ' checked="checked"' : '';?><?php echo 'name="taxtype_' . $Tax->TaxCalculationOrder . '" value="Inc" /><br /><strong>Exclusive</strong><input type="radio" ';?><?=$_POST['taxtype_' . $Tax->TaxCalculationOrder . ''] == 'Excl' ? ' checked="checked"' : '';?><?php echo ' name="taxtype_' . $Tax->TaxCalculationOrder . '" value="Excl" /></td>';
	echo '<tr>
	...
	foreach ($_SESSION['SuppTrans']->Taxes as $Tax) {
		...
		if (!isset($_POST['OverRideTax'])
			...
			if($_POST['taxtype_' . $Tax->TaxCalculationOrder . '']=='Excl'){ //tax exclusive
			if ($Tax->TaxOnTax ==1){
			...
			if ($Tax->TaxOnTax ==1){
				...
				$_SESSION['SuppTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxOvAmount = $_SESSION['SuppTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxRate/(1+$_SESSION['SuppTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxRate) * ($_SESSION['SuppTrans']->OvAmount1 + $TaxTotal);

			} else { /*Calculate tax without the tax on tax */

				$_SESSION['SuppTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxOvAmount = $_SESSION['SuppTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxRate/(1+$_SESSION['SuppTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxRate) * $_SESSION['SuppTrans']->OvAmount1;
...
//		}
    ...
    echo '</div>
		...
		if($_POST['taxtype_' . $OrderLine->LineNumber . '']=='Excl'){ //tax exclusive
		$_SESSION['SuppTrans']->OvAmount = $_SESSION['SuppTrans']->OvAmount;
		$_SESSION['SuppTrans']->OvAmount1 = $_SESSION['SuppTrans']->OvAmount;
		}else{ //tx inclusive
		if (!isset($_POST['OverRideTax']) OR $_POST['OverRideTax']=='Auto'){
		$_SESSION['SuppTrans']->OvAmount1 = $_SESSION['SuppTrans']->OvAmount;
		$_SESSION['SuppTrans']->OvAmount = (($_SESSION['SuppTrans']->OvAmount1*100)/116);
...
/*---------------------------------------------------------------------------------------*/
$TaxTotal = round($TaxTotal,$_SESSION['SuppTrans']->CurrDecimalPlaces);
$_SESSION['SuppTrans']->OvAmount =round($_SESSION['SuppTrans']->OvAmount,$_SESSION['SuppTrans']->CurrDecimalPlaces);
/*---------------------------------------------------------------------------------------*/
	...
	} elseif ( $_SESSION['SuppTrans']->OvAmount1 < round($_SESSION['SuppTrans']->Total_Shipts_Value() + $_SESSION['SuppTrans']->Total_GL_Value() + $_SESSION['SuppTrans']->Total_Contracts_Value()+ $_SESSION['SuppTrans']->Total_Assets_Value()+$_SESSION['SuppTrans']->Total_GRN_Value(),$_SESSION['SuppTrans']->CurrDecimalPlaces)){
						...
						if ($_SESSION['WeightedAverageCosting']==1){
							...
							*/
							...
							if ($TotalQuantityOnHand>0) {
								...
								$sql = "UPDATE farmdescriptions
										SET cost=materialcost+overheadcost+labourcost,
	...
	/*Add contract charges records as necessary */
		...
		echo '<br />
				<div class="centre">
					<a href="' . $RootPath . '/LSO_SupplierInvoice.php?&SupplierID=' .$_SESSION['SuppTrans']->SupplierID . '">' . _('Enter another Invoice for this Supplier') . '</a>
					<br />
...
if($InputError==true){ //add a link to return if users make input errors.
	echo '<div class="centre"><a href="'.$RootPath.'/LSO_SupplierInvoice.php" >' . _('Back to Invoice Entry') . '</a></div>';
