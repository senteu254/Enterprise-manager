...
if (isset($_GET['ModifyOrderNumber'])) {
	$Title = _('Modify Service Order') . ' ' . $_GET['ModifyOrderNumber'];
} else {
	$Title = _('Service Order Entry');
}
...
if (isset($_POST['EnterLinesf']) OR isset($_POST['AllowRePrint'])) {
	...
	if (!isset($_POST['AllowRePrint'])) { // user only hit update not "Enter Lines"
		...
		echo '<p>';
		prnMsg(_('You should automatically be forwarded to the entry of the purchase order line items page') . '. ' . _('If this does not happen') . ' (' . _('if the browser does not support META Refresh') . ') ' . '<a href="' . $RootPath . '/LSO_ServiceItems.php?identifier=' . $identifier . '">' . _('click here') . '</a> ' . _('to continue'), 'info');
	...
	if (!isset($_POST['AllowRePrint'])) { // user only hit update not "Enter Lines"
		...
		echo '<p>';
		prnMsg(_('You should automatically be forwarded to the entry of the purchase order line items page') . '. ' . _('If this does not happen') . ' (' . _('if the browser does not support META Refresh') . ') ' . '<a href="' . $RootPath . '/LSO_FarmItems.php?identifier=' . $identifier . '">' . _('click here') . '</a> ' . _('to continue'), 'info');
		include('includes/footer.inc');
		exit;
	} // end if reprint not allowed
...
echo '<span style="float:left"><a href="' . $RootPath . '/LSO_SelectOSPurchOrder.php?identifier=' . $identifier . '">' . _('Back to Service Orders') . '</a></span>';
...
if ($_SESSION['RequireSupplierSelection'] == 1 OR !isset($_SESSION['PO' . $identifier]->SupplierID) OR $_SESSION['PO' . $identifier]->SupplierID == '') {
	echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/supplier.png" title="' . _('Service Order') . '" alt="" />' . ' ' . _('Service Order: Select Contractor') . '</p>';
	...
	echo '<tr>
	<td>' . _('LSO No.') . ':</td>
	<td><input type="text" required="required" autofocus="autofocus" name="LSONo" class="number" size="16" value="" /></td>
	</tr>';
	}
	echo '<tr>
			<td>' . _('LSO Date') . ':</td>
	...
	if ($_SESSION['PO' . $identifier]->Status == '') { //then its a new order
		echo '<tr>
				<td><input type="hidden" name="Status" value="NewOrder" />' . _('New Service Order') . '</td>
			</tr>';
	} else {
		echo '<tr>
				<td>' . _('Status') . ' :  </td>
