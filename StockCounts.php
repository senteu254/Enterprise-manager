...
if ($_GET['Action'] == 'Enter'){
	...
	if (DB_num_rows($CatsResult) ==0) {
		...
		OR isset($_POST['Previous'])){
			...
			$RowCount=1;
			/*--------------------------------------------------------------------------------------------*/
			echo '<input type="hidden" name="EnterByCat" value="" />';
				$ListCount = DB_num_rows($StkItemsResult);
			$ListPageMax = ceil($ListCount / $_SESSION['DisplayRecordsMax']);
			if (isset($_POST['Go'])) {
			$_POST['PageOffset'] = $_POST['PageOffset'];
			}
			if (isset($_POST['Next'])) {
			...
			$RowIndex = 0;
			if (DB_num_rows($StkItemsResult) <> 0) {
				DB_data_seek($StkItemsResult, ($_POST['PageOffset'] - 1) * $_SESSION['DisplayRecordsMax']);
			}
			while (($StkRow = DB_fetch_array($StkItemsResult)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
			/*--------------------------------------------------------------------------------------------*/
