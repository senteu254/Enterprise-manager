...
$sql="select level,levelcode from pvlevel where id='".$_GET['id']."'  ";
$qry1 = DB_query($sql);
...
$sql="select * from pvlevel where level='$id'  ";
$qry1 = DB_query($sql);
$rec = DB_fetch_array($qry1);
$level=$rec['level'];
if($rec>0 ){
prnMsg( _('Not Updated! level Already Exists'), 'warn');
} else{
	$sql = "UPDATE pvlevel SET level='".$id."' WHERE id=".$_GET['id']." ";
$qry = DB_query($sql);
	if ($qry  ){
...
$rec = DB_fetch_array($qry1);
if($rec>0 ){
prnMsg( _('level Already Exists'), 'warn');
}
                else
					{
...
<div class="col-md-6 col-md-offset-3">
 ...
 $sql="select * from pvlevel ";
	$qry1 = DB_query($sql);
  $i=0;
  $sql="select * from pvlevel LIMIT $start_from, $endlimit";
	$query = DB_query($sql);
  ...
  ?>
		...
		<?php //paging codes "$welcome_viewed halin ni sa ging query sang $welcome_view"
		$num_rows = DB_num_rows($qry1);
		$total_pages = ceil($num_rows / $endlimit);
		$j=0;
		echo ' '.$_REQUEST["page"].' ';
				for($j=1; $j<=$total_pages; $j++ )
					{
						 echo"<ul class='pagination '> <li>&nbsp<a href = 'index.php?Application=PVP&Ref=pvapprlevels&page=".$j."'>".$j."</a></li></ul>";
					}
						echo'&nbsp&nbsp';
?>
	</div>
