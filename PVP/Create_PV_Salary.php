...
##############################################################################################
  ...
  <?php	
	...
	echo '<tr>
			<td>' . _('User') . ':</td>
			<td>';
			echo '<input class="user" type="checkbox" name="user" checked/>';
	echo '</td>
		</tr>';
		echo'<tr>
			<td>' .  _('Payee Name') . '</td>';
	...
	<div class="form-group" id="select_customer">				
				...
				echo '<input type="hidden" size="30" maxlength="40" id-="supplierid" name="supplierid" value="'.$VVotehead.'"/>';?> 
					<input type="text" autocomplete="off" name="customer" value="<?php echo $VVotehead; ?>" placeholder="Start typing Supplier  Name..." onKeyUp="AutoCompleteCust();" id="customer" class="form-control input-sm" size="50" tabindex="1"  />
					<span class="ui-helper-hidden-accessible" style="" role="status"></span><div id="outputcustomer"></div>					

				</div>	
	
	</div>
 	<div class="userfield" style="display:none"><input name="Username" style="width:350px" type="text" value="<?php echo $VVotehead; ?>"/></div>
  <?php
 echo'</td>';
	}	
	//--------------------------------------------------------------------
...
for($i=0;$i<=count($lpono);$i++)
{
	...
	echo '<tr>
			...
			<td><input type="text" size="35" maxlength="55" autofocus="autofocus" required="required"name="particulars[]" value="'.$VVotehead.'" /></td>
