...
$sqlq = "INSERT INTO payment_voucher_approval (voucherno,
	...
	echo '<table style="width: 300px;
			...
			echo '<form method="post" name="DForm" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Application=PVM&Ref=default&Link=cheque" id="DForm">';
			echo '<tr>
				<td>
	...
	echo '<table style="width: 350px;
			...
			echo '<form method="post" name="ApproveForm" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Application=PVM&Ref=default&Link=cheque" id="ApproveForm">';
			echo '<tr><td>';
...
if (isset($_GET['view'])) {
	...
	$sql = "SELECT  a. voucherid,
		...
		LEFT JOIN  votepaymenttrans b ON a.voucherid=b.Voucherid
		WHERE a.voucherid='" . $SelectedUser . "'";
...
echo '<a href="' . $RootPath . '/index.php?Application=PVM&Ref=default&Link=cheque"><button type="button" name="" class="button"><i class="fa fa-reply"></i> ' . _('Go Back') . '</button></a>';
echo'<br>';
echo'<br>';
...
echo '<table style="width:360px; id="dataTable" class="selection">';
echo '<tr>
		<th>' .  _('Particulars')  . '</th>
		<th>' .  _('LPO No'). '</th>
		<th>' .  _('Invoice No'). '</th>
...
for($i=0;$i<=count($lpono);$i++)
{
 ...
 if ($myrow['Voucherid']!='') {
  ...
  $total = str_replace(',','',$myrow['total']);
		...
		//$total_paid = $total- $TAX;
if ($tax==6) {
/*   $TAX += (round(($total*(16/116) * (6/16)),2));
   $AmountPaid = ($total) - ($TaxTotals);*/ 
   $TAX += (round(($total*(16/116) * (6/16)),2));
   $AmountPaid = ($total) - ($TAX);
   
 }else if ($tax == 8){ 
 $TAX += (round(($total*(8/108) * (6/8)),2));
   $AmountPaid = ($total) - ($TAX); 
 }else if ($tax==5) {
   $TAX += (round(($total*(5/100)),2));
   $AmountPaid = ($Amount_Total) - ($TaxTotals); 
 }else if ($tax==3) {
   $TAX += (round(($total*(3/100)),2));
   $AmountPaid = ($total) - ($TaxTotals); 
 }else if ($tax==10) {
   $TAX += (round(($total*(10/100)),2));
   $AmountPaid = ($total) - ($TaxTotals); 
 }else if($tax==0) {
   $TAX += 0;
   $AmountPaid = ($total) - ($TaxTotals); 
...
echo '</table>';
echo '<form method="post" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Application=PVM&Ref=default&Link=cheque" id="form">';
...
######################################################################################
	...
	echo '</select></td>';
		...
		if(isset($_POST['ref']) && $_POST['ref'] !=""){
		 $sort=" AND a.authorityref " . LIKE . " '%". $_POST['ref'] ."%' AND authorityref " . LIKE . " '%". $_POST['Yearend'] ."%'";
		 }else{
		 $sort =" AND a.authorityref " . LIKE . " '%". $_POST['Yearend'] ."%'";
		 }		
		 }
...
$myrow9 = DB_fetch_array($result9);
	if($myrow9['level']==0){
		...
		/*$sql = "SELECT  * FROM payment_voucher,pvlevel
				WHERE payment_voucher.process_level=7
				AND pvlevel.levelcode=7
				AND payment_voucher.state=0
				".$sort."";*/
		$sql = "SELECT  *,c.Amount FROM payment_voucher a
		        INNER JOIN pvlevel b ON a.process_level=b.levelcode
				LEFT JOIN  votepaymenttrans c ON a.voucherid=c.Voucherid
				WHERE a.process_level=7
				AND b.levelcode=7
				AND a.state=0
				".$sort."";
		$sql2 = " payment_voucher a
		        INNER JOIN pvlevel b ON a.process_level=b.levelcode
				LEFT JOIN  votepaymenttrans c ON a.voucherid=c.Voucherid
				WHERE a.process_level=7
				AND b.levelcode=7
	...
	if (isset($_POST['Next'])) {
		...
		if ($ListPageMax > 1) {
			...
			echo '</div>';
				...
				<?php
				echo pagination($sqlforPages,$per_page,$page,$url='?Application=PVM&Ref=default&Link=cheque&');
				?>
                </div>
	...
	<tr style="height:30px;">
				...
				<th>Tax</th>
				<th>Amt to Pay</th>
				<th style="width:15px;">ChequeNo</th>
...
<?php
	...
	######################################################################################################################################33
	  //$TotalTax += (round(($myrow['tax']/(100+$myrow['tax']) * ($myrow['total'])),2));
   //$AmountPaid = $myrow['total'] - $TotalTax; 
   //$TotalTax += (round(($myrow['tax']/(100+$myrow['tax']) * ($myrow['total'])),2));
   //$AmountPaid = $myrow['total'] - $TotalTax; 
   	if ($myrow['Voucherid']!='') {
  $total = str_replace(',','',$myrow['Amount']);
  }else{
  $total = str_replace(',','',$myrow['total']);
  }
  //$total = str_replace(',','',$myrow['total']);
  $tax = $myrow['tax'];
	if ($tax==6) {
	   $TAX += (round(($total*(16/116) * (6/16)),2));
	   $AmountPaid = number_format($total-$TAX,2);	
	...
	//$AmountPaid34=number_format($total-$TAX,2)	
	#########################################################################################################################################
		if ($k==1){
		...
		$app ="<button type='button' onclick=pop2('popDiv2',".$myrow['voucherid'].",".$myrow['process_level'].",'".$myrow['chequeNo']."') name='Approve'  class='btn btn-primary'><i class='fa fa-share'></i> Approve</button>";
		 // $app='<td><a title="Approve"  onclick="return confirm(\'' . _('Are you sure you wish to approve this Voucher?') . '\');" href="'.htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8').'?Application=PVM&amp;Ref=default&amp;Link=inbox&amp;PV='.$myrow['voucherid'].'"><button type="button" class="btn btn-primary" name="Submit"><i class="fa fa-share"></i>' . _('Approve') . ' </button></a>';
		    printf('<td>%s</td>
					...
					<td>%s</td>		
					<td>%s</td>				
					<td><center><a href="%s&amp;SelectedUser=%s&amp;view=1">' . _('') . '</a>'.($myrow['process_level']>=33 ? '<a href="%s'.($myrow['supplierid']=="" ? '' : '?SupplierID=%s').'&amp;Vid='.$myrow['voucherid'].'"><button type="button" class="btn btn-success"><i class="fa fa-money"></i> ' . _('Make Payment') . '</button></a></center>' : ''.$app.'
				    '.$decl.'' ).'</td>
					</tr>',
					'<span class="label2 label-'.(($myrow['label']=="Urgent" or $myrow['label']=="Emergency")? 'danger':'success').'">'.$myrow['label'].'</span>',
					htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8')  . '?Application=PVM&Ref=default&Link=cheque',
					$myrow['voucherid'],
					$myrow['payeename'],
					0,
					$myrow['total'],
