...
echo '<a href="' . $RootPath . '/index.php?Application=PVM&Ref=default&Link=drafted_PV"><button type="button" name="" class="button"><i class="fa fa-reply"></i> ' . _('Go Back') . '</button></a>';
echo '<table style="background:none repeat scroll 0% 0% #F1F1F1; border:none; box-shadow:none;">
      <tr><td>';
echo '<table class="selection">
      <tr><td>';

	echo '<table style="width:620px;background-color: white;">
			<tr>
				<td>' . _('Voucher ID') . ':</td>
...
if (!isset($SelectedUser)) {
	...
	<table style="width:100%; font-size:12px;" id="dataTable" class="selection table table-hover">';
	echo '<tr><th>' . _('VID') . '</th>
				<th>' . _('Priority') . '</th>
				<th>' . _('Book No') . '</th>
				<th>' . _('Date Raised') . '</th>
	...
	if ($myrow['datereq']=='') {
		...
		$edit='<td><a href="%s&amp;SelectedUser=%s" title="Edit"><span class="glyphicon glyphicon-edit"></span></a></td>';
		...
		$process='<td><a href="%s&amp;SelectedUser=%s&amp;process=1" title="Process" onclick="return confirm(\'' . _('Are you sure you wish to Process this Voucher?') . '\');"><span class="glyphicon glyphicon-play-circle"></span></a></td>';
		$view='';
		}
if($myrow['process_level'] >=10){
       ...
       $print='<td><a target="_blank" href="PDFprintPV.php?voucher='.$SelectedUser.'">Print PV</a></td>';
		printf('<td>%s</td>
					...
					$myrow['payeename'],
					$myrow['total'],
					htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8')  . '?Application=PVM&Ref=default&Link=drafted_PV',
					$myrow['voucherid'],
					htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8')  . '?Application=PVM&Ref=default&Link=drafted_PV',
					$myrow['voucherid'],
					htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8')  . '?Application=PVM&Ref=default&Link=drafted_PV',
...
echo '<form method="post" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Application=PVM&Ref=default&Link=drafted_PV" id="form">
	<table>
		<tr>';
...
echo '<td><input name="" style="width:350px" value="'.$_POST['supplierid'].'" disabled type="text" /><input name="supplierid" value="'.$_POST['supid'].'" type="hidden" /></td>';
