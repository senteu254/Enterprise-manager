...
$sqlq = "INSERT INTO payment_voucher_approval (voucherno,
	...
	echo '<table style="width: 300px;
			...
			echo '<form method="post" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Application=PVM&Ref=Aged_payment_voucher" id="form">';
			echo '<tr>
				<td>
...
echo '<a href="' . $RootPath . '/index.php?Application=PVM&Ref=Aged_payment_voucher"><button type="button" name="" class="button"><i class="fa fa-reply"></i> ' . _('Go Back') . '</button></a>';
...
######################################################################################
	...
	echo '<table><tr>
	    <td>' . _('Supplier/Contrator Name') . ':</td>
...
if (!isset($SelectedUser)) {
  $sqlforPages = " payment_voucher
	    ...
	    $sql = "SELECT  * FROM payment_voucher
				WHERE process_level<7
				".$sort."
				ORDER BY datereq,voucherid DESC LIMIT {$startpoint} , {$per_page}";
	$result = DB_query($sql);
	
	echo pagination($sqlforPages,$per_page,$page,$url='?Application=PVM&Ref=Aged_payment_voucher&');
		}
		?>
	<table style="font-size:12px; width:95%;" class="selection table table-hover">
...
<?php
	...
	while (($rec = DB_fetch_array($result)) AND ($RowIndex <> ($_SESSION['DisplayRecordsMax']))) {
		if ($k==1){
		...
		$LastVisitDate = $rec['datereq'];

		/*The SecurityHeadings array is defined in config.php */
$VoucherNo=$rec['voucherid'];
echo '<form method="post" name="form" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Application=PVM&Ref=Aged_payment_voucher" id="form">';
        $decl="<a href=# onclick=pop('popDiv',".$rec['voucherid'].")><img src='".$RootPath."/css/decline.png'/> </a>";
		...
		//$days1 ='<td  style="color:red;">Active For '.calculate_time_span($rec['datereq']).'</td>';
		$days ='<td><span class="label2 label-'.(($rec['label']=="Urgent" or $rec['label']=="Emergency")? 'danger':'danger').'">Active For '.calculate_time_span(FormatDateForSQL($rec['datereq'])).'</span></td>';
     /*echo'<td>'.$rec['datereq'].'</td>
			<td>'.$rec['voucherid'].'</td>
			<td>'.$rec['label'].'</td>
			<td>'.$rec['authorityref'].'</td>
			<td>'.$rec['payeename'].'</td>
			<td>'.$rec['total'].'</td>
	        <td  style="color:red;">Active For '.calculate_time_span($rec['datereq']).'</td>
			</tr>';*/
		printf('<td>%s</td>
		            <td>%s</td>
					<td><a href="%s&amp;SelectedUser=%s&amp;view=1">' . $rec['authorityref']. '</a></td>
					<td>%s</td>
					<td>%s</td>
					'.$days.'
					'.$payment.'			
					...
					$LastVisitDate,
					'<span class="label2 label-'.(($rec['label']=="Urgent" or $rec['label']=="Emergency")? 'danger':'success').'">'.$rec['label'].'</span>',
					htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8')  . '?Application=PVM&Ref=Aged_payment_voucher',
					$rec['voucherid'],
					$rec['payeename'],
					$rec['total'],
					htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8')  . '?Application=PVM&Ref=Aged_payment_voucher',
