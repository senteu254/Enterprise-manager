...
$InputError = 1;
prnMsg( _('Employee Name can not be Empty'), 'error' );
}*/
...
##############################################################################################
	...
	echo '<input name="FormID" type="hidden" value="'.$_SESSION['FormID'].'" />';
		...
		?>
				...
				<?php echo'<select required="required" name="supplierid">';
		
		$sqlF = "SELECT  * FROM suppliers";
		$resultD = DB_query($sqlF);
	  echo '<option selected="selected" value="">--Select Supplier--</option>';
	  while ($myrowF=DB_fetch_array($resultD)){
		if (isset($_POST['supplierid' . $identifier]) &&  $myrowF['supplierid']==$_POST['suppname']){
			echo '<option selected="selected" value="'. $myrowF['supplierid'] . '">' . $myrowF['suppname'] . '</option>';
		} else {
			echo '<option value="'. $myrowF['supplierid'] . '">' . $myrowF['suppname'] . '</option>';
		}
	 }	
	 echo '</select>';
	 ?>		
  ...
  <?php	
	...
	?>				
	<?php echo'<input name="FormID" type="hidden" value="'.$_SESSION['FormID'].'" />	
	<select name="supplierid">';		
	  ...
	  while ($myrowF=DB_fetch_array($resultD)){
		if (isset($_POST['supplierid' . $identifier]) &&  $myrowF['supplierid']==$_POST['suppname']){
			...
			echo '<option value="'. $myrowF['supplierid'] . '">' . $myrowF['supplierid'] . '&nbsp;&nbsp;&nbsp;' . $myrowF['suppname'] . '</option>';
		}
	 }	
	 echo '</select>';
	 ?>			
  ...
  <div class="userfield" style="display:none">
  <?php 
  /*echo'<select name="Username">';		
		$sqlS = "SELECT  * FROM www_users";
		$resultS = DB_query($sqlS);
	  echo '<option selected="selected" value="">--Select Employee--</option>';
	  while ($myrowT=DB_fetch_array($resultS)){
		if (isset($_POST['emp_id' . $identifier]) &&  $myrowT['emp_id']==$_POST['realname']){
			echo '<option selected="selected" value="'. $myrowT['realname'] . '">' . $myrowT['realname'] . '</option>';
		} else {
			echo '<option value="'. $myrowT['realname'] . '">' . $myrowT['emp_id'] . '&nbsp;&nbsp;&nbsp;' . $myrowT['realname'] . '</option>';
		}
	 }	
	 echo '</select>';*/
	 ?>
	 ...
	 <br></br>
	 Comment:<textarea name="emp_comment" cols="50" rows="2"></textarea>
  </div>
  <?php
 echo'</td>';
	}	
