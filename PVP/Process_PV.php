...
echo '<form method="post" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" id="form">';
	...
	if(isset($_GET['Tab']) && $_GET['Tab']=="ActionedPV"){
	$sql = "SELECT voucherid,
					authorityref,
					datereq,
					label,
					...
					total
				FROM payment_voucher
				WHERE process_level !=1 and process_level <10";
	}elseif(isset($_GET['Tab']) && $_GET['Tab']=="RejectedPV"){
	$sql = "SELECT voucherid,
					authorityref,
					datereq,
					label,
...
##########################################################################################
if ($myrow['process_level'] ==1) {
echo '<form method="post" name="form" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" id="form">';
		...
		printf('<td>%s</td>
					...
					'.$app.'
					'.$decl.'
					<td><a href="%s&amp;SelectedUser=%s&amp;view=1">' . _('View Details') . '</a></td>
					</tr>',
					$myrow['voucherid'],
					$myrow['authorityref'],
					$myrow['label'],
