...
$sqlq = "INSERT INTO payment_voucher_approval (voucherno,
	...
	echo '<table style="width: 300px;
			...
			echo '<tr>
				<td>
				<textarea required name="comment" cols="30" style="width:100%" rows="4"></textarea>
	...
	echo '<table style="width: 350px;
		...
		//}
			...
			<tr><td><input type="submit" name="Submit" value="' . _('Submit') . '" />
				</form>';
...
if ($myrow['process_level'] < ($myrow['process_level']+1)){
			...
			echo ($myrow['process_level']>=7 ? '<a href="Payments.php'.($myrow['supplierid']=="" ? '?' : '?SupplierID='.$myrow['supplierid']).'&amp;Vid='.$myrow['voucherid'].'"><button type="button" class="btn btn-success"><i class="fa fa-money"></i> ' . _('Make Payment') . '</button></a></center>' : '<input type="submit" name="Submit" value="' . _('Approve') . '" />  <a href=# onclick=pop(\'popDiv\')><img src=\''.$RootPath.'/css/decline.png\'/> </a>' );
...
echo '</form>';
	...
	<td valign="top" class="status">
	<div style="background:url(css/statuspv.png) left top no-repeat; height:310px; width:220px;">
	...
	<div style="padding-left:70px; padding-top:15px; font-weight:bold;">'.$label7.' &nbsp;MD</div>
	<div style="padding-left:70px; padding-top:15px; font-weight:bold;">'.$label8.' &nbsp;Cash Payment</div>';
...
echo '<form method="post" name="form" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">';
		...
		$decl ="<button type='button' onclick=pop('popDiv',".$myrow['voucherid'].") name='Cancel'  class='btn btn-danger'><i class='fa fa-reply'></i> Decline</button>";
		$app ="<button type='button' onclick=pop2('popDiv2',".$myrow['voucherid'].",".$myrow['process_level'].",'".$myrow['chequeNo']."') name='Approve'  class='btn btn-primary'><i class='fa fa-share'></i> Approve</button>";
		 // $app='<td><a title="Approve"  onclick="return confirm(\'' . _('Are you sure you wish to approve this Voucher?') . '\');" href="'.htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8').'?Application=PVM&amp;Ref=default&amp;Link=inbox&amp;PV='.$myrow['voucherid'].'"><button type="button" class="btn btn-primary" name="Submit"><i class="fa fa-share"></i>' . _('Approve') . ' </button></a>';
		    printf('<td>%s</td>
					...
					<td>%s</td>
					<td>%s</td>					
					<td><center><a href="%s&amp;SelectedUser=%s&amp;view=1">' . _('') . '</a>'.($myrow['process_level']>=7 ? '<a href="%s?'.($myrow['supplierid']=="" ? '' : 'SupplierID=%s&amp;').'Vid='.$myrow['voucherid'].'"><button type="button" class="btn btn-success"><i class="fa fa-money"></i> ' . _('Make Payment') . '</button></a></center>' : ''.$app.'
				    '.$decl.'' ).'</td>
					</tr>',
					...
					htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8')  . '?Application=PVM&Ref=default&Link=cheque',
					$myrow['voucherid'],
					($myrow['process_level']>=7 ? 'Payments.php' : ''),
					$myrow['supplierid'],
					htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Application=PVM&Ref=default&Link=cheque',
					$myrow['voucherid'],
					($myrow['process_level']>=7 ? 'Payments.php' : ''),
...
?>
		...
		<script type="text/javascript">
			...
			function pop2(div,id, level,cheque) {
				...
				if(level==5){
				document.getElementById('showcheque').style.display = 'block';
				}
			}
			function hide(div) {
