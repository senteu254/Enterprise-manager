...
$sql_level = "SELECT process_level,votehead FROM payment_voucher WHERE voucherid='".$pv."'";
$result_level = DB_query($sql_level);
$myrow_level = DB_fetch_array($result_level);
if ($myrow_level['process_level']==6){

if(isset($_POST['ActionType']) && $_POST['ActionType']=='Cash'){

$sql = "UPDATE payment_voucher SET chequeNo='" .$cheque. "',tax='" .$tax. "' WHERE voucherid = '". $pv . "'";
		$result = DB_query($sql);
$SQL = "SELECT voucherno FROM  payment_voucher_approval  WHERE voucherno='".$pv."'";
...
$sqlq = "INSERT INTO payment_voucher_approval (voucherno,
	...
	$sql_7="SELECT *,supplierid FROM payment_voucher WHERE voucherid='". $pv . "'";
$result_7 = DB_query($sql_7);
$myrow_7 = DB_fetch_array($result_7);
$Supp_ID=$myrow_7['supplierid'];
	echo "<script>window.location.href = 'Payments.php".($Supp_ID=="" ? '?' : '?SupplierID='.$Supp_ID)."&Vid=". $pv . "'</script>";
	}else{
	$sql = "UPDATE payment_voucher SET chequeNo='" .$cheque. "',tax='" .$tax. "' WHERE voucherid = '". $pv . "'";
		$result = DB_query($sql);
$SQL = "SELECT voucherno FROM  payment_voucher_approval  WHERE voucherno='".$pv."'";
...
$sqlq = "INSERT INTO payment_voucher_approval (voucherno,
		...
		$resulta = DB_query($sqlq);
/////////////////////////////////////////////////////////////////////////////////////
$SQL222 = "SELECT Vid FROM  tax_deducted  WHERE Vid='".$pv."'";
	$result2332 = DB_query($SQL222);
  if(DB_num_rows($result2332) >0){	
  echo'Already you have selected tax to be deducted for the selected Payment Voucher,proceed without Selecting Tax for payment';
  }else if(DB_num_rows($resu2)==0){
$id=$_POST['selector'];
	$N = count($id);
	for($i=0; $i < $N; $i++){
/*	$sqltax = DB_query("SELECT *,b.tax_name,a.tax_percentage FROM  tax_deducted a 
	         INNER JOIN pv_tax b ON a.tax_percentage=b.ptid WHERE b.ptid='".$id[$i]."'");*/
$sqltax = DB_query("SELECT *,percentage FROM pv_tax WHERE ptid='".$id[$i]."'");
		while ($row_tax = DB_fetch_array($sqltax)) {
		$pecentage = $row_tax['ptid'];
	
		

$sqltax= "INSERT INTO tax_deducted (Vid,`taxed_from`,tax_percentage,amount,transdate)
					   VALUES ('" . $pv . "',
						'".$row_tax['total']."',
						'" . $pecentage ."',
						'',
						'".date('Y.m.d')."')";
		$result_tax = DB_query($sqltax);
...
////////////////////////////////////////////////////////////////////////////////////
	...
	$sql_78="SELECT *,supplierid FROM payment_voucher WHERE voucherid='". $pv . "'";
$result_78 = DB_query($sql_78);
$myrow_78 = DB_fetch_array($result_78);
$Supp_ID8=$myrow_78['supplierid'];
	echo "<script>window.location.href = 'Payments.php".($Supp_ID8=="" ? '?' : '?SupplierID='.$Supp_ID8)."&Vid=". $pv . "'</script>";
...
$sqlq = "INSERT INTO payment_voucher_approval (voucherno,
		...
		$sql = "UPDATE payment_voucher SET process_level=process_level+1,chequeNo='" .$cheque. "',comment='' WHERE voucherid = '". $pv . "'";
		$result = DB_query($sql);
$SQL = "SELECT voucherno FROM  payment_voucher_approval  WHERE voucherno='".$pv."' AND process_level=process_level+1";
  if(DB_num_rows($resu) ==0){
$sql2 = "SELECT process_level FROM payment_voucher WHERE voucherid = '". $pv . "'";
...
$sqlq = "INSERT INTO payment_voucher_approval (voucherno,
	...
	echo '<table style="width: 300px;
			...
			echo '<form method="post" name="DForm" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Application=PVM&Ref=default&Link=Amended_PV" id="DForm">';
			echo '<tr>
				<td>
	...
	echo '<table style="width: 350px;
			...
			echo '<form method="post" name="ApproveForm" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Application=PVM&Ref=default&Link=Amended_PV" id="ApproveForm">';
			echo '<tr><td>';
...
echo '<a href="' . $RootPath . '/index.php?Application=PVM&Ref=default&Link=Amended_PV"><button type="button" name="" class="button"><i class="fa fa-reply"></i> ' . _('Go Back') . '</button></a>';
echo'<br>';
echo'<br>';
echo '<table style="background:none repeat scroll 0% 0% #F1F1F1; border:none; box-shadow:none;">
      <tr>
	  <td>Reason for Rejection</td><td>'.$_POST['comment'].'</td>
	  </tr>
	  </table>';
echo '<table style="background:none repeat scroll 0% 0% #F1F1F1; border:none; box-shadow:none;">
      <tr><td>';
...
echo '</table>';
echo '<form method="post" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Application=PVM&Ref=default&Link=Amended_PV" id="form">';
if ($myrow['process_level']==6){
...
<?php
$result_tax=DB_query("SELECT *  FROM pv_tax");
	 while($myrow_tax = DB_fetch_array($result_tax)){	
	$taxcatid=$myrow_tax['ptid'];
	echo'<tr>';
    echo'<td width="40">
	<input id="" class="" name="selector[]" type="checkbox" value="'.$taxcatid. '"></td>
	<td>'.$myrow_tax['tax_name'].'</td>';
echo'</tr>';
	 }
echo '<tr>';
/*echo'<input type="hidden" class="form-control pull-right" id="product" name="voucherid[]" " value="'. $_POST['voucherid'] .'"  size="10"required>';
echo'<tr>
<td>Witholding Tax(6%)</td>
         <td><select  name="tax6_'.$_POST['voucherid'].'">';		  
	     echo'<option selected="selected" value="0">No</option>';
		 echo'<option value="6">Yes</option>';
	    echo '</select></td>
</tr>
<tr>
<td>Rental Income (10%)</td>
         <td><select  name="tax10_'.$_POST['voucherid'].'">';		  
	     echo'<option  selected="selected" value="0">No</option>';
		 echo'<option value="10">Yes</option>';
	    echo '</select></td>
</tr>
<tr>
<td>Contractual Fee (3%)</td>
         <td><select  name="tax3_'.$_POST['voucherid'].'">';		 
	      echo'<option selected="selected" value="0">No</option>';
...
<tr>
<td>VAT Fuel (8%)</td>
         <td><select  name="tax8_'.$_POST['voucherid'].'">';		  
	     echo'<option selected="selected" value="0">No</option>';
		 echo'<option value="3">Yes</option>';
	   echo '</select></td>
</tr>
<tr>
<td>Proffesianal.Fee(5%)</td>
         <td><select  name="tax5_'.$_POST['voucherid'].'">';		  
	     echo'<option selected="selected" value="0">No</option>';
		 echo'<option value="5">Yes</option>';
	   echo '</select></td>
</tr>';*/
 /* echo'<tr><td>Witholding Tax(6%)</td><td><input type="checkbox" style="width:25px; height:25px;" class="form-control" name="tax[]" unchecked="true" value="6"></td>
		<td>Rental Income (10%)</td><td><input type="checkbox" style="width:25px; height:25px;" class="form-control" name="tax[]" unchecked="true" value="10"></td></tr>
		<tr><td>Contractual Fee (3%)</td><td><input type="checkbox" style="width:25px; height:25px;" class="form-control" name="tax[]" unchecked="true" value="3"></td>
		<td>VAT Fuel (8%)</td><td><input type="checkbox" style="width:25px; height:25px;" class="form-control" name="tax[]" unchecked="true" value="8"></td></tr>
		<tr><td>Proffesianal.Fee(5%)</td><td><input type="checkbox" style="width:25px; height:25px;" class="form-control" name="tax[] unchecked="true" value="5"></td></tr>
	</td>';*/
	echo'</tr>';
/*echo'<td><select name="tax" required="required">
		  <option selected value="0">Tax</option>
		  <option value="10"> Rental Income (10%)</option>
		  <option value="3">Contractual Fee (3%)</option>
		  <option value="8">VAT Fuel (8%)</option>
		  <option value="5">Proffesianal.Fee(5%)</option>
		  <option value="6">Withholding (6%)</option>
	   </select>
</td>';*/
	echo'</tr>';
echo '<tr>
...
if ($myrow['process_level'] < ($myrow['process_level']+1)){
			...
			echo ($myrow['process_level']>=33 ? '<a href="Payments.php'.($myrow['supplierid']=="" ? '?' : '?SupplierID='.$myrow['supplierid']).'&amp;Vid='.$myrow['voucherid'].'"><button type="button" class="btn btn-success"><i class="fa fa-money"></i> ' . _('Make Payment') . '</button></a></center>' : '<input type="submit" name="Submit" value="' . _('Approve') . '" />  <a href=# onclick=pop(\'popDiv\')><img src=\''.$RootPath.'/css/decline.png\'/> </a>' );
...
$myrow9 = DB_fetch_array($result9);
	if($myrow9['level']==0){
		$sql = "SELECT  *,voucherid FROM payment_voucher,pvroles,pvlevel 
		...
		$sql = "SELECT  *,voucherid  FROM payment_voucher,pvroles,pvlevel
				...
				AND pvroles.authoriser='" . $_SESSION['UserID'] ."'
				AND payment_voucher.review_rejected=2
				".$sort."";
		$sql2 = " payment_voucher,pvroles,pvlevel
				WHERE payment_voucher.process_level=pvlevel.levelcode 
				AND pvlevel.levelcode=pvroles.level
	...
	if (isset($_POST['Next'])) {
		...
		if ($ListPageMax > 1) {
			...
			echo '</div>';
				...
				?>
				<?php
				echo pagination($sqlforPages,$per_page,$page,$url='?Application=PVM&Ref=default&Link=Amended_PV&');
				?>
      <!--new amended-->
 <style type="text/css">
  table.blueTable {
...
<!--end of amended-->       
	<table style="width:100%;font-size:12px;" class=blueTable>
...
echo '<form method="post" name="form" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">';
	...
	if($myrow['level']==6){
		...
		$decl1l='<div class="sidbutton" id='.$sid.' style="display:inline;border-radius:10px;" onclick=display_detail('.$myrow['voucherid'].')>
					<button type="button" class="btn btn-success">History</button></div>';
		 // $app='<td><a title="Approve"  onclick="return confirm(\'' . _('Are you sure you wish to approve this Voucher?') . '\');" href="'.htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8').'?Application=PVM&amp;Ref=default&amp;Link=inbox&amp;PV='.$myrow['voucherid'].'"><button type="button" class="btn btn-primary" name="Submit"><i class="fa fa-share"></i>' . _('Approve') . ' </button></a>';
		    printf('<td>%s</td>
					...
					<td>%s</td>
					<td>%s</td>			
					<td><a href="%s&amp;SelectedUser=%s&amp;view=1">' . _('') . '</a>'.($myrow['process_level']>=33 ? '<a href="%s'.($myrow['supplierid']=="" ? '' : '?SupplierID=%s').'&amp;Vid='.$myrow['voucherid'].'"><button type="button" class="btn btn-success"><i class="fa fa-money"></i> ' . _('Make Payment') . '</button></a>' : ''.$app.' '.$decl.' '.$app2.' '.$decl1l.'' ).'</td>
					
				</tr>',
					$myrow['votehead'],
					'<span class="label2 label-'.(($myrow['label']=="Urgent" or $myrow['label']=="Emergency")? 'danger':'success').'">'.$myrow['label'].'</span>',
					htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8')  . '?Application=PVM&Ref=default&Link=Amended_PV',
					$myrow['voucherid'],					
					$myrow['payeename'],
					$myrow['total'],
					htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8')  . '?Application=PVM&Ref=default&Link=Amended_PV',
					$myrow['voucherid'],
					($myrow['process_level']>=33 ? 'Payments.php' : ''),
					$myrow['supplierid'],
					htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Application=PVM&Ref=default&Link=Amended_PV',
					$myrow['voucherid'],
					($myrow['process_level']>=33 ? 'Payments.php' : ''),
			  ...
			  echo'<div  id='.$myrow['voucherid'].'>
			  <tbody>
					<tr style="height:30px; align:left;" class="child '.$sid.'child">
						<td width:3%><b>VoucherId :</b> </td><center><td><b>'.$myrow['voucherid'].'</b></td></center>
					</tr>
					<tr  style="height:30px;align:left;" class="child '.$sid.'child">	
						<td width:3%><b>Ref. No.</b> : </td><center><td><b>'.$myrow['authorityref'].'</b></td></center>
					</tr>
					<tr  style="height:60px;align:left;" class="child '.$sid.'child">	
						<td width:3%><b>Comment.</b> : </td><center><td style="color:red"><b>'.$myrow['comment'].'</b></b></td></center>
					</tr>
		...
		<script type="text/javascript">
			...
			if(level==6){
			 ...
			 document.getElementsByName('ApproveForm')[0].submit();
				...
				function pop3(div,id, level,cheque,tax) {
		
				var con = confirm("Are you sure you wish to approve this Voucher and Pay with Cash?");
				if(con){
				document.getElementById('pvs2').value = id;
  			 	document.getElementById('cheque').value = cheque;
				//document.getElementById('tax').value = check;
				document.getElementById('ActionType').value = 'Cash';
				document.getElementsByName('ApproveForm')[0].submit();
  ...
  for(i = 0; i < sidbuttons.length; i++) {
    if(sidbuttons[i].id == sid){
      if(sidbuttons[i].classList.contains('bopen')){
        sidbuttons[i].innerHTML = '<button type="button" class="btn btn-success">History</button>';
        sidbuttons[i].classList.remove('bopen');
      }else{
        sidbuttons[i].innerHTML = '<button type="button" class="btn btn-success">Hide</button>';
