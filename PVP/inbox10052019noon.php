...
$sql = "UPDATE payment_voucher SET process_level=process_level+1,chequeNo='" .$cheque. "',tax='" .$tax. "' WHERE voucherid = '". $pv . "'";
		$result = DB_query($sql);
		...
		$app2 ="";
		 ...
		 // $app='<td><a title="Approve"  onclick="return confirm(\'' . _('Are you sure you wish to approve this Voucher?') . '\');" href="'.htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8').'?Application=PVM&amp;Ref=default&amp;Link=inbox&amp;PV='.$myrow['voucherid'].'"><button type="button" class="btn btn-primary" name="Submit"><i class="fa fa-share"></i>' . _('Approve') . ' </button></a>';
		    printf('<td>%s</td>
					...
					<td>%s</td>
					<td>%s</td>				
					<td><center><a href="%s&amp;SelectedUser=%s&amp;view=1">' . _('') . '</a>'.($myrow['process_level']>=33 ? '<a href="%s'.($myrow['supplierid']=="" ? '' : '?SupplierID=%s').'&amp;Vid='.$myrow['voucherid'].'"><button type="button" class="btn btn-success"><i class="fa fa-money"></i> ' . _('Make Payment') . '</button></a></center>' : ''.$app.' '.$decl.' '.$app2.'' ).'</td>
					</tr>',
					$myrow['votehead'],
