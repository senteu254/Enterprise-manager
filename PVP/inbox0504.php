...
$sqlq = "INSERT INTO payment_voucher_approval (voucherno,
	...
	echo "<script>window.location.href = 'Payments.php?Vid=". $pv . "'</script>";
	}else{
	$sql = "UPDATE payment_voucher SET process_level=process_level+1,chequeNo='" .$cheque. "' WHERE voucherid = '". $pv . "'";
		$result = DB_query($sql);
$SQL = "SELECT voucherno FROM  payment_voucher_approval  WHERE voucherno='".$pv."' AND process_level=process_level+1";
...
if (isset($_GET['view'])) {
	...
	$_POST['total'] = $myrow['total'];
		$sql_votehead = "SELECT * FROM voteheadmaintenance WHERE Votecode='" . $myrow['votehead'] . "'";
	$result_votehead = DB_query($sql_votehead);
	$head = DB_fetch_array($result_votehead);
	$Votehead = $head['Votehead'];
	/////////////////////////////////////////////////////////////////////////////////////////
...
if ($myrow['process_level'] < ($myrow['process_level']+1)){
			...
			echo ($myrow['process_level']==6 ? '<a href="Payments.php'.($myrow['supplierid']=="" ? '?' : '?SupplierID='.$myrow['supplierid']).'&amp;Vid='.$myrow['voucherid'].'"><button type="button" class="btn btn-success"><i class="fa fa-money"></i> ' . _('Make Payment') . '</button></a></center>' : '<input type="submit" name="Submit" value="' . _('Approve') . '" />  <a href=# onclick=pop(\'popDiv\')><img src=\''.$RootPath.'/css/decline.png\'/> </a>' );
...
echo '<form method="post" name="form" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">';
		...
		$decl ="<button type='button' onclick=popdecline('popDiv',".$myrow['voucherid'].") name='Cancel'  class='btn btn-danger'><i class='fa fa-reply'></i> Decline</button>";
			if($myrow['level']==6){
	    $app ="<button type='button' onclick=pop2('popDiv2',".$myrow['voucherid'].",".$myrow['process_level'].",'".$myrow['chequeNo']."') name='Approve'  class='btn btn-primary'><i class='fa fa-share'></i> Cheque</button>";
		...
		$app2 ="";
		 ...
		 //$app='<a title="Approve"  onclick="return confirm(\'' . _('Are you sure you wish to approve this Voucher?') . '\');" href="Payments.php'.($myrow['supplierid']=="" ? '?' : '?SupplierID='.$myrow['supplierid'].'&amp;').'Vid='.$myrow['voucherid'].'"><button type="button" class="btn btn-primary" name="Submit"><i class="fa fa-share"></i>' . _('Approve') . ' </button></a>';
		    printf('<td>%s</td>
		            <td><a href="%s&amp;SelectedUser=%s&amp;view=1">' . $myrow['authorityref']. '</a></td>
					<td>%s</td>
					<td>%s</td>				
					<td><center><a href="%s&amp;SelectedUser=%s&amp;view=1">' . _('') . '</a>'.($myrow['process_level']==8? '<a href="%s'.($myrow['supplierid']=="" ? '?' : '?SupplierID='.$myrow['supplierid'].'&amp;').'Vid='.$myrow['voucherid'].'"><button type="button" class="btn btn-primary"><i class="fa fa-money"></i> ' . _('Approve') . '</button></a></center>' : ''.$app.' '.$decl.' '.$app2.'' ).'</td>
					</tr>',
