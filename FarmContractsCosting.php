...
include('includes/DefineFarmContractClass.php');
include('includes/session.inc');
$Title = _(' Farm Contract Costing');
...
$sql = "SELECT stockmoves.stockid,
		...
		FROM stockmoves INNER JOIN  farmdescriptions
		ON stockmoves.stockid= farmdescriptions. description_Id
...
while ($InventoryIssuesRow = DB_fetch_array($InventoryIssuesResult)){
	$InventoryIssues[$InventoryIssuesRow['stockid']]['descritption_Id'] = $InventoryIssuesRow['stockid'];
...
if ($_SESSION['Contract'.$identifier]->Status==3){
	echo _('Closed')  . ' ';
} elseif ($_SESSION['Contract'.$identifier]->Status==0){
...
foreach ($_SESSION['Contract'.$identifier]->ContractBOM as $Component) {
	...
	if (isset($InventoryIssues[$Component->StockID])){
		$InventoryIssues[$Component->StockID]['Matched']=0;
...
foreach ($InventoryIssues as $Component) { //actual inventory components used
	...
	if ($Component['Matched'] == 0) { //then its a component that wasn't budget for
		echo '<tr>
				<td colspan="6"></td>
				<td>' . $Component['description_Id'] . '</td>
