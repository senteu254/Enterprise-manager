...
if (isset($_GET['ModifyOrderNumber'])) {
	$Title = _('Modify Local Purchase Order') . ' ' . $_GET['ModifyOrderNumber'];
} else {
	$Title = _('Local Purchase Order Entry');
}
...
if (isset($_POST['EnterLines']) OR isset($_POST['AllowRePrint'])) {
	...
	prnMsg(_('Local Purchase Order Number cannot be empty') . '. ', 'warn');
	$error = 1;
	}
	...
	prnMsg(_('local Purchase Order Number entered already exist in the system. Please try again') . '. ', 'warn');
	$error = 1;
	}else{
	...
	if (isset($_POST['RePrint']) AND $_POST['RePrint'] == 1) {
		...
		$ErrMsg = _('An error occurred updating the local purchase order to allow reprints') . '. ' . _('The error says');
		$UpdateResult = DB_query($sql, $ErrMsg);
...
echo '<span style="float:left"><a href="' . $RootPath . '/PO_SelectOSPurchOrder.php?identifier=' . $identifier . '">' . _('Back to Local Purchase Orders') . '</a></span>';
...
if (isset($_POST['Select'])) {
	...
	if (($AuthRow = DB_fetch_array($AuthResult) and $AuthRow['cancreate'] == 0)) {
		...
		prnMsg(_('You do not have the authority to raise Local Purchase Orders for') . ' ' . $myrow['suppname'] . '. ' . _('Please Consult your system administrator for more information.') . '<br />' . _('You can setup authorisations') . ' ' . '<a href="PO_AuthorisationLevels.php">' . _('here') . '</a>', 'warn');
		include('includes/footer.inc');
		exit;
	}
...
if ($_SESSION['RequireSupplierSelection'] == 1 OR !isset($_SESSION['PO' . $identifier]->SupplierID) OR $_SESSION['PO' . $identifier]->SupplierID == '') {
	echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/supplier.png" title="' . _('Purchase Order') . '" alt="" />' . ' ' . _('Local Purchase Order: Select Supplier') . '</p>';
	...
	if (isset($Purch_Item)) {
		...
		/* the link */
		echo '<a href="' . $RootPath . '/PO_Items.php?NewItem=' . $Purch_Item . '&identifier=' . $identifier . '">' . _('Enter Line Item to this Local purchase order') . '</a>';
	...
	echo '<tr>
	<td>' . _('LPO No.') . ':</td>
	<td><input type="text" required="required" autofocus="autofocus" name="PONo" class="number" size="16" value="" /></td>
	</tr>';
	}
	echo '<tr>
			<td>' . _('LPO Date') . ':</td>
	...
	if ($_SESSION['PO' . $identifier]->Status == '') { //then its a new order
		echo '<tr>
				<td><input type="hidden" name="Status" value="NewOrder" />' . _('New Local Purchase Order') . '</td>
			</tr>';
	} else {
		echo '<tr>
				<td>' . _('Status') . ' :  </td>
