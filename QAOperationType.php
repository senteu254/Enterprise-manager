...
$Title = _('Hardness Annealing Operation Types');
...
if (isset($_POST['Submit'])) {
	...
	if (trim($_POST['MeasureName']) == '') {
		$InputError = 1;
		prnMsg( _('The Operation Type may not be empty'), 'error');
	}
	...
	if (isset($_POST['SelectedMeasureID']) AND $_POST['SelectedMeasureID']!='' AND $InputError !=1) {
		...
		$sql = "SELECT count(*) FROM qaoperationtype
				WHERE id <> '" . $SelectedMeasureID ."'
				AND typename ".LIKE." '" . $_POST['MeasureName'] . "'";
		$result = DB_query($sql);
		$myrow = DB_fetch_row($result);
		if ( $myrow[0] > 0 ) {
			$InputError = 1;
			prnMsg( _('The Operation Type can not be renamed because another with the same name already exist.'),'error');
		} else {
			...
			// relavant
			$sql = "SELECT typename FROM qaoperationtype
				WHERE id = '" . $SelectedMeasureID . "'";
			$result = DB_query($sql);
			if ( DB_num_rows($result) != 0 ) {
				...
				$sql = array();
				$sql[] = "UPDATE qaoperationtype
					SET typename='" . $_POST['MeasureName'] . "',
					operationid='" . $_POST['operation'] ."'
					WHERE typename ".LIKE." '".$OldMeasureName."'";
				
			} else {
				$InputError = 1;
				prnMsg( _('The operation type no longer exist.'),'error');
			}
		}
		$msg = _('Operation Type changed successfully');
	} elseif ($InputError !=1) {
		/*SelectedMeasureID is null cos no item selected on first time round so must be adding a record*/
		$sql = "SELECT count(*) FROM qaoperationtype
				WHERE typename " .LIKE. " '".$_POST['MeasureName'] ."'";
		$result = DB_query($sql);
		$myrow = DB_fetch_row($result);
		if ( $myrow[0] > 0 ) {
			$InputError = 1;
			prnMsg( _('The Operation Type can not be created because it already exists.'),'error');
		} else {
			$sql = "INSERT INTO qaoperationtype (typename,operationid)
					VALUES ('" . $_POST['MeasureName'] ."','" . $_POST['operation'] ."')";
		}
		$msg = _('New Operation Type Added successfully');
	}
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'stockmaster'
	...
	if ( DB_num_rows($result) == 0 ) {
		// This is probably the safest way there is
		prnMsg( _('Cannot delete this Operation Type because it no longer exist'),'warn');
	} else {
		$myrow = DB_fetch_row($result);
		$OldMeasureName = $myrow[0];
			$sql="DELETE FROM qaoperationtype WHERE typename ".LIKE."'" . $OldMeasureName . "'";
			$result = DB_query($sql);
			prnMsg( $OldMeasureName . ' ' . _('Operation Type has been deleted successfully') . '!','success');
...
/* An unit of measure could be posted when one has been edited and is being updated
  ...
  or deletion of the records*/
	...
	$sql = "SELECT a.id,
			a.typename,
			b.operation
			FROM qaoperationtype a
			LEFT JOIN qaoperation b ON a.operationid=b.id
			ORDER BY a.id";
	...
	while ($myrow = DB_fetch_row($result)) {
		...
		echo '<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?SelectedMeasureID=' . $myrow[0] . '&amp;delete=1" onclick="return confirm(\'' . _('Are you sure you wish to delete this Operation Type?') . '\');">' . _('Delete')  . '</a></td>';
		echo '</tr>';
...
if (isset($SelectedMeasureID)) {
	echo '<div class="centre">
			<a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">' . _('Review Operation Type') . '</a>
		</div>';
}
...
if (! isset($_GET['delete'])) {
	...
	if (isset($SelectedMeasureID)) {
		...
		if ( DB_num_rows($result) == 0 ) {
			...
			$_POST['MeasureName']  = $myrow['typename'];
	...
	echo '<option '.($_POST['operation']==$myr['id']? 'selected':'').' value="'.$myr['id'].'">'.$myr['operation'].'</option>';
	}
	echo '</select></td>
		</tr>';
	echo '<tr>
