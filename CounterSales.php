...
/*CustomerID variable registered by header.inc */
		...
		$_SESSION['Items' . $identifier]->CurrDecimalPlaces = $myrow['decimalplaces'];$_SESSION['Items' . $identifier]->SalesPerson = $myrow['salesperson'];
...
if (isset($_POST['ProcessInvoice']) AND $_POST['ProcessInvoice'] != ''){
	...
	//}
	foreach ($_SESSION['Items'.$identifier]->LineItems as $OrderLine) {
	if ($OrderLine->Controlled==1 && $_SESSION['Items'.$identifier]->TRANSACTION !="SALES ORDER"){
	...
	/* Insert the tax totals for each tax authority where tax was charged on the invoice */
		...
		/*Insert the taxes that applied to this line */
			...
			/*-----------------------------------------------------------------------------------------------------------------------------------*/
					...
					$DbgMsg = _('The following SQL to insert the serial stock movement records was used');
					$Result = DB_query($SQL, $ErrMsg, $DbgMsg, true);
					$QtyDispatched += $Item->BundleQty;
			/*-----------------------------------------------------------------------------------------------------------------------------------*/
				}// foreach controlled item in the serialitems array
			} //end if the orderline is a controlled item
			else{
			$QtyDispatched = $OrderLine->Quantity;
			}
...
if($NewSalesOder == TRUE){
	...
	$SQLi = "UPDATE debtortrans
					...
					ovgst = ovgst + " . filter_number_format($_POST['TaxTotal']) . ",
...
$SQLs = "UPDATE debtortranstaxes
		...
		//Loop around each item on the sale and process each in turn
		foreach ($_SESSION['Items'.$identifier]->LineItems as $OrderLine) {
		$SQL = "SELECT (quantity-qtyinvoiced) FROM salesorderdetails
							WHERE orderno = '" . $OrderNo . "'
							AND orderlineno = '" . $OrderLine->LineNumber . "'";
		$Result = DB_query($SQL);
		$ROS=DB_fetch_row($Result);
		
		if ($OrderLine->Quantity>=$ROS[0]){
