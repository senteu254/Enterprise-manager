...
if (!isset($_SESSION['PO'.$identifier])) {
	...
	echo '<div class="centre">
			<a href="' . $RootPath . '/PO_SelectOSPurchOrderOverseas.php">' . _('Select a Contract to receive'). '</a>
		</div>
		<br />';
...
if (isset($_GET['LineNo']) AND $_GET['LineNo']>0){
	...
	echo '<div class="centre">
			<a href="' . $RootPath . '/GoodsReceivedOverseas.php">' . _('Select a line Item to Receive') . '</a>
		</div>';
	prnMsg( _('This page can only be opened if a Line Item on a Contract has been selected') . '. ' . _('Please do that first'), 'error');
	include( 'includes/footer.inc');
	exit;
}
...
if ($LineItem->Controlled !=1 ){ /*This page only relavent for controlled items */
	...
	echo '<div class="centre">
			<a href="' . $RootPath . '/GoodsReceivedOverseas.php">' . _('Back to the Contract Order'). '</a>
		</div>';
...
echo '<div class="centre">
		<br />
		<a href="'.$RootPath.'/GoodsReceivedOverseas.php?identifier=' .$identifier . '">' .  _('Back To Contract Order'). ' # '. $_SESSION['PO'.$identifier]->OrderNo . '</a>
		<br />
