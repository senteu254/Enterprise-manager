...
if($days ==0){
prnMsg(_("Invalid Dates! Please review and try again"),'error');
	echo '<p></p>';
}elseif($from < date('Y-m-d')){
prnMsg(_("Date From cannot be Earlier than today! Please review and try again"),'error');
	echo '<p></p>';
}elseif($to < date('Y-m-d')){
prnMsg(_("Date To cannot be Earlier than today! Please review and try again"),'error');
	echo '<p></p>';
}elseif($check[0] >0){
...
$narrative = 'Application to <b>be away from duty</b> from <b>'.date('d/m/Y',strtotime($_POST['from'])).'</b> to <b>'.date('d/m/Y',strtotime($_POST['to'])).'</b>. Number of days applied for <b>'.$days.'</b> days. Reason being <b>'.$reason.'</b>. My destination will be <b>'.$dest.'</b>.';
$ErrMsg = _('The Information cannot be inserted because');
$sql = "INSERT INTO `leave_off_duty`(`off_id`, `emp_id`, `leave_type`, `from_date`, `to_date`, `days`, `reason`, `destination`, `narrative`, `send`, `added_by`) 
									VALUES (".$LeaveID.",'".$userid."',1,'".$from."','".$to."','".$days."','".$reason."','".$dest."', '".$narrative."','".$send."','".$_SESSION['UserID']."')";
...
<?php
        ...
        <label class="onoffswitch-label" for="myonoffswitch">
			...
			<?php
			echo '<input type="text" class="date" onfocus="findDateDiff()" alt="m/d/Y" name="from" value="'.date('m/d/Y').'" id="form-control" required="" />';
			?>
			  </div>
			<div class="col-md-5">To: (Format: mm/dd/yyyy)
			<?php
			echo '<input type="text" class="date" onfocus="findDateDiff()" alt="m/d/Y" name="to" value="'.date('m/d/Y').'" id="form-control" required="" />';
			?>
			 </div>
			 <div class="col-md-2">No of Days
				<input id="days" onmouseover="findDateDiff()" name="days" readonly="" type="text" class="form-control input-md" />
			 </div>
				</div>
			  <div class="form-group">
				<div class="col-md-7">Reason:
				<textarea name="reason" class="form-control input-md" required="" ></textarea>
			 </div>
			<div class="col-md-5">Destination:
				<input name="destination"  type="text"  class="form-control input-md" required=""/>
			 </div>
				</div>
            </div>
