...
$narrative = 'Application of <b>Compassionate Leave</b> from <b>'.date('d/m/Y',strtotime($_POST['from'])).'</b> to <b>'.date('d/m/Y',strtotime($_POST['to'])).'</b>. Number of days applied for <b>'.$days.'</b> days. While I will be on leave, my duties will be performed by <b>'.$duties.'</b>. My contact address while on leave will be <b>'.$contact.'</b>.';
...
$sql = "INSERT INTO `leave_annual`(`leave_id`, `emp_id`, `leave_type`,`type`, `from_date`, `to_date`, `days`, `assigned_duties`, `contact`,`em_name`, `em_address`, `em_phone`, `narrative`, `send`, `added_by`) 
									VALUES (".$LeaveID.",'".$userid."',".$leaveid.",6,'".$from."','".$to."','".$days."','".$duties."','".$contact."','".$name."','".$add."','".$phone."','".$narrative."','".$send."','".$_SESSION['UserID']."')";
