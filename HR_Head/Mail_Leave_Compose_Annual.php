...
echo _('<div class="alert alert-danger alert-dismissible">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true"><i class="fa fa-window-close"></i></button>
                <h4><i class="icon fa fa-warning"></i> Alert!</h4>
              Days you are trying to apply is more than the available leave days! Please review and try again.
              </div>');
}elseif($check[0] >0){
echo _('<div class="alert alert-danger alert-dismissible">
                <button type="button" class="close" data-dismiss="alert" aria-hidden="true"><i class="fa fa-window-close"></i></button>
...
$narrative = 'Application of <b>Annual Leave</b> from <b>'.date('d/m/Y',strtotime($_POST['from'])).'</b> to <b>'.date('d/m/Y',strtotime($_POST['to'])).'</b>. Leave entitlement/balance <b>'.$Avdays.'</b> days. Number of days applied for <b>'.$days.'</b> days. Leave balance <b>'.($Avdays-$days).'</b> days. While I will be on leave, my duties will be performed by <b>'.$duties.'</b>. My contact address while on leave will be <b>'.$contact.'</b>.';
...
$sql = "INSERT INTO `leave_annual`(`leave_id`, `emp_id`, `leave_type`,`type`, `from_date`, `to_date`, `days`, `assigned_duties`, `contact`,`em_name`, `em_address`, `em_phone`, `narrative`, `send`, `added_by`) 
									VALUES (".$LeaveID.",'".$userid."',".$leaveid.",3,'".$from."','".$to."','".$days."','".$duties."','".$contact."','".$name."','".$add."','".$phone."','".$narrative."','".$send."','".$_SESSION['UserID']."')";
...
<?php
			...
			function LeaveDaysPre(){
			if(isset($_POST['empid']) && $_POST['empid'] !=""){
			$employ = $_POST['empid'];
			}else{
			$employ = $_SESSION['UserID'];
			}
			$sqla="SELECT IFNULL((a.opening_bal+a.leave_days),0) as days, 
						IFNULL((SELECT SUM(days) FROM leave_annual b
							...
							WHERE a.emp_id='".$employ."' AND a.year='".date('Y')."' AND a.leave_type=3";
			...
			<?php
				$ErrMsg = _('An error occurred in retrieving the records');
						...
						$results = "SELECT * FROM employee a
											INNER JOIN leave_days_allocation b ON a.emp_id = b.emp_id
						...
						$results = "SELECT * FROM employee a
											...
											INNER JOIN www_users c ON a.emp_id=c.emp_id
											WHERE b.year='".date('Y')."' 
											AND b.leave_type=3 AND c.userid='".$_SESSION['UserID']."'";
						$option ='';
							}
						$welcome_viewed = DB_query($results,$ErrMsg,$DbgMsg);
					if(DB_num_rows($welcome_viewed) >=1){
					echo '<div class="col-md-5">Applicant';
					echo '<select name="empid" class="form-control input-md" required onChange="LeaveDays(this);">';
					echo $option;
				...
				?>
				<div id="output"><?php echo LeaveDaysPre(); ?></div>
