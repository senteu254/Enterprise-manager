...
echo '<div class="page_help_text">' . _('There is No.payment voucher select to be payed at the moment,click the button to select voucher.') . '</div>
	<br />';
echo'<br>';
echo '<a href="' . $RootPath . '/index.php?Application=PVM"><button type="button" name="" class="button"><i class="fa fa-reply"></i> ' . _('View Payment Voucher to Approve') . '</button></a>';
exit;
}
#################################################################################################3
...
if (isset($_POST['VoteHead']) AND $_POST['VoteHead']!=''){
	...
	echo '<tr>';	
		...
		$_SESSION['PaymentDetail' . $identifier]->TaxTotals6 += (round(($_SESSION['PaymentDetail' . $identifier]->Amount_Total*(16/116) * (6/16)),2));
				}
...
$SQL = "INSERT INTO cash (`type`,TransNo,Payment_Type,FVoucherNo,VoucherID,Account, date,`descs`, `debit`, `credit`) ";
			...
			$SQL = $SQL . "VALUES ('" . $_SESSION['PaymentDetail' . $identifier]->VoucherID . "',
			                      '" . $_SESSION['PaymentDetail' . $identifier]->SupplierID . "',
								   ...
								   '" . $_SESSION['PaymentDetail' . $identifier]->TaxTotals. "',								  
								   '" .date('Y.m.d')."')";
...
$sql_update = "UPDATE payment_voucher SET process_level=process_level+1,chequeNo='" . $_SESSION['PaymentDetail' . $identifier]->ChequeNum . "' WHERE voucherid = '". $_SESSION['PaymentDetail' . $identifier]->VoucherID . "'";
