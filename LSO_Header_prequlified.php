...
if (isset($_GET['ModifyOrderNumber'])) {
	$Title = _('Modify Local Service Order') . ' ' . $_GET['ModifyOrderNumber'];
} else {
	$Title = _('Local Service Order Entry');
}

if (isset($_GET['SupplierID'])) {
	$_POST['Select'] = $_GET['SupplierID'];
}
if (isset($_SESSION['RequestStockID'])
	AND isset($_SESSION['Requisition_No'])){
	$_POST['Requisition'] = $_SESSION['Requisition_No'];
}

/* webERP manual links before header.inc */
$ViewTopic= 'LocalPurchaseOrdering';
$BookMark = 'LocalPurchaseOrdering';
	...
	if ($_SESSION['ExistingOrder'] == 0) {
		...
		if ($_POST['Status'] == 'Pending') {
			...
			if (($AuthorityLevel > $OrderTotal OR $_SESSION['UserID'] == $_SESSION['PO' . $identifier]->Initiator) AND $OKToUpdateStatus == 1) {
				...
				$OKToUpdateStatus = 0;
				prnMsg(_('You do not have permission to change the status of this Local Service order') . '.<br />' . _('This order is for') . ' ' . $_SESSION['PO' . $identifier]->CurrCode . ' ' . $OrderTotal . '. ' . _('Your authorisation limit is set at') . ' ' . $_SESSION['PO' . $identifier]->CurrCode . ' ' . $AuthorityLevel . '.<br />' . _('If you think this is a mistake please contact the systems administrator'), 'warn');
		...
		if ($OKToUpdateStatus == 1) {
			...
			$SQL = "UPDATE lsorders SET status='" . $_POST['Status'] . "',
							stat_comment='" . $_SESSION['PO' . $identifier]->StatusComments . "',
							allowprint='" . $AllowPrint . "'
					WHERE lsorders.orderno ='" . $_SESSION['ExistingOrder'] . "'";
			...
			if ($_POST['Status']=='Completed' OR $_POST['Status']=='Cancelled' OR $_POST['Status']=='Rejected') {
				$SQL = "UPDATE lsorderdetails SET completed=1 WHERE orderno='" . $_SESSION['ExistingOrder'] . "'";
				$UpdateResult =DB_query($SQL,$ErrMsg);
			} else {//To ensure that the purchorderdetails status is correct when it is recovered from a cancelled orders
				$SQL = "UPDATE lsorderdetails SET completed=0 WHERE orderno='" . $_SESSION['ExistingOrder'] . "'";
				$UpdateResult = DB_query($SQL,$ErrMsg);
			}
		} //$OKToUpdateStatus == 1
...
if (isset($_POST['EnterLinesf']) OR isset($_POST['AllowRePrint'])) {
	/*User hit the button to enter line items -
	 *  ensure session variables updated then meta refresh to PO_Items.php*/
    $error = 0;
	/*User hit the button to enter line items -
	 *  ensure session variables updated then meta refresh to PO_Items.php*/
	if(!isset($_POST['LSONo']) or $_POST['LSONo'] == ""){
	prnMsg(_('Service Order Number cannot be empty') . '. ', 'warn');
	$error = 1;
	}
	$sql = "SELECT orderno FROM lsorders
				WHERE lsorders.orderno='" . $_POST['LSONo'] . "'";
	$ErrMsg = _('An error occurred checking the Service order existance') . '. ' . _('The error says');
	$Result = DB_query($sql, $ErrMsg);
	
	if(DB_num_rows($Result) >0){
	prnMsg(_('Service Order Number entered already exist in the system. Please try again') . '. ', 'warn');
	$error = 1;
	}else{

	$_SESSION['PO'.$identifier]->OrderNo = $_POST['LSONo'];
	...
	if (isset($_POST['RePrint']) AND $_POST['RePrint'] == 1) {
		...
		$sql = "UPDATE lsorders
				SET lsorders.allowprint='1'
				WHERE lsorders.orderno='" . $_SESSION['PO' . $identifier]->OrderNo . "'";

		$ErrMsg = _('An error occurred updating the Local Service order to allow reprints') . '. ' . _('The error says');
		$UpdateResult = DB_query($sql, $ErrMsg);
	...
	if (!isset($_POST['AllowRePrint'])) { // user only hit update not "Enter Lines"
		echo '<meta http-equiv="Refresh" content="0; url=' . $RootPath . '/LSO_ServiceItems.php?identifier=' . $identifier . '">';
		echo '<p>';
		prnMsg(_('You should automatically be forwarded to the entry of the Local Service order line items page') . '. ' . _('If this does not happen') . ' (' . _('if the browser does not support META Refresh') . ') ' . '<a href="' . $RootPath . '/LSO_ServiceItems.php?identifier=' . $identifier . '">' . _('click here') . '</a> ' . _('to continue'), 'info');
		include('includes/footer.inc');
		exit;
	} 
	// end if reprint not allowed
}else if (isset($_POST['EnterLines']) OR isset($_POST['AllowRePrint'])) {
	/*User hit the button to enter line items -
	 *  ensure session variables updated then meta refresh to PO_Items.php*/
    	/*User hit the button to enter line items -
	 *  ensure session variables updated then meta refresh to PO_Items.php*/
	...
	$sql = "SELECT orderno FROM lsorders
				WHERE lsorders.orderno='" . $_POST['LSONo'] . "'";
	$ErrMsg = _('An error occurred checking the Local Service order existance') . '. ' . _('The error says');
	$Result = DB_query($sql, $ErrMsg);
	
	if(DB_num_rows($Result) >0){
	prnMsg(_('Local Service Order Number entered already exist in the system. Please try again') . '. ', 'warn');
	$error = 1;
	}else{
	$_SESSION['PO'.$identifier]->OrderNo = $_POST['LSONo'];
	$_SESSION['PO' . $identifier]->Location = $_POST['StkLocation'];
	...
	if (!isset($_POST['AllowRePrint'])) { // user only hit update not "Enter Lines"
		echo '<meta http-equiv="Refresh" content="0; url=' . $RootPath . '/LSO_FarmItems.php?identifier=' . $identifier . '">';
		echo '<p>';
		prnMsg(_('You should automatically be forwarded to the entry of the Local Service order line items page') . '. ' . _('If this does not happen') . ' (' . _('if the browser does not support META Refresh') . ') ' . '<a href="' . $RootPath . '/LSO_FarmItems.php?identifier=' . $identifier . '">' . _('click here') . '</a> ' . _('to continue'), 'info');
		include('includes/footer.inc');
		exit;
	} // end if reprint not allowed
...
echo '<span style="float:left"><a href="' . $RootPath . '/LSO_SelectOSPurchOrder.php?identifier=' . $identifier . '">' . _('Back to Local Service Orders') . '</a></span>';
...
if (isset($_GET['ModifyOrderNumber'])) {
	include('includes/SO_ReadInOrder.inc');
}
...
if (isset($_POST['SearchSuppliers'])) {
	...
	if (mb_strlen($_POST['Keywords']) > 0) {
		...
		$SQL = "SELECT suppliers.supplierid,
							...
							suppliers.address6,
							suppliers.currcode,
							prequalifiedsuppliers.supprid,
							prequalifiedsuppliers.CatID,
							prequalifiedsuppliers.FY,
							prequalifiedsuppliers.name
						FROM suppliers 
						INNER JOIN prequalifiedsuppliers ON suppliers.supplierid=prequalifiedsuppliers.supprid
						WHERE prequalifiedsuppliers.FY = '".  Date($_SESSION['DefaultDateFormat'],YearEndDate($_SESSION['YearEnd'],0)) . "'
						GROUP BY suppliers.supplierid
						ORDER BY prequalifiedsuppliers.name";
		} else {			
		$SQL = "SELECT suppliers.supplierid,
							suppliers.suppname,
							suppliers.supptype,
						...
						FROM suppliers
						INNER JOIN prequalifiedsuppliers ON suppliers.supplierid=prequalifiedsuppliers.supprid
						WHERE prequalifiedsuppliers.name " . LIKE . " '" . $SearchString . "'
						AND prequalifiedsuppliers.CatID='" . $_POST['SelectedCategory'] . "'
						AND prequalifiedsuppliers.FY = '".  Date($_SESSION['DefaultDateFormat'],YearEndDate($_SESSION['YearEnd'],0)) . "'
						GROUP BY suppliers.supplierid
						ORDER BY prequalifiedsuppliers.name";
		}
		...
		if ($_POST['StockCat'] == 'All') {
			$SQL = "SELECT suppliers.supplierid,
						...
						FROM suppliers
						INNER JOIN prequalifiedsuppliers ON suppliers.supplierid=prequalifiedsuppliers.supprid
						WHERE prequalifiedsuppliers.supprid " . LIKE . " '%" . $_POST['SuppCode'] . "%'
						AND prequalifiedsuppliers.FY = '".  Date($_SESSION['DefaultDateFormat'],YearEndDate($_SESSION['YearEnd'],0)) . "'
						GROUP BY suppliers.supplierid
						ORDER BY prequalifiedsuppliers.name";
		} else {
		...
		if ($_POST['StockCat'] == 'All') {
			...
			$SQL = "SELECT suppliers.supplierid,
						...
						FROM suppliers
						INNER JOIN prequalifiedsuppliers ON suppliers.supplierid=prequalifiedsuppliers.supprid
						WHERE prequalifiedsuppliers.CatID='" . $_POST['SelectedCategory'] . "'
						AND prequalifiedsuppliers.FY = '".  Date($_SESSION['DefaultDateFormat'],YearEndDate($_SESSION['YearEnd'],0)) . "'
						GROUP BY suppliers.supplierid
						ORDER BY prequalifiedsuppliers.name";
		}
...
if (isset($_POST['Select'])) {
	...
	if (($AuthRow = DB_fetch_array($AuthResult) and $AuthRow['cancreate'] == 0)) {
		...
		prnMsg(_('You do not have the authority to raise Local Service Orders for') . ' ' . $myrow['suppname'] . '. ' . _('Please Consult your system administrator for more information.') . '<br />' . _('You can setup authorisations') . ' ' . '<a href="PO_AuthorisationLevels.php">' . _('here') . '</a>', 'warn');
		include('includes/footer.inc');
		exit;
	}
...
if ($_SESSION['RequireSupplierSelection'] == 1 OR !isset($_SESSION['PO' . $identifier]->SupplierID) OR $_SESSION['PO' . $identifier]->SupplierID == '') {
	echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/supplier.png" title="' . _('Service Order') . '" alt="" />' . ' ' . _('Local Service Order: Select Contractor') . '</p>';
	...
	<tr>
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////\
	$result11 = DB_query("SELECT typeid, typename FROM suppliertype");
	echo '<table class="selection"><tr>';
echo '<td>' . _('Select Category') . ':';
...
while ($myrow1 = DB_fetch_array($result11)) {
	if ($myrow1['typeid'] == $_POST['SelectedCategory']) {
		echo '<option selected="selected" value="' . $myrow1['typeid'] . '">' . $myrow1['typename'] . '</option>';
	} else {
		echo '<option value="' . $myrow1['typeid'] . '">' . $myrow1['typename'] . '</option>';
	}
}
echo '</select></td>';
...
echo '</td></tr></table><br />';
	...
	if ($_SESSION['ExistingOrder']) {
		echo _(' Modify Service Order Number') . ' ' . $_SESSION['PO' . $identifier]->OrderNo;
	}
	...
	if (isset($Purch_Item)) {
		...
		/* the link */
		echo '<a href="' . $RootPath . '/LSO_FarmItems.php?NewItem=' . $Purch_Item . '&identifier=' . $identifier . '">' . _('Enter Line Item to this purchase order') . '</a>';
		...
		$sql = "SELECT farmdescriptions.controlled,
						farmdescriptions.serialised,
						farmdescriptions.description,
						farmdescriptions.units ,
						farmdescriptions.decimalplaces,
						purchdata.price,
				...
				FROM farmdescriptions INNER JOIN stockcategory
					ON farmdescriptions.categoryid=stockcategory.categoryid
				LEFT JOIN purchdata
					ON farmdescriptions.stockid = purchdata.stockid
				WHERE farmdescriptions.stockid='" . $Purch_Item . "'
	...
	if(isset($_SESSION['PO'.$identifier]->OrderNo)){
	 $_POST['LSONo'] = $_SESSION['PO'.$identifier]->OrderNo;
	}

	echo '<br />
		<table width="80%">
		<tr>
			<th><h3>' . _('LSO Initiation Details') . '</h3></th>
			<th><h3>' . _('LSO Status') . '</h3></th>
		</tr>
	...
	echo '<tr>
	<td>' . _('LSO No.') . ':</td>
	<td><input type="text" required="required" autofocus="autofocus" name="LSONo" class="text" size="16" value="'.$_POST['LSONo'].'" /></td>
	</tr>';
	}
	echo '<tr>
			<td>' . _('LSO Date') . ':</td>
	...
	echo '<tr>
		...
		<tr>
			...
			<td><input type="text" name="Requisition" size="16" maxlength="15" title="' . _('Enter our Service requisition reference if needed') . '" value="' . $_POST['Requisition'] . '" /></td>
		</tr>
		<tr>
	...
	if (isset($_POST['AllowRePrint'])) {
		$sql = "UPDATE lsorders SET allowprint=1 WHERE orderno='" . $_SESSION['PO' . $identifier]->OrderNo . "'";
		$result = DB_query($sql);
	}
	...
	if ($_SESSION['PO' . $identifier]->AllowPrintPO == 0 AND empty($_POST['RePrint'])) {
		...
		echo '<tr>
				<td colspan="2"><a target="_blank"  href="' . $RootPath . '/LSO_PDFPurchOrder.php?OrderNo=' . $_SESSION['ExistingOrder'] . '&amp;identifier=' . $identifier . '">' . _('Reprint Now') . '</a></td></tr>';
	} //$Printed
	...
	if ($_SESSION['ExistingOrder'] != 0 AND $_SESSION['PO' . $identifier]->Status == 'Printed') {
		echo '<tr>
				<td><a href="' . $RootPath . '/LSOServiceReceived.php?PONumber=' . $_SESSION['PO' . $identifier]->OrderNo . '&amp;identifier=' . $identifier . '">' . _('Receive this order') . '</a></td>
			</tr>';
	}

	if ($_SESSION['PO' . $identifier]->Status == '') { //then its a new order
		echo '<tr>
				<td><input type="hidden" name="Status" value="NewOrder" />' . _('New Local Service Order') . '</td>
			</tr>';
	} else {
		echo '<tr>
				<td>' . _('Status') . ' :  </td>
	...
	echo '</table></td>
		...
		<tr>
			<th><h3>' . _('Department info') . '</h3></th>
		<!--    <th><h3>' . _('Delivery To') . '</h3></th> -->
			<th><h3>' . _('Contractor Info') . '</h3></th>
		</tr>
		<tr><td valign="top">';
	/*nested table level1 */
	...
	echo '<div class="centre">
			<input type="submit" name="EnterLines" value="' . _('Enter Farm Service Items') . '" />
			<input type="submit" name="EnterLinesf" value="' . _('Enter Service Items') . '" />
