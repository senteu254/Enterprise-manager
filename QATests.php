...
if (isset($_POST['submit'])) {
	...
	if (mb_strlen($_POST['QATestName']) > 130) {
		$InputError = 1;
	...
	$sql= "SELECT COUNT(*), groupby FROM qatests WHERE qatests.name='".$_POST['QATestName']."'
										AND qatests.testid <> '" .$SelectedQATest. "'";
	$result = DB_query($sql);
	$myrow = DB_fetch_row($result);
	if ($myrow[0]>0 && $myrow[1]==$_POST['GroupBy']) {
		$InputError = 1;
...
if (! isset($_GET['delete'])) {
	...
	echo '<tr>
			...
			<td>
			<textarea '. (in_array('QATestName',$Errors) ? 'class="inputerror"' : '' ) .' name="QATestName"  required="required" title="' . _('The name of the Test you are setting up') . '" maxlength="150" cols="40" rows="2">' . $_POST['QATestName'] . '</textarea></td>
		</tr>';
	echo '<tr>
			<td>' . _('Part') . ':</td>
			<td><input type="text" name="Part" title="' . _('Name represented by this test') . '" size="50" maxlength="50" value="' . $_POST['Part'] . '" /></td>
		</tr>';
	echo '<tr>
			<td>' . _('Method') . ':</td>
	...
	echo '<tr>
			<td>' . _('Group By') . ':</td>
			<td>
			<select title="' . _('Can be used to group certain Tests on the Product Specification or Certificate of
Analysis or left blank') . '" name="GroupBy">';
	if ($_POST['GroupBy']=='Critical Defects'){
		echo '<option selected="selected" value="Critical Defects">' . _('Critical Defects') . '</option>';
	} else {
		echo '<option Type="Critical Defects">' . _('Critical Defects') . '</option>';
	}
	if ($_POST['GroupBy']=='Major Defects'){
		echo '<option selected="selected" value="Major Defects">' . _('Major Defects') . '</option>';
	} else {
		echo '<option value="Major Defects">' . _('Major Defects') . '</option>';
	}
	if ($_POST['GroupBy']=='minor Defects'){
		echo '<option selected="selected" value="minor Defects">' . _('minor Defects') . '</option>';
	} else {
		echo '<option Type="minor Defects">' . _('minor Defects') . '</option>';
	}
	if ($_POST['GroupBy']=='Special Check'){
		echo '<option selected="selected" value="Special Check">' . _('Special Check') . '</option>';
	} else {
		echo '<option value="Special Check">' . _('Special Check') . '</option>';
	}
	if ($_POST['GroupBy']=='Hardness Test'){
		echo '<option selected="selected" value="Hardness Test">' . _('Hardness Test') . '</option>';
	} else {
		echo '<option value="Hardness Test">' . _('Hardness Test') . '</option>';
	}
	if ($_POST['GroupBy']=='Base Hardness'){
		echo '<option selected="selected" value="Base Hardness">' . _('Base Hardness') . '</option>';
	} else {
		echo '<option value="Base Hardness">' . _('Base Hardness') . '</option>';
	}
	if ($_POST['GroupBy']=='Primer Sensitivity'){
		echo '<option selected="selected" value="Primer Sensitivity">' . _('Primer Sensitivity') . '</option>';
	} else {
		echo '<option value="Primer Sensitivity">' . _('Primer Sensitivity') . '</option>';
	}
	if ($_POST['GroupBy']=='EPVAT Test'){
		echo '<option selected="selected" value="EPVAT Test">' . _('EPVAT Test') . '</option>';
	} else {
		echo '<option value="EPVAT Test">' . _('EPVAT Test') . '</option>';
	}
	if ($_POST['GroupBy']=='Wall Thickness Test'){
		echo '<option selected="selected" value="Wall Thickness Test">' . _('Wall Thickness Test') . '</option>';
	} else {
		echo '<option value="Wall Thickness Test">' . _('Wall Thickness Test') . '</option>';
	}
	echo '</select></td>
		</tr>';
	echo '<tr>
