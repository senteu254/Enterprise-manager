   ...
   if(isset($_POST['Yearend'])){
	...
	echo'There is Allocation in this Fincial year.';
		 ...
		 $sql="SELECT 
			...
			$totals_paid=$comm+$payables;
			 $Ava_Balance33=($totalAlloc-$totals_paid);
			    ...
			    if($dept != $myrow2['Vbook']){
				$pdf->SetFillColor(206, 219, 226);
				$pdf->SetFont( '', 'B', 12 );
				$pdf->SetTextColor(255, 0, 0);
				$LeftOvers = $pdf->addTextWrap($Left_Margin+1,$YPos-5,770,$FontSize=12,'','center',-1,-1);
				$LeftOvers = $pdf->addTextWrap($Left_Margin+390,$YPos-5,770,$FontSize=14,$myrow2['groupid']);
				$pdf->SetFillColor(206, 219, 226);
				...
				$dept = $myrow2['Vbook'];
				 ...
				 $pdf->SetTextColor(0,0,0);
				 	$FontSize = 8;
				 ###############################################################################################################################
				 $LeftOvers = $pdf->addTextWrap($Left_Margin+0,$YPos,95,$FontSize,$myrow2['votecode']);
				 $LeftOvers = $pdf->addTextWrap($Left_Margin+30,$YPos,250,$FontSize,$myrow2['Votehead']);
				 ...
				 $LeftOvers = $pdf->addTextWrap($Left_Margin+520,$YPos,95,$FontSize,locale_number_format($payables,2),'right');
				 $LeftOvers = $pdf->addTextWrap($Left_Margin+595,$YPos,95,$FontSize,locale_number_format($totals_paid,2),'right');
				 if ($Ava_Balance33<0){
				 $pdf->SetTextColor(255, 0, 0);
				 $LeftOvers = $pdf->addTextWrap($Left_Margin+678,$YPos,95,$FontSize,locale_number_format($Ava_Balance33,2),'right');
				 }else{
				 $pdf->SetTextColor(0,0,0);
				  $LeftOvers = $pdf->addTextWrap($Left_Margin+678,$YPos,95,$FontSize,locale_number_format($Ava_Balance33,2),'right');
				 }
			...
			$pdf->line($Left_Margin+31, $TopOfColHeadings+12,$Left_Margin+31,$Bottom_Margin+(0.0*$line_height));
		...
		//	$FontSize=10;
			...
			$YPos+=10;			    
				$pdf->addText($Page_Width-$Right_Margin-730, $YPos - ($line_height*2)+20,$FontSize, 'TOTAL : ','right'); //total field
				...
				$LeftOvers = $pdf->addTextWrap($Left_Margin+40,$YPos,400,$FontSize,' ' . _($status));
                $LeftOvers = $pdf->addTextWrap($Left_Margin+100,$YPos,400,$FontSize,' ' . _($original));
