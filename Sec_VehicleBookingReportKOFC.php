...
// Javier:
	...
	$line_height = 12;
      ...
      need QOH, QOO, QDem, Sales Mth -1, Sales Mth -2, Sales Mth -3, Sales Mth -4*/
		$SQL = "SELECT *
					...
					INNER JOIN vehicle_timeout b ON a.VehicleNo=b.VehicleNo
					WHERE DATE_FORMAT(b.time_out,'%Y-%m-%d') >='".FormatDateForSQL($_POST['datefrom'])."' AND DATE_FORMAT(b.time_out,'%Y-%m-%d') <='".FormatDateForSQL($_POST['dateto'])."'
					ORDER BY b.time_out DESC";
	...
	if (DB_error_no() !=0) {
	  ...
	  include('includes/header.inc');
	   ...
	   exit;
		...
		$ListCount = 0;
	include ('includes/PDFVehicleReportPageHeaderKOFC.inc');
	...
	$kmin = number_format($rows['KmIn']);
	$covered = number_format(($rows['KmIn'] - $rows['KmOut']));
	}else{
	$timein = '';
	$kmin ='';
	$covered ='';
		...
		$LeftOvers = $pdf->addTextWrap(50, $YPos, 70,$FontSize,strtoupper($rows['RegNo']),'left');
		$LeftOvers = $pdf->addTextWrap(120, $YPos, 80,$FontSize,ucwords(strtolower($rows['Make'])),'left');
		$LeftOvers = $pdf->addTextWrap(200, $YPos, 140,$FontSize,ucwords(strtolower($rows['DriverName'])),'left');
		//$LeftOvers = $pdf->addTextWrap(170, $YPos, 40,$FontSize,$rows['IdNo'],'left');
		$LeftOvers = $pdf->addTextWrap(330, $YPos, 100,$FontSize,$rows['Destination'],'left');
		$LeftOvers = $pdf->addTextWrap(430, $YPos, 80,$FontSize,number_format($rows['KmOut']),'left');
		$LeftOvers = $pdf->addTextWrap(480, $YPos, 100,$FontSize,$timeout,'left');
		$LeftOvers = $pdf->addTextWrap(590, $YPos, 100,$FontSize,$kmin,'left');
		$LeftOvers = $pdf->addTextWrap(650, $YPos, 100,$FontSize,$timein,'left');
		$LeftOvers = $pdf->addTextWrap(760, $YPos, 100,$FontSize,$covered,'left');
		$YPos -= ($line_height);
		$LeftOvers = $pdf->addTextWrap(50, $YPos, 260,$FontSize,'Details : '.$rows['Details'],'left');
		//$LeftOvers = $pdf->addTextWrap(440, $YPos, 280,$FontSize,'Destination : '.$rows['Destination'],'left');
		$LeftOvers = $pdf->addTextWrap(480, $YPos, 120,$FontSize,$rows['sec_officer'],'left');
		$LeftOvers = $pdf->addTextWrap(650, $YPos, 120,$FontSize,$rows['sec_officer_in'],'left');
		$YPos -= ($line_height);
	...
	if ($ListCount == 0){
		...
		?>
								<script>
								window.location.href = "index.php?Application=SEC2&Link=Reports&Rep=KOFCVehicles";
								</script>
							<?php
		exit;
	} else {
