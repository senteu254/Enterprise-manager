<?php
/* $Id: PO_SelectPurchOrder.php 6941 2014-10-26 23:18:08Z daintree $*/

include ('includes/session.inc');
$Title = _('Search Service Orders');
include ('includes/header.inc');

echo '<p class="page_title_text">
		<img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Local Service Orders') . '" alt=""  />' . ' ' . _('Local service  Orders') .
	'</p>';
...
if (isset($_POST['SearchParts'])) {
	...
	if ($_POST['Keywords']) {
		...
		$SQL = "SELECT stockmaster.stockid,
				...
				SUM(farmlocstock.quantity) as qoh,
				stockmaster.units,
				SUM(lsorderdetails.quantityord-lsorderdetails.quantityrecd) AS qord
			FROM stockmaster INNER JOIN locstock
			ON stockmaster.stockid = locstock.stockid INNER JOIN lsorderdetails
			ON stockmaster.stockid=lsorderdetails.itemcode
			WHERE lsorderdetails.completed=1
		...
		$SQL = "SELECT stockmaster.stockid,
			...
			FROM stockmaster INNER JOIN locstock
				...
				INNER JOIN lsorderdetails ON stockmaster.stockid=lsorderdetails.itemcode
			WHERE lsorderdetails.completed=1
		...
		$SQL = "SELECT stockmaster.stockid,
			...
			FROM stockmaster INNER JOIN locstock ON stockmaster.stockid = locstock.stockid
				INNER JOIN lsorderdetails ON stockmaster.stockid=orderdetails.itemcode
			WHERE lsorderdetails.completed=1
			AND stockmaster.categoryid='" . $_POST['StockCat'] . "'
			GROUP BY stockmaster.stockid,
				stockmaster.description,
...
if (!isset($OrderNumber) or $OrderNumber == "") {
	...
	if ($_POST['Status']=='Rejected'){
 	...
 	echo '</select> <input type="submit" name="SearchOrders" value="' . _('Search Service  Orders') . '" /></td>
		</tr>
		</table>';
}
...
echo '<br />
		...
		<tr>
			<td>';
echo _('To search for Local Service  orders for a specific part use the part selection facilities below') . '</td></tr>';
echo '<tr>
...
echo '</select></td>
	...
	<tr>
		<td></td>
		<td><b>' . _('OR') . ' </b>' . _('Enter extract of the') . '<b> ' . _('Service Code') . '</b>:</td>
...
//end if stock search results to show
	...
	if (!isset($_POST['Status']) OR $_POST['Status']=='Pending_Authorised_Completed'){
		$StatusCriteria = " AND (lsorders.status='Pending' OR lsorders.status='Authorised' OR lsorders.status='Printed' OR lsorders.status='Completed') ";
		...
		$StatusCriteria = " AND lsorders.status='Completed' ";
	}
	if (isset($OrderNumber) AND $OrderNumber != '') {
		$SQL = "SELECT lsorders.orderno,
					...
					WHERE lsorders.orderno='" . filter_number_format($OrderNumber) . "'
					GROUP BY lsorders.orderno,
						suppliers.suppname,
						lsorders.orddate,
						lsorders.initiator,
						lsorders.requisitionno,
		...
		if (isset($SelectedSupplier)) {
			if (isset($SelectedStockItem)) {
				$SQL = "SELECT lsorders.orderno,
							...
							INNER JOIN currencies
							ON suppliers.currcode=currencies.currabrev
							WHERE  lsorderdetails.itemcode='" . $SelectedStockItem . "'
				...
				$SQL = "SELECT lsorders.orderno,
							...
							INNER JOIN currencies
							ON suppliers.currcode=currencies.currabrev
							WHERE lsorders.supplierno='" . $SelectedSupplier . "'
							AND lsorders.intostocklocation = '" . $_POST['StockLocation'] . "'
							" . $StatusCriteria . "
							GROUP BY lsorders.orderno,
								suppliers.suppname,
			...
			if (isset($SelectedStockItem)) {
				...
				$SQL = "SELECT lsorders.orderno,
							...
							INNER JOIN currencies
							ON suppliers.currcode=currencies.currabrev
							WHERE lsorders.intostocklocation = '" . $_POST['StockLocation'] . "'
							" . $StatusCriteria . "
							GROUP BY lsorders.orderno,
								suppliers.suppname,
								lsorders.orddate,
	...
	if (DB_num_rows($PurchOrdersResult) > 0) {
		...
		while ($myrow = DB_fetch_array($PurchOrdersResult)) {
			...
			$ViewPurchOrder = $RootPath . '/LSO_OrderDetails.php?OrderNo=' . $myrow['orderno'];
