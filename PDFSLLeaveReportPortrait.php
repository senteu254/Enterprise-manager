...
$pdf->addInfo('Subject', _('Sick Leave Report') );
$FontSize = 12;
...
if (DB_num_rows($result)>0){
	...
	include('PDFSLLeaveReportHeader.php');
	...
	while ($myrow2=DB_fetch_array($result)){
		...
		$LeftOvers = $pdf->addText(360, $YPos+$FontSize, $FontSize, $myrow2['edate']);
		$LeftOvers = $pdf->addText(420, $YPos+$FontSize, $FontSize, $myrow2['endate']);
		$LeftOvers = $pdf->addText(480, $YPos+$FontSize, $FontSize, $myrow2['no_days']);
...
if ($ListCount == 0){
    ...
    $pdf->OutputI($_SESSION['DatabaseName'] . '_Sick Leave Report_' . '_' . date('Y-m-d') . '.pdf');
    $pdf->__destruct();
}
ob_end_flush();
?>
