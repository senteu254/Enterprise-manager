...
//the link to delete a selected record was clicked instead of the submit button		
			...
			$sql="DELETE FROM qapropellantacceptance WHERE id='" . $SelectedUser . "'";
			$ErrMsg = _('The Record could not be deleted because');
			$result = DB_query($sql,$ErrMsg);
			$sql2="DELETE FROM qapropellantacceptancedata WHERE testno='" . $SelectedUser . "'";
...
$search="";
						...
						$statement = " qapropellantacceptance a
	...
	?>
              ...
              <div class="mailbox-controls">
                ...
                <div class="col-xs-1">
                <!-- /.btn-group -->
                <a href="index.php?Application=QA&Link=PropAcc"><button type="button" title="Refresh" class="btn btn-default btn-sm"><i class="fa fa-refresh"></i></button></a>
				</div>
				<div class="col-xs-4">
				<form enctype="multipart/form-data" method="post">
				<?php echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />'; ?>
				<div class="input-group input-group-sm">
                <input type="text" placeholder="Search Country..." id="myInput" onkeyup="myFunction()" name="Searchfield" value="<?php echo isset($_POST['Searchfield']) ? $_POST['Searchfield'] : ""; ?>" class="form-control">
			  ...
			  <div class="col-xs-7">
				...
				<div class="btn-group">
				<?php
				echo pagination_inbox($sqlforPages,$per_page,$page,$url='?Application=QA&Link=PropAcc&');
				?>
                  </div>
                  <!-- /.btn-group -->
                </div>
              ...
              <div class="table-responsive mailbox-messages">
				...
				<th></th>
                  ...
                  echo '<tr>
                    ...
                    <td class="mailbox-name"><a href="index.php?Application=QA&Link=PropAccRead&ID='.$row['id'].'&amp;New=Yes">'.str_pad($row['id'],10,'0',STR_PAD_LEFT).'</a></td>
				...
				echo '<td width="60"><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Application=QA&amp;Link=NewPropAcc&amp;SelectedUser=' . $row['id'] .'" title="Edit"><i class="fa fa-edit"></i></a> || <a href="' .htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8')  .'?Application=QA&amp;Link=PropAcc&amp;delete=yes&amp;SelectedUser=' . $row['id'].'" onclick="return confirm(\'' . _('Are you sure you wish to delete this Record ?') . '\');" title="Delete" style="color:red;"><i class="fa fa-trash"></i></a></td>';
					}else{
					echo '<td></td>';
					}
                  echo '</tr>';
		...
		$search1="";
						...
						$sqlforPages = $statement;
						$sql = "SELECT *, a.id, (SELECT MAX(levelcheck) FROM qa_approval_levels WHERE type=7) AS LastLevel FROM ".$statement." GROUP BY a.id ORDER BY a.id DESC  LIMIT {$startpoint} , {$per_page}";
						$welcome_viewed = DB_query($sql,$ErrMsg,$DbgMsg);
						$num_rows = DB_num_rows($welcome_viewed);
			echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
	?>
	...
	<div class="panel panel-primary">
		<div class="panel-heading">Forwarded Propellant Acceptance Forms</div>
			<div class="panel-body">
              ...
              <div class="mailbox-controls">
				...
				<?php echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />'; ?>
				<div class="input-group input-group-sm">
                <input type="text" placeholder="Search Country..." id="myInput1" onkeyup="myFunction1()" name="Searchfield1" value="<?php echo isset($_POST['Searchfield1']) ? $_POST['Searchfield1'] : ""; ?>" class="form-control">
                    <span class="input-group-btn">
                      <button type="submit" class="btn btn-info btn-flat"><i class="fa fa-search"></i></button>
                    </span>
              </div> 
              ...
              <div class="table-responsive mailbox-messages">
				...
				<th>CreatedBy</th></tr>
                  ...
                  echo '<tr>
                    ...
                    <td class="mailbox-name"><a href="index.php?Application=QA&Link=PropAccRead&ID='.$row['id'].'&amp;New=No">'.str_pad($row['id'],10,'0',STR_PAD_LEFT).'</a></td>
                    <td class="mailbox-attachment">'.$row['country'].'</td>
					<td class="mailbox-date">'.$row['company'].'</td>
					<td class="mailbox-date">'.$LastVisitDate.'</td>
					<td class="mailbox-date">'.$row['technicianid'].'</td>';
				  ...
				  echo '<tr><td class="alert-danger" colspan="6"><center><b style="color:#FF0000">No Record Found</b></center></td></tr>';
				  }
