...
$search=" AND (a.id LIKE '%".$_POST['Searchfield']."%' OR lot LIKE '%".$_POST['Searchfield']."%' OR machine LIKE '%".$_POST['Searchfield']."%' OR calibre LIKE '%".$_POST['Searchfield']."%') ";
}else{
$search="";
}
						...
						$statement = " qanonconformingproducts a
	...
	?>
              ...
              <div class="mailbox-controls">
                ...
                <div class="col-xs-1">
                <!-- /.btn-group -->
                <a href="index.php?Application=QA&Link=Non-Conformance"><button type="button" title="Refresh" class="btn btn-default btn-sm"><i class="fa fa-refresh"></i></button></a>
				</div>
				<div class="col-xs-4">
				<form enctype="multipart/form-data" method="post">
				<?php echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />'; ?>
				<div class="input-group input-group-sm">
                <input type="text" placeholder="Search Machine/Calibre..." id="myInput" onkeyup="myFunction()" name="Searchfield" value="<?php echo isset($_POST['Searchfield']) ? $_POST['Searchfield'] : ""; ?>" class="form-control">
			  ...
			  <div class="col-xs-7">
				...
				<div class="btn-group">
				<?php
				echo pagination_inbox($sqlforPages,$per_page,$page,$url='?Application=QA&Link=Non-Conformance&');
				?>
                  </div>
                  <!-- /.btn-group -->
                </div>
              ...
              <div class="table-responsive mailbox-messages">
				...
				<th></th>
                  ...
                  echo '<tr>
                    ...
                    <td class="mailbox-name"><a href="index.php?Application=QA&Link=Non-ConformanceRead&ID='.$row['id'].'&amp;New=Yes">'.str_pad($row['id'],10,'0',STR_PAD_LEFT).'</a></td>
                    <td class="mailbox-subject">'.$row['machine'].'</td>
                    <td class="mailbox-attachment">'.$LastVisitDate.'</td>
                    <td class="mailbox-date">'.$row['calibre'].'</td>
					<td class="mailbox-date">'.$row['lot'].'</td>
					<td class="mailbox-date">'.$row['technicianid'].'</td>';
					if($row['process_level']==0){
				echo '<td width="70"><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Application=QA&amp;Link=NewNonConformance&amp;SelectedUser=' . $row['id'] .'" title="Edit"><i class="fa fa-edit"></i></a> || <a href="' .htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8')  .'?Application=QA&amp;Link=Non-Conformance&amp;delete=yes&amp;SelectedUser=' . $row['id'].'" onclick="return confirm(\'' . _('Are you sure you wish to delete this Record ?') . '\');" title="Delete" style="color:red;"><i class="fa fa-trash"></i></a></td>';
					}else{
					echo '<td></td>';
					}
                  echo '</tr>';
		...
		$search1=" AND (a.id LIKE '%".$_POST['Searchfield1']."%' OR lot LIKE '%".$_POST['Searchfield1']."%' OR machine LIKE '%".$_POST['Searchfield1']."%' OR calibre LIKE '%".$_POST['Searchfield1']."%') ";
		}else{
		$search1="";
		}
						$ErrMsg = _('An error occurred in retrieving the records');
						...
						$sqlforPages = $statement;
						$sql = "SELECT *,a.id, (SELECT MAX(levelcheck) FROM qa_approval_levels WHERE type=2) AS LastLevel FROM ".$statement." GROUP BY a.id ORDER BY a.id DESC LIMIT {$startpoint} , {$per_page}";
						$welcome_viewed = DB_query($sql,$ErrMsg,$DbgMsg);
						$num_rows = DB_num_rows($welcome_viewed);
			echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
	?>
		...
		<div class="panel-heading">Forwarded Non-Conforming Products</div>
			<div class="panel-body">
              ...
              <div class="mailbox-controls">
				...
				<?php echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />'; ?>
				<div class="input-group input-group-sm">
                <input type="text" placeholder="Search Machine/Calibre..." id="myInput1" onkeyup="myFunction1()" name="Searchfield1" value="<?php echo isset($_POST['Searchfield1']) ? $_POST['Searchfield1'] : ""; ?>" class="form-control">
                    <span class="input-group-btn">
                      <button type="submit" class="btn btn-info btn-flat"><i class="fa fa-search"></i></button>
                    </span>
              </div> 
              ...
              <div class="table-responsive mailbox-messages">
				...
				<th>CreatedBy</th></tr>
                  ...
                  echo '<tr>
                    ...
                    <td class="mailbox-name"><a href="index.php?Application=QA&Link=Non-ConformanceRead&ID='.$row['id'].'&amp;New=No">'.str_pad($row['id'],10,'0',STR_PAD_LEFT).'</a></td>
                    <td class="mailbox-subject">'.$row['machine'].'</td>
                    <td class="mailbox-attachment">'.$LastVisitDate.'</td>
                    <td class="mailbox-date">'.$row['calibre'].'</td>
					<td class="mailbox-date">'.$row['lot'].'</td>
