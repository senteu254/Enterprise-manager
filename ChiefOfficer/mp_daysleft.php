  ...
  <head>
	...
	<body>
		...
		//INITIALIZE DATE RANGE Start
			...
			$sql1="select *,SUM(credits) AS nod From mp_credits
					where  date >= '$cyrr-$cyr' 
						AND date <= '$csx-$cyy' AND mp_credits.emp_id = '$_GET[id]'";
			$qry1 = DB_query($sql1);
			...
			$sql="select *,SUM(nodays) AS nod1,employee.emp_id as id, emp_fname, emp_lname, leavetype, nodays, sdate, eddate  From mp_log, employee,managing_director
					where managing_director.md = vacation_log.managing_director
						  AND managing_director.md = '2'
						  AND mp_log.emp_id = employee.emp_id 
							 AND mp_log.emp_id = '$_GET[id]'";
			...
			$sql="select *,SUM(nodays) AS nod1,employee.emp_id as id, emp_fname, emp_lname, leavetype, nodays, sdate, eddate  From mp_log, employee,general_manager
					where general_manager.gm = mp_log.general_manager
			...
			$sql="select *,SUM(nodays) AS nod1,employee.emp_id as id, emp_fname, emp_lname, leavetype, nodays, sdate, eddate  From mp_log, employee,program_head
					where program_head.prog_head = mp_log.prog_head
			...
			$sql1="select *,SUM(credits) AS nod From mp_credits
					where  date >= '$cyrr-$cyr' 
							AND date <= '$cyx-$cmr' AND mp_credits.emp_id = '$_GET[id]'";
			$qry1 = DB_query($sql1);
			$rec = DB_fetch_array($qry1);
			$cred = $rec['nod'];
			...
			$sql="select *,SUM(nodays) AS nod1,employee.emp_id as id, emp_fname, emp_lname, leavetype, nodays, sdate, eddate  From mp_log, employee,managing_director
					where managing_director.md = mp_log.managing_director
						  AND managing_director.md = '2'
						  AND mp_log.emp_id = employee.emp_id 
							 AND mp_log.emp_id = '$_GET[id]'";
			...
			<div class = "row">
					<div class="col-md-12">
						<div class="page-header">
								<h1 align="center">Maternity/Paternity Leave</h1>
						</div>
					</div>
				</div>
			<br/>
