		...
		<?php
			$sql="select * From employee,mp_log where employee.emp_id = mp_log.emp_id AND mp_id = '$_GET[id]'";
			$qry3 = DB_query($sql);
			$rec = DB_fetch_array($qry3);
			$grade = $rec ['grade'];
						//this codes is that all the approve will go to the approve submenu
						if($grade=='MANAGER'){
						$sql = "SELECT *,employee.emp_id as id FROM employee, mp_log, departments,section, general_manager,managing_director,approve_hr
								    ...
								    AND employee.stat='1' 
									AND mp_log.mp_id = '$_GET[id]'";
						$qry=DB_query($sql);
						}
						
					else if($grade=='I-CHIEF OFFICER'){
					
			$sql1 = "SELECT *,employee.emp_id as id FROM employee, mp_log, departments,section, general_manager,program_head,approve_hr
									WHERE employee.emp_id = mp_log.emp_id 
									AND employee.id_dept = departments.departmentid
									AND approve_hr.hr_approve= mp_log.hr_approve
									AND section.id_sec = employee.id_sec
						...
						$sql2 = "SELECT *,employee.emp_id as id FROM employee, mp_log, departments,section, section_head, program_head, chief_officer,approve_hr
									WHERE employee.emp_id = mp_log.emp_id 
									AND employee.id_dept = departments.departmentid
									AND approve_hr.hr_approve= mp_log.hr_approve
									AND section.id_sec = employee.id_sec 
			...
			<div class="col-md-4 col-md-offset-1">
					<a href="<?php echo $mainlink; ?>MPTrack"><input type = "button" value="Back" class="btn btn-default"/></a>
			</div>
	</div>
