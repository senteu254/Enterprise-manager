...
If (!isset($_GET['id']) || $_GET['id']==""){
        $Title = _('Select Paternity/Maternity Leave To Print');
        include('includes/header.inc');
        echo '<div class="centre">
				<br />
				<br />
				<br />';
        prnMsg( _('Select a Paternity/Maternity Leave to Print before calling this page') , 'error');
        echo '<br />
				<br />
				<br />
				...
				<tr>
					<td class="menu_group_item">
						<ul><li><a href="index.php?Application=HR&Ref=ViewMAll' . _(' Paternity/Maternity All Approve') . '</a></li>
						</ul>
					</td>
				</tr>
				</table>
...
$ErrMsg = _('There was a problem retrieving the Paternity/Maternity Leave header details for this Vacation Number') . ' ' . $_GET['id'] . ' ' . _('from the database');
$sql = "SELECT *,b.emp_id as id FROM mp_log a
...
if (DB_num_rows($result)==0){
	$Title = _('Print Paternity/Maternity  Leave Request Error');
	include('includes/header.inc');
	 echo '<div class="centre">
			<br />
			<br />
			<br />';
	prnMsg( _('Unable to Locate Paternity/Maternity Leave Number') . ' : ' . $_GET['id'] . ' ', 'error');
	echo '<br />
			<br />
			<br />
			<table class="table_index">
			<tr>
				<td class="menu_group_item">
					<ul><li><a href="index.php?Application=HR&Ref=ViewMAll">' . _(' Paternity/Maternity  All Approve') . '</a></li></ul>
				</td>
			</tr>
			</table>
			</div>
...
$pdf->addInfo('Title', _('Paternity/Maternity Leave') );
$pdf->addInfo('Subject', _('Paternity/Maternity Leave') . ' ' . $_GET['id']);
$FontSize = 12;
$line_height = 12;// Recommended: $line_height = $x * $FontSize.

	/*Yes there are line items to start the ball rolling with a page header */
	include ('PDFMPHeader.php');
		...
		$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos-120,100,$FontSize, _('Start Date:'));
		$LeftOvers = $pdf->addTextWrap(300,$YPos-120,100,$FontSize, _('End Date.'));
		$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos-140,100,$FontSize, _('No of Days'));
		$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos-160,300,$FontSize, _('Contact Address while on leave'));
		...
		$LeftOvers = $pdf->addTextWrap(180,$YPos-120,100,$FontSize, $myrow['sdate']);
		$LeftOvers = $pdf->addTextWrap(350,$YPos-120,100,$FontSize, $myrow['eddate']);
		$LeftOvers = $pdf->addTextWrap(180,$YPos-140,300,$FontSize, $myrow['nodays']);
		$LeftOvers = $pdf->addTextWrap(180,$YPos-160,300,$FontSize, $myrow['emp_add']);
		$YPos = $YPos-130;
