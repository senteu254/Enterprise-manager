...
$Title = _('Enter Supplier Invoice Against Service Received');
	...
	for ($i=0;isset($_POST['GRNNo'.$i]);$i++) { //loop through all the possible form variables where a GRNNo is in the POST variable name
		...
		if (!is_numeric(filter_number_format($_POST['ChgPrice' . $i])) AND filter_number_format($_POST['ChgPrice' . $i])<0){
			...
			if (filter_number_format($_POST['ChgPrice' . $i])/$_SESSION['SuppTrans']->GRNs[$_POST['GRNNo'.$i]]->OrderPrice > (1+ ($_SESSION['OverChargeProportion'] / 100))){
				prnMsg(_('The price being invoiced is more than the Service order price by more than') . ' ' . $_SESSION['OverChargeProportion'] . '%. ' .
...
echo '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') .'" method="post">
		...
		<tbody>
		<tr>
			<th colspan="6"><h3>' . _('Invoiced Service Received Selected') . '</h3></th>
		</tr>
		<tr>
			<th>' . _('Sequence') . ' #</th>
			<th>' . _('Service NO.') . ' #</th>
			...
			<th>' . _('Service Price') . ' ' . $_SESSION['SuppTrans']->CurrCode . '</th>
			<th>' . _('Inv Price') . ' ' . $_SESSION['SuppTrans']->CurrCode . '</th>
			<th>' . _('Service Value') . ' ' . $_SESSION['SuppTrans']->CurrCode . '</th>
		</tr>';
...
echo '</tbody>
	...
	<br />
	<div class="centre">
		<a href="' . $RootPath . '/LSO_SupplierInvoice.php">' . _('Back to Invoice Entry') . '</a>
	</div>
	<br />';
...
$SQL = "SELECT grnbatch,
		...
		AND lsogrns.qtyrecd - lsogrns.quantityinv > 0
		ORDER BY lsogrns.grnno";
$GRNResults = DB_query($SQL);

if (DB_num_rows($GRNResults)==0){
	prnMsg(_('There are no outstanding Service received from') . ' ' . $_SESSION['SuppTrans']->SupplierName . ' ' . _('that have not been invoiced by them') . '<br />' . _('The Service must first be received using the link below to select Service orders to receive'),'warn');
	echo '<div class="centre"><p><a href="' . $RootPath . '/LSO_SelectOSPurchOrder.php?SupplierID=' . $_SESSION['SuppTrans']->SupplierID .'">' . _('Select Service Orders to Receive')  . '</a></p></div>';
	include('includes/footer.inc');
	exit;
}
...
if (!isset($_GET['Modify'])){
	if (count( $_SESSION['SuppTransTmp']->GRNs)>0){   /*if there are any outstanding GRNs then */
		echo '<table class="selection">
				<tr>
					<th><h3>' . _('Service Received Yet to be Invoiced From') . ' ' . $_SESSION['SuppTrans']->SupplierName . '</h3></th>
				</tr>
				</table>
				<table>
					<tbody>
					<tr>
						...
						<th class="ascending">' . _('SDN Number') . '</th>
						<th class="ascending">' . _('Service No.') . '</th>
