...
if (isset($_POST['submit'])) {
	...
	if (isset($SelectedLocation) AND $InputError !=1) {
		...
		$sql = "UPDATE locations SET loccode='" . $_POST['LocCode'] . "',
									...
									purchasing_officer='" . $_POST['purchase'] . "',
...
or deletion of the records*/
	...
	echo '<tr>
			...
			<th class="ascending">' . _('Authoriser') . '</th>
			<th class="ascending">' . _('Purchasing Officer') . '</th>
...
if (!isset($_GET['delete'])) {
	...
	if (isset($SelectedLocation)) {
		...
		$_POST['purchase'] = $myrow['purchasing_officer'];
	...
	while ($myrow=DB_fetch_array($userresult)) {
		if ($myrow['userid']==$_POST['Authoriser']) {
	...
	echo '</select></td>
		...
		<tr>
			<td>' . _('Purchasing Officer') . '</td>
			<td><select name="purchase">';
	$usersql="SELECT userid FROM www_users";
	$userresult=DB_query($usersql);
	while ($myrow=DB_fetch_array($userresult)) {
		if ($myrow['userid']==$_POST['purchase']) {
			echo '<option selected="True" value="'.$myrow['userid'].'">' . $myrow['userid'] . '</option>';
		} else {
			echo '<option value="'.$myrow['userid'].'">' . $myrow['userid'] . '</option>';
		}
