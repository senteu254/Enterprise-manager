...
if (isset($_POST['Submit'])) {
	...
	if (ContainsIllegalCharacters($_POST['DepartmentName'])) {
		$InputError = 1;
		prnMsg( _('The description of the section must not contain the character') . " '&amp;' " . _('or the character') ." '",'error');
	}
	if (trim($_POST['DepartmentName']) == '') {
		$InputError = 1;
		prnMsg( _('The Name of the Section should not be empty'), 'error');
	}
	...
	if (isset($_POST['SelectedSectionID'])
		...
		if ( $myrow[0] > 0 ) {
			$InputError = 1;
			prnMsg( _('This Section name already exists.'),'error');
		} else {
			...
			if ( DB_num_rows($result) != 0 ) {
				...
				$OldDepartmentName = $myrow['section_name'];
				$sql = array();
				$sql[] = "UPDATE section
							SET section_name='" . $_POST['DepartmentName'] . "',
								id_dept='" . $_POST['Department'] . "',
								emp_id='" . $_POST['Authoriser'] . "'
							WHERE section_name " . LIKE . " '" . $OldDepartmentName . "'";
			} else {
				$InputError = 1;
				prnMsg( _('The section does not exist.'),'error');
			}
		}
		$msg = _('The Section has been modified');
	} elseif ($InputError !=1) {
		/*SelectedDepartmentID is null cos no item selected on first time round so must be adding a record*/
		$sql = "SELECT count(*) FROM section
				WHERE section_name " . LIKE . " '" . $_POST['DepartmentName'] . "'";
		$result = DB_query($sql);
		$myrow = DB_fetch_row($result);
		if ( $myrow[0] > 0 ) {
			$InputError = 1;
			prnMsg( _('There is already a section with the specified name.'),'error');
		} else {
			$sql = "INSERT INTO section (id_dept,section_name,
											 emp_id )
					VALUES ('" . $_POST['Department'] . "','" . $_POST['DepartmentName'] . "',
							'" . $_POST['Authoriser'] . "')";
		}
		$msg = _('The new Section has been created');
	}
	...
	if ($InputError!=1){
		...
		if (is_array($sql)) {
			$result = DB_Txn_Begin();
			$ErrMsg = _('The Section could not be inserted');
...
//the link to delete a selected record was clicked instead of the submit button
	...
	$result = DB_query($sql);
	if ( DB_num_rows($result) == 0 ) {
		prnMsg( _('You cannot delete this Section'),'warn');
	} else {
			$sql="DELETE FROM section WHERE id_sec='" . $SelectedDepartmentID . "'";
			$result = DB_query($sql);
			prnMsg( _('The Section has been removed') . '!','success');
		}
 ...
 if (!isset($SelectedDepartmentID)) {
	...
	$sql = "SELECT *
			FROM section,departments
			WHERE section.id_dept = departments.departmentid 
			ORDER BY id_sec";

	$ErrMsg = _('There are no sections created');
	...
	while ($myrow = DB_fetch_array($result)) {
		...
		echo '<td>' . $myrow['section_name'] . '</td>
				<td>' . $myrow['description'] . '</td>
				<td>' . $myrow['emp_id'] . '</td>
				<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?SelectedSectionID=' . $myrow['id_sec'] . '">' . _('Edit') . '</a></td>
				<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?SelectedSectionID=' . $myrow['id_sec'] . '&amp;delete=1" onclick="return confirm(\'' . _('Are you sure you wish to delete this Section?') . '\');">'  . _('Delete')  . '</a></td>
			</tr>';
...
if (isset($SelectedDepartmentID)) {
	echo '<div class="centre">
			<a href="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '">' . _('View all Sections') . '</a>
		</div>';
}
...
if (! isset($_GET['delete'])) {
	...
	if (isset($SelectedDepartmentID)) {
		...
		$sql = "SELECT *
				FROM section,departments
				WHERE section.id_dept=departments.departmentid 
				AND id_sec='" . $SelectedDepartmentID . "'";

		$result = DB_query($sql);
		if ( DB_num_rows($result) == 0 ) {
			prnMsg( _('The selected section could not be found.'),'warn');
			unset($SelectedDepartmentID);
		} else {
			$myrow = DB_fetch_array($result);

			$_POST['DepartmentID'] = $myrow['id_sec'];
			$_POST['DepartmentName']  = $myrow['section_name'];
