...
if (isset($_SESSION['SupplierID'])) {
	...
	<br />';
	echo '<a href="' . $RootPath . '/PDFSupplierTaxReference.php?SupplierID=' . $_SESSION['SupplierID'] . '">' . _('Supplier Withholding Transaction') . '</a>
	<br />';
		echo '<a href="' . $RootPath . '/SupplierAccount.php?SupplierID=' . $_SESSION['SupplierID'] . '">' . _('Supplier Account Statement') . '</a>
		<br />';
		echo '<a href="' . $RootPath . '/printSupplierstatement.php?SupplierID=' . $_SESSION['SupplierID'] . '">' . _('Print Supplier Statement') . '</a>
		<br />
		<br />';
    echo '<br /><a href="' . $RootPath . '/LSO_SelectOSPurchOrder.php?SelectedSupplier=' . $_SESSION['SupplierID'] . '">' . _('View Outstanding Local Service Orders') . '</a>';
	...
	echo '<a href="' . $RootPath . '/LSO_SupplierInvoice.php?SupplierID=' . $_SESSION['SupplierID'] . '">' . _('Enter a Suppliers Invoice for LSO') . '</a><br />';
	...
	echo '<a href="' . $RootPath . '/Suppliers.php">' . _('Add a New Supplier') . '</a>
		...
		<br /><a href="' . $RootPath . '/SupplierBalanceUpdate.php?SupplierID=' . $_SESSION['SupplierID'] . '">' . _('Add/Modify/Delete Supplier Balance') . '</a>
		<br />
...
if (isset($_POST['Search'])) {
	...
	echo '<tr>
			...
			<th class="ascending">' . _('Group Type') . '</th>
	...
	while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
		...
		$So= DB_query("SELECT groupname FROM   suppliergrouptype
								         WHERE groupid='".$myrow['suppliergroup'] ."'");
	...
	echo '<table>';
		echo'
			 <td></td>';
		while (($myrow4 = DB_fetch_array($total))) {
				
		  echo' <tr>	';
				  echo'<td>' . _('Total Suppliers') . '</td><td></td> <td>'.$myrow4['allsuppliers'] .'</td>';				 
		  echo'</tr>';
		}
		echo '</table>';
