		...
		$AlreadyOnThisOrder = 0;
			...
			if ($AlreadyOnThisOrder!=1 AND filter_number_format($Quantity) > 0){
				$sql = "SELECT description,
						...
						WHERE  farmdescriptions.stockid = '". $ItemCode . "'";
...
if (isset($_POST['Search']) OR isset($_POST['Prev']) OR isset($_POST['Next'])){  /*ie seach for stock items */
	...
	if ($_POST['Keywords']) {
		...
		if ($_POST['StockCat']=='All'){
			if ($_POST['SupplierItemsOnly']=='on'){
				 $sql = "SELECT farmdescriptions.stockid,
		...
		if ($_POST['StockCat']=='All'){
			if ($_POST['SupplierItemsOnly']=='on'){
				$sql = "SELECT farmdescriptions.stockid,
						...
						AND stockid " . LIKE . " '" . $_POST['StockCode'] . "'
						ORDER BY stockid ";
			} else {
				$sql = "SELECT farmdescriptions.stockid,
								farmdescriptions.description,
	...
	if (DB_num_rows($SearchResult)==0 AND $debug==1){
		prnMsg( _('There are no productions items to display for production'),'warn');
	}
