...
/* $Id: DeliveryDetails.php 6945 2014-10-27 07:20:48Z daintree $ */
	...
	*/
	...
	if ($InputErrors==0){
		...
		if (!isset($_SESSION['Items'.$identifier])) {
			...
			$_SESSION['Items'.$identifier]->Approver = $_POST['Approver'];
		} else {
			...
			$_SESSION['Items'.$identifier]->CustRef = $_POST['CustRef'];
			$_SESSION['Items'.$identifier]->Comments = $_POST['Comments'];
			$_SESSION['Items'.$identifier]->SalesPerson = $_POST['SalesPersons'];
			$_SESSION['Items'.$identifier]->FreightCost = round($_POST['FreightCost'],2);
			$_SESSION['Items'.$identifier]->Quotation = $_POST['Quotation'];
			$_SESSION['Items'.$identifier]->Approver = $_POST['Approver'];
		}
...
/* finally write the order header to the database and then the order line details */
	...
	} elseif ($_SESSION['Items'.$identifier]->Quotation==2){
		prnMsg(_('Proforma Invoice Number') . ' ' . $OrderNo . ' ' . _('has been entered'),'success');
	}else {
	...
	if (count($_SESSION['AllowedPageSecurityTokens'])>1){
		...
		if ($_POST['Quotation']==0) { /*then its not a quotation its a real order */
			...
			echo '</table>';
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		} elseif ($_POST['Quotation']==2) {
			/*link to print the quotation */
			echo '<br /><table class="selection">
					<tr>
						<td><img src="'.$RootPath.'/css/'.$Theme.'/images/printer.png" title="' . _('Order') . '" alt=""></td>
						<td>' . ' ' . '<a href="' . $RootPath . '/PDFPinvoice.php?identifier='.$identifier . '&amp;PINo=' . $OrderNo . '" target="_blank">' .  _('Print Proforma Invoice (Landscape)')  . '</a></td>
					</tr>
					</table>';
			echo '<br /><table class="selection">
					<tr>
						<td><img src="'.$RootPath.'/css/'.$Theme.'/images/printer.png" title="' . _('Order') . '" alt="" /></td>
						<td>' . ' ' . '<a href="' . $RootPath . '/PDFProformaInvoicePortrait.php?identifier='.$identifier . '&amp;PINo=' . $OrderNo . '" target="_blank">' .  _('Print  Proforma Invoice  (Portrait)')  . '</a></td>
					</tr>
					</table>';
////////////////////////////////////////////////////////////////////////////////////////////////////////
		}else {
...
/* update the order header then update the old order line details and insert the new lines */
	...
	$HeaderSQL = "UPDATE salesorders SET debtorno = '" . $_SESSION['Items'.$identifier]->DebtorNo . "',
										...
										approver = '". DB_escape_string($_SESSION['Items'.$identifier]->Approver) ."',
	...
	if($Quotation==1){ //handle Quotations and Orders print after modification
	...
	}elseif($Quotation==2){ //handle Quotations and Orders print after modification
		prnMsg(_('Profroma Invoice Number') .' ' . $_SESSION['ExistingOrder'.$identifier] . ' ' . _('has been updated'),'success');

		/*link to print the quotation */
		echo '<br /><table class="selection">
				<tr>
					<td><img src="'.$RootPath.'/css/'.$Theme.'/images/printer.png" title="' . _('Order') . '" alt=""></td>
					<td>' . ' ' . '<a href="' . $RootPath . '/PDFProformaInvoicePortrait.php?identifier='.$identifier . '&amp;PINo=' . $_SESSION['ExistingOrder'.$identifier] . '" target="_blank">' .  _('Print Proforma Invoice (Landscape)')  . '</a></td>
				</tr>
				</table>';
		echo '<br /><table class="selection">
				<tr>
					<td><img src="'.$RootPath.'/css/'.$Theme.'/images/printer.png" title="' . _('Order') . '" alt="" /></td>
					<td>' . ' ' . '<a href="' . $RootPath . '/PDFProformaInvoicePortrait.php?identifier='.$identifier . '&amp;PINo=' . $_SESSION['ExistingOrder'.$identifier] . '" target="_blank">' .  _('Print ProformacInvoice (Portrait)')  . '</a></td>
				</tr>
				</table>';
	}else{
...
echo'	<tr>
	...
	if ($CustomerLogin  == 1){
		...
		echo '<tr>
				...
				<td><select name="SalesPersons">';
		$PeopleResult = DB_query("SELECT salesmancode, salesmanname FROM salesman WHERE current=1 and salesmancode='".$_SESSION['UserID']."'");
		$SalesPeopleResult = DB_query("SELECT salesmancode, salesmanname FROM salesman WHERE current=1");
		if(DB_num_rows($PeopleResult)>0){
		$PersonRow = DB_fetch_array($PeopleResult);
		$_SESSION['Items'.$identifier]->SalesPerson = $_SESSION['UserID'];
			if ($PersonRow['salesmancode']==$_SESSION['Items'.$identifier]->SalesPerson){
				echo '<option selected="selected" value="' . $PersonRow['salesmancode'] . '">' . $PersonRow['salesmanname'] . '</option>';
			}	
		}else{
		if (!isset($_POST['SalesPersons']) AND $_SESSION['UserID']!=NULL ){
			$_SESSION['Items'.$identifier]->SalesPerson = $_SESSION['UserID'];
		}
		...
		echo '<tr><td>' .  _('Type Of Sale') .':</td>
				<td><select name="Quotation">';
		if ($_SESSION['Items'.$identifier]->Quotation==2){
			echo '<option selected="selected" value="2">' . _('Yes') . '</option>';
			echo '<option value="1">' . _('Quatation') . '</option>';
		} else {
			echo '<option value="2">' . _('Proforma Invoice') . '</option>';
			echo '<option selected="selected" value="1">' . _('Quatation') . '</option>';
			echo '<option selected="selected" value="0">' . _('Sales Order') . '</option>';
		}
		echo'<tr>';
		echo'<td>' .  _('Approved by') .':</td>
		<td><input type="text" size="25" maxlength="25" name="Approver" value="' . $_SESSION['Items'.$identifier]->Approver . '" title="' . _('Enter the Approver Name') . '" /></td>
	</tr>';
