...
echo _('TO'), ':<input type="text" required="required" class="date" alt="', $_SESSION['DefaultDateFormat'], '" id="datepicker" name="TransToDate" value="', $_POST['TransToDate'], '" minlength="0" maxlength="10" size="12" />';
...
$SQL = "SELECT systypes.typename,
			...
			WHERE debtortrans.debtorno = '" . $CustomerID . "'
				...
				AND debtortrans.trandate <='" .$TransToDate. "'
				AND debtortrans.status=0
				AND debtortrans.transtate=0
...
echo '<table class="selection"><thead>
	<tr>
		...
		<th class="ascending">', _('Receipt No.'), '</th>
...
echo '<p class="page_title_text"><img src="'.$RootPath.'/css/'.$Theme.'/images/supplier.png" title="' . _('Reversed payment') . '" alt="" /> ' .
		_('Reversed payment Transactions') . '</p>';
echo '<br /><table cellpadding="2" class="selection">';
	...
	if ($total<0){
	echo'<tr><td>Total Amount Reversed<td>'.$totalAmts.'</tr>';
	}else{
