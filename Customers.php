...
/* First check that all the necessary items have been setup */
	...
	echo '<input tabindex="6" type="hidden" name="Address4" size="42" maxlength="40" /><input tabindex="7" type="hidden" name="Address5" size="22" maxlength="20" />';
...
//DebtorNo exists - either passed when calling the form or from the form itself
	...
	if (isset($_GET['Modify'])) {
		...
		echo '<input ' . (in_array('Address4',$Errors) ?  'class="inputerror"' : '' ) .' type="hidden" name="Address4" size="42" maxlength="40" value="' . $_POST['Address4'] . '" /><input ' . (in_array('Address5',$Errors) ?  'class="inputerror"' : '' ) .' type="hidden" name="Address5" size="42" maxlength="40" value="' . $_POST['Address5'] . '" />';
		echo '<tr>
				<td>' . _('Country') . ':</td>
				<td><select name="Address6">';
		foreach ($CountriesArray as $CountryEntry => $CountryName){
