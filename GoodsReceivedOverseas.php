...
include('includes/header.inc');
	echo '<a href="'. $RootPath . '/PO_SelectOSPurchOrderOveseas.php">' . _('Back to Overseas Orders'). '</a>
	<br />';
...
$_SESSION['SupplierID'] = $_SESSION['PO'.$identifier]->SupplierID;
...
if (!isset($_POST['ProcessGoodsReceived'])) {
	if (!isset($_POST['DefaultReceivedDate'])){
		/* This is meant to be the date the goods are received - it does not make sense to set this to the date that we requested delivery in the purchase order - I have not applied your change here Tim for this reason - let me know if I have it wrong - Phil invoiceno */
		$_POST['DefaultReceivedDate'] = Date($_SESSION['DefaultDateFormat']);
		$_POST['InvoiceNo'] = $_POST['InvoiceNo'];
	}
...
if (($_SESSION['PO'.$identifier]->LineItems)>0 and !isset($_POST['ProcessGoodsReceived'])){
	...
	*/
	...
	//Setup & Format values for LineItem display
		...
		if ($LnItm->Controlled == 1) {
			...
			echo '<input type="hidden" name="RecvQty_' . $LnItm->LineNo . '" autofocus="autofocus" value="' . locale_number_format($LnItm->ReceiveQty,$LnItm->DecimalPlaces) . '" /><a href="GoodsReceivedControlledOversea.php?identifier=' . $identifier . '&amp;LineNo=' . $LnItm->LineNo . '">' . locale_number_format($LnItm->ReceiveQty,$LnItm->DecimalPlaces) . '</a></td>';
...
maxlength="10" size="10" value="' . locale_number_format(round($LnItm->ReceiveQty,$LnItm->DecimalPlaces),$LnItm->DecimalPlaces) . '" /></td>';
		...
		if ($LnItm->Controlled == 1) {
			if ($LnItm->Serialised==1){
				echo '<td><a href="GoodsReceivedControlledOversea.php?identifier=' . $identifier . '&amp;LineNo=' . $LnItm->LineNo . '">' .
...
if (($_SESSION['PO'.$identifier]->LineItems)>0){
...
// Otherwise if you try to fullfill item quantities separately will give error.
	...
	while ($myrow = DB_fetch_array($Result)) {
		...
		if ($_SESSION['PO'.$identifier]->LineItems[$LineNo]->GLCode != $myrow['glcode'] OR
			...
			echo '<div class="centre"><a href="' . $RootPath . '/PO_SelectOSPurchOrderOverseas.php">' .
				_('Select a different purchase order for receiving goods against') . '</a></div>';
			echo '<div class="centre"><a href="' . $RootPath . '/GoodsReceivedOverseas.php?PONumber=' . $_SESSION['PO'.$identifier]->OrderNo . '">' .  _('Re-read the updated purchase order for receiving goods against'). '</a></div>';
			unset($_SESSION['PO'.$identifier]->LineItems);
			unset($_SESSION['PO'.$identifier]);
			unset($_POST['ProcessGoodsReceived']);
            echo '</div>';
...
/*Now the SQL to do the update to the PurchOrderDetails */
			...
			if ($OrderLine->ReceiveQty >= ($OrderLine->Quantity - $OrderLine->QtyReceived)){
				$SQL = "UPDATE overseaorderdetails SET quantityrecd = quantityrecd + '" . $OrderLine->ReceiveQty . "',
													stdcostunit='" . $_SESSION['PO'.$identifier]->LineItems[$OrderLine->LineNo]->StandardCost . "',
													completed=1
						WHERE podetailitem = '" . $OrderLine->PODetailRec . "'";
			} else {
				$SQL = "UPDATE overseaorderdetails SET
...
/* Need to get the current location quantity will need it later for the stock movement */
	...
	echo '<br />
		<div class="centre">
			...
			<br />
			<br />
			<a href="PDFGrn.php?GRNNo='.$GRN .'&amp;PONo='.$PONo.'">' .  _('Print this Receipt Voucher (RV)') . '</a>
			<br />
			<br />
			<a href="' . $RootPath . '/SupplierInvoice.php?SupplierID=' . $_SESSION['SupplierID'] . '">' . _('Enter a Suppliers Invoice') . '</a><br />
			<br />
...
/*end of process goods received entry */
	...
	echo '<br />
		<div class="centre">
			<a href="' . $RootPath . '/Overseas_Header.php?ModifyOrderNumber=' .$_SESSION['PO'.$identifier]->OrderNo . '">' . _('Modify Order Items'). '</a>
		</div>
		<br />
		<div class="centre">
