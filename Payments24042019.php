...
#########################################################################################################################################################################
	...
	/*$pat  = unserialize($myr['particulars']);
	$lpono = unserialize($myr['lpo_no']);
	$invoiceno = unserialize($myr['invoice_no']);*/
	}
#######################################################################################################################################################################
...
#############################################################################################################################
	$headers = "SELECT *,a.voucherid,
				...
				INNER JOIN voteheadmaintenance c ON a.votehead=c.Votecode
				WHERE b.Financial_Year='".Date($_SESSION['DefaultDateFormat'],YearEndDate($_SESSION['YearEnd'],+0))."'
				AND c.Votecode = '" . $_SESSION['PaymentDetail' . $identifier]->Votecode . "'";
...
// End select Tag
	...
	if (isset($_POST['GLAmount'])) {
		echo '<tr>
				...
				<td><input type="text" required="required" name="GLAmount" maxlength="12" size="12" class="number" value="' . $_SESSION['PaymentDetail' . $identifier]->PV_Total . '" /></td>
			</tr>';
	} else {
		echo '<tr>
				<td>' . _('Amount') . ' (' . $_SESSION['PaymentDetail' . $identifier]->Currency . '):</td>
...
the fields for entry of receipt amt and disc */
	...
	echo '<table class="selection">
		<tr>
			...
			<td><input class="number" type="text" name="Amount" maxlength="12" size="13" value="' . $_SESSION['PaymentDetail' . $identifier]->PV_Total . '" /></td>
		</tr>';
