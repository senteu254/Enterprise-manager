...
/*If the page was called without $SupplierID passed to page then assume a new supplier is to be entered show a form with a Supplier Code field other wise the form showing the fields with the existing entries against the supplier will show for editing with only a hidden SupplierID field*/
	...
	echo '</select></td>
		...
		<tr>
			<td>' . _('Supplier Group Type') . ':</td>
			<td><select name="GroupType">';
	$result2=DB_query("SELECT groupid,groupname FROM suppliergrouptype");
	while ($myrow2 = DB_fetch_array($result2)) {
		echo '<option value="' . $myrow2['groupid'] . '">' . $myrow2['groupname'] . '</option>';
	} //end while loop
	echo '</select></td>
		</tr>';
...
//SupplierID exists - either passed when calling the form or from the form itself
	...
	$result2=DB_query("SELECT groupid, groupname FROM suppliergrouptype");
	while ($myrow2 = DB_fetch_array($result2)) {
		if ($_POST['GroupType']==$myrow['groupid']) {
			echo '<option selected="selected" value="'. $myrow2['groupid'] . '">' . $myrow2['groupname'] . '</option>';
		} else {
			echo '<option value="' . $myrow2['groupid'] . '">' . $myrow2['groupname'] . '</option>';
		}
	} //end while loop
