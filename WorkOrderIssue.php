...
if (!isset($_POST['WO']) OR !isset($_POST['StockID1'])) {
	...
	echo '<input type="hidden" name="StockID1" value="' .$_POST['StockID1'] . '" />';
}
...
if (isset($_POST['Process'])){ //user hit the process the work order issues entered.
	...
	$WOResult = DB_query("SELECT workorders.loccode,
							...
							WHERE woitems.stockid='" . $_POST['StockID1'] . "'
...
/************************ BEGIN SQL TRANSACTIONS ************************/
		...
		$SQL = "INSERT INTO stockmoves (stockid,
					...
					VALUES ('" . $_POST['IssueItem'] . "',
							...
							'" . $WOIssueNo . "',
							'" . $_POST['FromLocation'] . "',
							'" . FormatDateWithTimeForSQL($_POST['IssuedDate'].' '.date('H:i:s')) . "',
							'" . $_SESSION['UserID'] . "',
							'" . $IssueItemRow['cost'] . "',
							'" . $PeriodNo . "',
							'" . $_POST['WO'] . "',
		...
		if ($IssueItemRow['controlled'] ==1){
			...
			/*the form for entry of batch controlled items is only 15 possible fields */
				...
				/*  We need to add the StockSerialItem record and
					...
					$Lesult = DB_query("SELECT COUNT(*)
						FROM wobrasslotno
						WHERE stockid='".$_POST['StockID1']."' AND lotno='".$_POST['BatchRef' .$i]."' AND  brassid = '".$_POST['IssueItem']."'");
					$LoRow = DB_fetch_row($Lesult);
					if($LoRow[0]>0){
					$SQL = "UPDATE wobrasslotno SET quantity=quantity+'".(filter_number_format($_POST['Qty'.$i]))."'
								WHERE stockid='".$_POST['StockID1']."' AND lotno='".$_POST['BatchRef' .$i]."' AND  brassid = '".$_POST['IssueItem']."'";
					}else{
					$SQL = "INSERT INTO wobrasslotno (wonumber,
													  `stockid`, 
													  `brassid`, 
													  `lotno`, 
													  `calibre`, 
		...
		echo '<p><ul><li><a href="' . $RootPath . '/WorkOrderIssue.php?WO=' . $_POST['WO'] . '&amp;StockID=' . $_POST['StockID1'] . '">' . _('Issue more components to this work order') . '</a></li>';
...
if (!isset($_POST['IssueItem'])){ //no item selected to issue yet
	...
	$RequirmentsResult = DB_query("SELECT worequirements.stockid,
									...
									WHERE worequirements.wo='" . $_POST['WO'] . "' AND woitems.stockid='" . $_POST['StockID1'] . "'
	...
	while ($RequirementsRow = DB_fetch_array($RequirmentsResult)){
		...
		$sql = "SELECT locstock.quantity FROM locstock INNER JOIN locations
		ON locstock.loccode=locations.loccode
		INNER JOIN locationusers ON locationusers.loccode=locations.loccode AND locationusers.userid='" .  $_SESSION['UserID'] . "' AND locationusers.canview=1
		WHERE locstock.stockid = '" . $RequirementsRow['stockid'] . "'
		ORDER BY locations.locationname";
$ErrMsg = _('The stock held at each location cannot be retrieved because');
$DbgMsg = _('The SQL that was used to update the stock item and failed was');
$LocStockResult = DB_query($sql, $ErrMsg, $DbgMsg);
$myr = DB_fetch_array($LocStockResult);

		echo '<td class="number">' . locale_number_format($myr['quantity'],$RequirementsRow['decimalplaces']) . '</td>';
		...
		if (DB_num_rows($SearchResult)>1){
			...
			while ($myrow=DB_fetch_array($SearchResult)) {
				...
				if (!in_array($myrow['stockid'],$ItemCodes)){
					...
					$IssueLink = htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?WO=' . $_POST['WO'] . '&amp;StockID=' . $_POST['StockID1'] . '&amp;IssueItem=' . $myrow['stockid'] . '&amp;FromLocation=' . $_POST['FromLocation'];
					printf('<td>%s</td>
							<td>%s</td>
							<td>%s</td>
							<td>%s</td>
...
echo '<tr><td valign="top"><table class="selection">';
	...
	echo '</table></td><td>';
	$LocationOut = $_POST['FromLocation'];
