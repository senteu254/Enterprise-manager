...
if (isset($_SESSION['Contract'.$identifier]) AND
	...
	$_SESSION['Contract'.$identifier]->ContractNo=$_POST['ContractNo'];
	...
	$_SESSION['Contract'.$identifier]->field = $_POST['field'];
...
/*User hit the button to enter line items -
  then meta refresh to Contract_Items.php*/
	...
	if (isset($_POST['EnterContractBOM']) AND !$InputError){
		echo '<meta http-equiv="refresh" content="0; url=' . $RootPath . '/FarmContractBOM.php?identifier='.$identifier. '" />';
		echo '<br />';
		prnMsg(_('You should automatically be forwarded to the entry of the Contract line items page') . '. ' .
		_('If this does not happen') . ' (' . _('if the browser does not support META Refresh') . ') ' . '<a href="' . $RootPath . '/FarmContractBOM.php?identifier='.$identifier . '">' . _('click here') . '</a> ' . _('to continue'),'info');
		include('includes/footer.inc');
		exit;
	}
	if (isset($_POST['EnterContractRequirements']) AND !$InputError){
		echo '<meta http-equiv="refresh" content="0; url=' . $RootPath . '/FarmContractOtherReqts.php?identifier='.$identifier. '" />';
		echo '<br />';

		prnMsg(_('You should automatically be forwarded to the entry of the Contract requirements page') . '. ' .
		_('If this does not happen') . ' (' . _('if the browser does not support META Refresh') . ') ' .
		'<a href="' . $RootPath . '/FarmContractOtherReqts.php?identifier=' . $identifier . '">' . _('click here') . '</a> ' . _('to continue'),'info');
		include('includes/footer.inc');
		exit;
	}
} /* end of if going to contract BOM or contract requriements */

echo '<a href="'. $RootPath . '/FarmSelectContract.php">' .  _('Back to Contract Selection'). '</a><br />';
...
if (isset($_GET['ModifyContractRef'])){
	...
	include('includes/FarmContract_Readin.php');
...
/*The cancel button on the header screen - to delete the contract */
	...
	if ($OK_to_delete==true){
		$sql = "DELETE FROM FarmContractBOM WHERE contractref='" . $_SESSION['Contract'.$identifier]->ContractRef . "'";
		$ErrMsg = _('The contract bill of materials could not be deleted because');
		$DelResult=DB_query($sql,$ErrMsg);
		$sql = "DELETE FROM  Structure farmcontractreqts WHERE contractref='" . $_SESSION['Contract'.$identifier]->ContractRef . "'";
		$ErrMsg = _('The contract requirements could not be deleted because');
		$DelResult=DB_query($sql,$ErrMsg);
		$sql= "DELETE FROM farmcontracts WHERE contractref='" . $_SESSION['Contract'.$identifier]->ContractRef . "'";
...
if (isset($_POST['CommitContract']) OR isset($_POST['CreateQuotation'])){
	...
	if (DB_num_rows($result)==1){ // then we have an existing contract with this contractref
		...
		if ($ExistingContract['status']==0 AND ! $InputError){
			//then the quotation will need to be updated with the revised contract  if necessary
			$ContractBOM =0;
			foreach ($_SESSION['Contract'.$identifier]->ContractBOM as $Component) {
				$ContractBOM += ($Component->Item *  $Component->Quantity);
			}
			$ContractReqts=0;
			foreach ($_SESSION['Contract'.$identifier]->ContractReqts as $Requirement) {
		...
		$sql = "INSERT INTO farmcontracts ( contractref,
		                                contractNo,
										debtorno,
										contractdescription,
										categoryid,
		...
		$ErrMsg = _('Could not add a component to the contract bill of material');
		foreach ($_SESSION['Contract'.$identifier]->ContractBOM as $Component){
			$sql = "INSERT INTO farmcontractbom (contractref,
											stockid,
											workcentreadded,
											quantity)
							VALUES ('" . $_POST['ContractRef'] . "',
		...
		$ErrMsg = _('Could not add a requirement to the contract requirements');
		foreach ($_SESSION['Contract'.$identifier]->ContractReqts as $Requirement){
			$sql = "INSERT INTO farmcontractreqts (contractref,
												requirement,
												costperunit,
												quantity)
							VALUES ( '" . $_POST['ContractRef'] . "',
...
if (isset($_POST['SearchCustomers'])){
	...
	//insert wildcard characters in spaces
		...
		$SQL = "SELECT *
					FROM suppliers
					WHERE supplierid " . LIKE . " '%" . $_POST['CustCode'] . "%'
					ORDER BY supplierid";
 ...
 * so parse the $Select string into debtorno and branch code */
	...
	$sql = "SELECT *
					FROM suppliers
			WHERE supplierid='" . $_SESSION['Contract'.$identifier]->DebtorNo  . "'" ;
	...
	if (DB_num_rows($result)==0){
		...
		$_SESSION['Contract'.$identifier]->BranchName = $myrow['suppname'];
		$_SESSION['RequireCustomerSelection'] = 0;
...
if (!isset($_SESSION['Contract'.$identifier]->DebtorNo)
	...
	if (isset($result_CustSelect)) {
		...
		$TableHeader = '<tr>
							<th>' . _('Contractor No') . '</th>
							<th>' . _('Contractor Name') . '</th>
							<th>' . _('Contact') . '</th>
							<th>' . _('Phone') . '</th>
							<th>' . _('Email') . '</th>
						</tr>';
		...
		while ($myrow=DB_fetch_array($result_CustSelect)) {
			...
			if ($LastCustomer != $myrow['supplierid']) {
				echo '<td>' .  $myrow['supplierid']  . '</td>';
			} else {
				echo '<td></td>';
			}
			echo '<td><input type="submit" name="Submit'.$j.'" value="' . $myrow['suppname'] . '" /></td>
					<input type="hidden" name="SelectedCustomer'.$j.'" value="'. $myrow['supplierid'] . '" />
					<td>' . $myrow['address1']  . '</td>
					<td>' . $myrow['telephone'] . '</td>
					<td>' . $myrow['email'] . '</td>
					</tr>';
			$LastCustomer=$myrow['supplierid'];
			$j++;
...
//end if RequireCustomerSelection
	...
	echo '<p class="page_title_text">
			<img src="'.$RootPath.'/css/'.$Theme.'/images/contract.png" title="' . _('Contract') . '" alt="" /> ' . $_SESSION['Contract'.$identifier]->BranchName;
	...
	echo '</td>
		...
		<tr>
			<td>' . _('Contract No.') . ':</td>
			<td><input type="text" autofocus="autofocus" required="required" size="21" name="contractNo" title="' . _('Enter the contract No.') . '" data-type="no-illegal-chars" value="' . $_SESSION['Contract'.$identifier]->ContractNo . '" /></td>
		</tr>
		<tr>
			<td>' . _('Category') . ':</td>
	...
	echo '</select></td></tr>';
		$sql2 = "SELECT code, Field_Name FROM farmfield";
	$result2 = DB_query($sql2);

	echo '<tr><td>' . _('Select Field') . ': </td><td>';

	echo '<select name="field">';

while ($myrow2 = DB_fetch_array($result2)) {
		if (isset($_POST['field']) and $myrow2['code']==$_POST['field']) {
			echo '<option selected="selected" value="'.$myrow2['code'] . '">' . $myrow2['Field_Name'] . '</option>';
		} else {
			echo '<option value="'.$myrow2['code'] . '">' . $myrow2['Field_Name'] . '</option>';
		}
	} //end while loop
	...
	echo '</select></td>
		...
		<tr>
			...
			<td><textarea name="ContractDescription" style="width:98%" required="required" title="' . _('A description of the contract is required') . '" minlength="5" rows="5" cols="40">' . $_SESSION['Contract'.$identifier]->ContractDescription . '</textarea></td>
		</tr>';
