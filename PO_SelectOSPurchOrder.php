...
echo '<option value="All">All Store Locations</option>';
...
if (isset($StockItemsResult)) {
	...
	if (isset($OrderNumber) AND $OrderNumber != '') {
		...
		if (isset($SelectedSupplier)) {
			...
			if ($_POST['StockLocation']=='All'){
				...
				$stockloc =" AND purchorders.intostocklocation = '" . $_POST['StockLocation'] . "'";
			}
	...
	while ($myrow = DB_fetch_array($PurchOrdersResult)) {
		...
		echo '<td><a href="' . ($myrow['completed']==0? $ModifyPage : '#') . '">' . $myrow['orderno'] . '</a></td>
		...
		echo '<td>' . ($myrow['completed']==0? _($myrow['status']) : 'Completed') . '</td>
				<td>' . ($myrow['completed']==0? $PrintPurchOrder : '') . '</td>
				<td>' . ($myrow['completed']==0? $ReceiveOrder : '') . '</td>
			</tr>';
