...
$sql = "UPDATE qadailyreport SET process_level='" . 4 . "'
					WHERE id = '". $SelectedUser . "'";

		$result = DB_query($sql);
DB_query("DELETE FROM qadailyreportremarks WHERE approver=4 AND refid='". $SelectedUser ."'");
$sql = "INSERT INTO qadailyreportremarks (refid,remarks,approver,approvertitle,approvername)
					VALUE('". $SelectedUser ."','". $_POST['remarks'] ."',4,'QA Manager Remarks','". $_SESSION['UsersRealName'] ."')";
...
$sql = "UPDATE qadailyreport SET process_level='" . 12 . "'
					WHERE id = '". $SelectedUser . "'";

		$result = DB_query($sql);
DB_query("DELETE FROM qadailyreportremarks WHERE approver=4 AND refid='". $SelectedUser ."'");
$sql = "INSERT INTO qadailyreportremarks (refid,remarks,approver,approvertitle,approvername)
					VALUE('". $SelectedUser ."','". $_POST['remarks'] ."',4,'QA Manager Reason For Rejection','". $_SESSION['UsersRealName'] ."')";
...
echo '<a href="' . $RootPath . '/QADailyReportManager.php">' . _('Back to Main Menu') . '</a>';
...
if (!isset($SelectedUser)) {
	$sql = "SELECT *
				FROM qadailyreport where process_level >=3 ORDER BY id DESC";
	$result = DB_query($sql);
	...
	if ($myrow['date']=='') {
		...
		if($myrow['process_level'] >=4 && $myrow['process_level'] != 12){
		$status = '<td><b class="jj" style="background: #3399FF; font-size:12px;" id="">Confirmed</b></td>';
		$print='<td><a href="%sid=%s">' . _('Print PDF') . '</a></td>';
