...
header('location:index.php');
							...
							$view = (isset($_GET['Ref']) && $_GET['Ref'] != '') ? $_GET['Ref'] : '';
								switch ($view) {
								case 'Create-Request' :
									$title="Create Request For Purchase";
									$subtitle ='Create New Request For Purchase';
								...
								case 'Archives' :
									$title="Completed Request";
									$subtitle ='List of completed Internal Stock Request';
									$content1='<iframe style="border:none; border-top:solid; background:#f5f5dc;" src="'.$path.'Archives.php" height="610px" width="100%"></iframe>';
									$class8= 'class="active"';	
									break;
							...
							INNER JOIN irq_documents f ON z.doc_id = f.doc_id
							INNER JOIN locations g ON a.loccode = g.loccode
							WHERE closed=1 AND z.Requesteddate >= DATE_SUB(NOW(), INTERVAL 1 MONTH) AND z.doc_id='".$_SESSION['Doc_Store']."' AND CASE WHEN d.userid ='HOD' THEN e.authoriser='".$_SESSION['UserID']."' WHEN d.userid ='ISSUE' THEN g.authoriser='".$_SESSION['UserID']."' WHEN d.userid ='PROCURE' THEN g.purchasing_officer='".$_SESSION['UserID']."' ELSE d.userid='".$_SESSION['UserID']."' END";
							$result= mysqli_query($conn,$query) or die (mysqli_error($conn));
							...
							INNER JOIN irq_documents f ON z.doc_id = f.doc_id
							INNER JOIN locations g ON a.loccode = g.loccode
							WHERE closed=1 AND z.Requesteddate >= DATE_SUB(NOW(), INTERVAL 1 MONTH) AND z.doc_id='".$_SESSION['Document_id']."' AND CASE WHEN d.userid ='HOD' THEN e.authoriser='".$_SESSION['UserID']."' WHEN d.userid ='ISSUE' THEN g.authoriser='".$_SESSION['UserID']."' WHEN d.userid ='PROCURE' THEN g.purchasing_officer='".$_SESSION['UserID']."' ELSE d.userid='".$_SESSION['UserID']."' END GROUP BY z.requestid";
							$result= mysqli_query($conn,$query) or die (mysqli_error($conn));
							$cnum=mysqli_num_rows($result);
							}
							$Lang = $_SESSION['Lang'];
				...
				<div class="controls">
					<nav class="links">
						<ul>
							...
							<li><a href="<?php echo $_SERVER['PHP_SELF'].'?Ref=Archives' ?>" class="ico2">Archive</a></li>
						</ul>
					</nav>
