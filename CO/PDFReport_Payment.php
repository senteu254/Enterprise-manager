...
function Header()
{
    ...
    $this->SetFont('Arial','B',12);
	 $this->Ln(5);
    $this->Cell(0,10,'CONTRACT PAYMENT REPORT',0,0,'C');
...
function FancyTable($header, $data,$qry)
{
	...
	while($row = mysqli_fetch_row($data))
	{
		...
		// Arial bold 15
    	$this->SetFont('Arial','B',10);
		$this->Cell($w[2],8,'Total Amount Paid:',0,0,'R');
		$this->SetFont('Arial','B',12);
		$this->Cell($w[3],8,number_format($rows['total_paid'], 2),1,0,'R');
		$this->Ln();
	
	
}
function Particulars($rest)
{
   while($row=mysqli_fetch_array($rest)){
	$conname=$row['Contract_Name'];
    ...
    $this->SetFont('Arial','B',10);
    $this->Ln(6);
    $this->Cell(0,0,'Contract Name      :  '.$conname.'',0,0,'L');
    $this->Ln(6);
    $this->Cell(0,0,'Contract No.          :  '.$no.'',0,0,'L');
    $this->Ln(6);
    $this->Cell(0,0,'Begin Date             :  '.$bdate.'',0,0,'L');
    $this->Ln(6);
    $this->Cell(0,0,'Amount Allocated :  '.$amnt.'',0,0,'L');
	$this->Ln(9);
    $this->Cell(100,0,'All Amounts stated in '.$curr.'',0,1,'L');
	$this->Ln(3);
}
//pdf class end
}
...
$id=$_GET['id'];
$SQL = "SELECT PaymentID,Description,Amount_Paid,Amount_FC,Amount_LC,Date_Paid,Currency FROM contract_payment WHERE ContractID=".$id."";
$data = mysqli_query($conn,$SQL);

$qry = mysqli_query($conn," SELECT SUM(Amount_LC) AS total_paid FROM contract_payment WHERE ContractID=".$id."");

$header = array('P.ID', 'Description', 'Date Paid', 'Amnt (L.C)');

$SQLi = "SELECT * FROM contract_details WHERE ContractID=$id";
