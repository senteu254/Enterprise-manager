...
<?php 
echo '<form method="post" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Application=CON&Page=Assign">';
...
<div class="box" style="width:130%;">
					...
					<div class="box-head" style="background:#337ab7; color: white; border-radius: 7px; no-repeat 0 0; padding:0 0 0 15px;">
						...
						<h2 class="left"><span class="logo" >Contractor Name:</span>
						<select class="field small-field" required="true" onChange="this.form.submit();" name="id">
						<option selected="selected" value="">--Please Select A Supplier--</option>
						<?php
...
<div class="box" style="width:130%;">
				...
				?>
	<table style="width:100%;" border="0" class="tble" cellspacing="0" cellpadding="0">
		<tr>
       <td><label style="font-size:14px; font-weight:bold">Contract Name</label>
		<select size="6" name="cont" style="width: 285px;float:left;">
		<?php
		$SQLc = "SELECT  * FROM contract_details WHERE Status=0";
		$Queryc = DB_query($SQLc);
		while($rowc=DB_fetch_array($Queryc)){
		$contract=$rowc['Contract_Name'];
		$id=$rowc['ContractID'];
		$no=$rowc['Contract_Number'];
		
		echo "<option value=".$id.">".$no."-".$contract."</option>";
		}
		?>
		</select>
      <td width="110px">
...
<label style="font-size:14px; font-weight:bold"><?php if(isset($nameselected)){echo "Assigned to ".$nameselected;}else{echo "";} ?></label>
	<select   size="6" name="contid" style="width: 285px;float:left;">
	<?php
