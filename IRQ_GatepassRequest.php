...
header('location:index.php');
							...
							$view = (isset($_GET['Ref']) && $_GET['Ref'] != '') ? $_GET['Ref'] : '';
								switch ($view) {
								case 'Create-Request' :
									$title="Create Gate Pass Request";
									$subtitle ='Create New Gate Pass Request';
								...
								case 'Follow-Up' :
									...
									$content=$path.'Follow_Up_Gatepass.php';	
									$class2= 'class="active"';	
									break;
								...
								case 'Review-Tasks' :
									...
									$content=$path.'Follow_Up_Maintenance.php';		
									$class4= 'class="active"';	
									break;
								...
								case 'Draft' :
									$title="Saved Request";
									$subtitle ='Created and saved Transport Request';
							...
							INNER JOIN irq_gatepass z on a.requestid = z.gatepassid 
							WHERE draft=1 AND closed=0 AND initiator = '".$_SESSION['UserID']."'";
							$result= mysqli_query($conn,$query) or die (mysqli_error($conn));
							...
							INNER JOIN departments e ON a.departmentid = e.departmentid
							INNER JOIN irq_documents f ON z.doc_id = f.doc_id
							WHERE closed=1 AND userid='". $_SESSION['UserID'] ."' GROUP BY z.requestid";
							$result= mysqli_query($conn,$query) or die (mysqli_error($conn));
...
border-radius: 4px;}
.note_box img{margin-left:4px; margin-top:5px;}
.add{width:140px; 
font-family:Tahoma; margin-top:-39px; font-size:14px;
padding:4px; margin-left:48px; }
.com_im{margin-top:-7px;}
.time{color:#7f7f7f; margin-left:30px;
margin-top:-20px;}.close{float:right; margin-right:2px; cursor:pointer;}
				...
				$.ajax({
				  ...
				  success: function(){
					...
					if (window.isActive) {
						...
						'<img src="<?php echo $path;?>/images/Doc.png" width="40" height="40" class="pro" align="middle">'+
						'<img src="<?php echo $path;?>/images/close1.png" width="17" height="17" align="middle" class="close">'+
						...
						'</div>'+
						'<div class="com_im"><img src="<?php echo $path;?>/images/nW-U1RPGDdA.png" width="20" height="20" align="middle">'+
