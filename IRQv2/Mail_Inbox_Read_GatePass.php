...
if($final==TRUE){
		...
		DB_query($insert);
		$_SESSION['msg'] = _('Gate Pass Requisition No. '). $LID . ' ' . _('has been appoved');
		}else{
$HSQL="INSERT INTO irq_authorize_state (requisitionid,
											level,
											approvaldate,
...
?>
		...
		echo '<tr><th>Request No</th><th>Department</th><th>Building</th><th>Requested Date</th><th>Requested By</th></tr>';
		echo '<tr><td align="center">'.$rows['requestid'].'</td>';
		echo '<td align="center">'.$rows['description'].'</td>';
		echo '<td align="center">'.$rows['building'].'</td>';
		echo '<td align="center">'.date("d, M Y",strtotime($rows['Requesteddate'])).'</td>';
		echo '<td align="center" >'.$rows['initiator'].'</td>';
		echo '</tr>';
		echo '<tr><td align="center" colspan="5">Invoice No. : '.$rows['invoiceno'].'</td></tr>';
		echo '<tr><td align="center" colspan="5">Delivery No. : '.$rows['deliveryno'].'</td></tr>';
		echo '</table>';		
		echo '<br />';
		echo '<table class="table" style="width:100%; ">';
		echo '<tr><td>Vehicle Reg No.</td>';
		echo '<td>'.$rows['vregno'].'</td>';
		echo '<td>Vehicle Type</td>';
		echo '<td>'.$rows['vtype'].'</td></tr>';
		echo '<tr><td>Vehicle\'s Company</td>';
		echo '<td colspan="3">'.$rows['vcompany'].'</td>';
		echo '<tr><td>Driver Name</td>';
		echo '<td>'.$rows['driver_name'].'</td>';
		echo '<td>License No</td>';
		echo '<td>'.$rows['licenseno'].'</td></tr>';
		echo '<tr><td>Passenger Name</td>';
		echo '<td>'.$rows['passanger_name'].'</td>';
		echo '<td>Passenger ID No</td>';
		echo '<td>'.$rows['passanger_idno'].'</td></tr>';
		echo '</table>';
		}else{
		echo '<table class="table" style="width:100%; ">';
		echo '<tr><td>Request No.</td>';
		echo '<td>'.$rows['requestid'].'</td>';
		echo '<td>Requested Date</td>';
		echo '<td>'.date("d, M Y",strtotime($rows['Requesteddate'])) .' Time:'. date("H:i:s",strtotime($rows['Requesteddate'])).'</td></tr>';
		echo '<tr><td>Bearer Name</td>';
		echo '<td>'.$rows['driver_name'].'</td>';
		echo '<td>Requestion Officer</td>';
		echo '<td>'.$rows['requesting_officer'].'</td></tr>';
		echo '<tr><td>Vehicle Reg No</td>';
		echo '<td>'.$rows['vregno'].'</td>';
		echo '<td>Vehicle Type</td>';
		echo '<td>'.$rows['vtype'].'</td></tr>';
		echo '<tr><td>Frrom Department</td>';
		echo '<td>'.$rows['description'].'</td>';
		echo '<td>Item Destination</td>';
		echo '<td>'.$rows['destination'].'</td></tr>';
		echo '</table>';
		}
		echo '<br />';	
		echo '<div class="line"> ';
		...
		echo '</div><br />';
		echo '<table class="table table-hover table-striped" style="width:80%; ">';
		echo '<tr>
						<th>' . _('S/No') . '</th>
						<th>' . _('Oty') . '</th>
		...
		echo '<br />';
            ...
            <div class="box-footer">
			  ...
			  <?php if($final == TRUE){  ?>
			   <button type="submit" name="Cancel" onclick="return confirm('Are you sure you want to Cancel this Request?')" class="btn btn-danger"><i class="fa fa-ban"></i> Cancel</button>
			   <?php } ?>
			  ...
			  <a href="http://localhost/erp20181123/IRQ_PDFGatepassPortrait.php?id=<?php echo $rows['requestid']; ?>" target="_blank"><button type="button" class="btn btn-default"><i class="fa fa-print"></i> Print</button></a>
            </div>
			</form>
			<br></br>
			<?php
