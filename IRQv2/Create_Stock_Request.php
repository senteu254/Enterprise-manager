...
if(isset($_GET['Link']) && $_GET['Link']=='StoresRequest'){
$doc=4;
$link = '&Link=StoresRequest';
}else{
$doc=1;
$link = '&Link=PurchaseRequest';
}
...
/******************************************************************************************/
		...
		echo _('<div class="alert alert-success alert-dismissible">
                <h4><i class="icon fa fa-check"></i> Success</h4>Requisition No. '). $RequestNo . ' ' . _('has been forwarded to'). ' ' . $myEmail['realname'] . ' for authoritation</div>'; //. _('and emailed to') . ' ' . $myEmail['email'].'</div>';

		}else{
		echo _('<div class="alert alert-success alert-dismissible">
                <h4><i class="icon fa fa-check"></i> Success</h4> Requisition No. '). $RequestNo. ' ' . _('has been forwarded for authoritation').'</div>';
			}
/*--------------------------------------------------------------------------------------------------*/
	...
	if($state==1){
	echo _('<div class="alert alert-success alert-dismissible">
                <h4><i class="icon fa fa-check"></i> Success</h4>Your request has been saved as a Draft.').'</div>';
		
		}
	}
	unset($_SESSION['Request']);
	echo '<br /><div class="centre"><a href="'. htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?Application=IRQ2'.$link.'&New=Yes">' . _('Create another request') . '</a></div>';
			exit;
}

echo '<form action="'.$mainlink.$link.'" method="post">';
...
?>
	...
	<table class="selection table" style="border:none;">
  <tr>
    <th colspan="2"><?php if(isset($_GET['Link']) && $_GET['Link']=="StoresRequest"){ echo 'Store Requisition and Issue Voucher'; }else{ echo 'Request For Purchase or Services'; }?></th>
  </tr>
  <tr>
    <td>Department</td>
						<?php
...
?>
  ...
  <tr>
    ...
    <td><?php echo '<textarea name="Narrative" cols="40" rows="2">' . $_SESSION['Request']->Narrative . '</textarea>'; ?></td>
  </tr>
...
if (isset($_GET['Edit'])) {
	echo '<br /><form action="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?Application=IRQ2'.$link.'" method="post">';
    echo '<div>';
	echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
	echo '<table class="selection table">';
	echo '<tr>
			<th colspan="2">' . _('Edit the Request Line') . '</th>
		</tr>';
	echo '<tr>
			<td>' . _('Line number') . '</td>
...
echo '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?Application=IRQ2'.$link.'" method="post">';
echo '<div>';
echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
echo '<br />
	<table class="selection table" style="border:none;">
	<tr>
		<th colspan="7">' . _('Details of Items Requested') . '</th>
	</tr>
	<tr>
		<th>' .  _('Line Number') . '</th>
...
foreach ($_SESSION['Request']->LineItems as $LineItems) {
	...
	echo '<td>' . $LineItems->LineNumber . '</td>
			...
			<td>' . $LineItems->UOM . '</td>
			<td><a href="'. htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?Application=IRQ2'.$link.'&Edit='.$LineItems->LineNumber.'">' . _('Edit') . '</a></td>
			<td><a href="'. htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?Application=IRQ2'.$link.'&Delete='.$LineItems->LineNumber.'">' . _('Delete') . '</a></td>
		</tr>';
}
...
echo '<table class="selection table" style="border:none;">
	<tr>
...
echo '</tr>
		<tr>
			<td></td>
			<td>' . _('Enter partial') . ' <b>' . _('Stock Code') . '</b>:</td>';
...
if (isset($SearchResult)) {
	...
	echo '<br />
		...
		<br />
		<form action="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?Application=IRQ2'.$link.'" method="post" id="orderform">
		<div>
		<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />
		<table class="table1 table" style="border:none">
		<tr>
			<td>
