...
if(isset($_POST['vregno']) and $decision==1){
		$a = "UPDATE irq_transport SET vregno='".$_POST['vregno']."' WHERE TransportID=".$LID."";	
		DB_query($a);
	}
	if($final==TRUE){
		...
		DB_query($insert);
		$_SESSION['msg'] = _('Transport Requisition No. '). $LID . ' ' . _('has been appoved');
		}else{
$HSQL="INSERT INTO irq_authorize_state (requisitionid,
											level,
											approvaldate,
...
?>
		...
		echo '</table>';
			...
			<?php
			if($decision==1){
		echo '<div style="width:90%" align="left" >Vehicle Available : <input name="vregno" placeholder="Enter Vehicle Reg No" style="height:25px; background:#CCCCFF; font-size:14px;" type="text" /></div>';
		}
