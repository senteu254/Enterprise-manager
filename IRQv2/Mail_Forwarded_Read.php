...
$LID = $_GET['LID'];
						...
						$results = "SELECT *, a.loccode, z.doc_id, a.departmentid,z.closed FROM irq_request z 
							...
							WHERE draft=0 AND b.Sent=1 AND z.requestid='" . $LID . "' GROUP BY requestid";
...
?>
			...
			$query = "SELECT *, locstock.quantity as stkbal, irq_stockrequestitems.quantity as qty FROM irq_stockrequestitems,stockmaster,locstock
				...
				echo '<tr>
					...
					<td align="center">'.($row['cancelled'] ==2 ? 'RFP(Ref:'.$row['on_order'].')' : ($row['cancelled'] ==1 ? '<span title="Cancelled By '.$row['cancelled_by'].'">Cancelled</span>' : ($row['cancelled'] ==0 && $row['completed'] ==0 ? 'Progress' : 'Completed'))).'</td>';
			echo '</tr>';
			}
			echo '<tbody></table>';
			?>
...
$commentresults= DB_query($comment);
		...
		echo' <tr>
				...
				<td><div class="bubble"><span class="time">From: <a href="#">'.$myrows['approver'].'</a></span> <br /> '.htmlentities($myrows['approver_comment']).'
