...
$search="";
						...
						$statement = " irq_request z 
							...
							WHERE draft=0 AND b.Sent=1 AND ".$search."
						...
						$results = "SELECT requestid as id, e.description, doc_name, Requesteddate, y.realname,z.doc_id,level,approvaldate FROM {$statement} GROUP BY requestid";
						$results2 = "SELECT requestid as id, e.description, doc_name, Requesteddate, y.realname,z.doc_id,level,approvaldate FROM {$transport} GROUP BY requestid";
						$results3 = "SELECT requestid as id, e.description, doc_name, Requesteddate, y.realname,z.doc_id,level,approvaldate FROM {$maintenance} GROUP BY requestid";
						$results4 = "SELECT requestid as id, e.description, doc_name, Requesteddate, y.realname,z.doc_id,level,approvaldate FROM {$gatepass} GROUP BY requestid";
	...
	?>
              ...
              <div class="mailbox-controls">
                ...
                <!-- /.btn-group -->
                <a href="index.php?Application=IRQ2&Link=Forwarded"><button type="button" title="Refresh" class="btn btn-default btn-sm"><i class="fa fa-refresh"></i></button></a>
				</div>
				<div class="col-xs-4">
			  ...
			  <div class="pull-right">
				<div class="btn-group">
				<?php
				echo pagination_inbox($sqlforPages,$per_page,$page,$url='?Application=IRQ2&Link=Forwarded&');
				?>
                  </div>
                  <!-- /.btn-group -->
                </div>
                ...
                <table style="width:100%" class="table table-hover table-striped">
				 ...
				 if($num_rows>0){
					...
					$RType = 'ForwardedReadTransport';
					}elseif($row['doc_id']==5 || $row['doc_id']==6){
					$RType = 'ForwardedReadMaintenance';
					}elseif($row['doc_id']==7 || $row['doc_id']==8){
					$RType = 'ForwardedReadGatePass';
					}
                  echo '<tr>
