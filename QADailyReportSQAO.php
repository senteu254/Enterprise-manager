...
$sql = "UPDATE qadailyreport SET process_level='" . 3 . "'
					WHERE id = '". $SelectedUser . "'";

		$result = DB_query($sql);
DB_query("DELETE FROM qadailyreportremarks WHERE approver=3 AND refid='". $SelectedUser ."'");
$sql = "INSERT INTO qadailyreportremarks (refid,remarks,approver,approvertitle,approvername)
					VALUE('". $SelectedUser ."','". $_POST['remarks'] ."',3,'SQAO Remarks','". $_SESSION['UsersRealName'] ."')";
...
$sql = "UPDATE qadailyreport SET process_level='" . 11 . "'
					WHERE id = '". $SelectedUser . "'";

		$result = DB_query($sql);
DB_query("DELETE FROM qadailyreportremarks WHERE approver=3 AND refid='". $SelectedUser ."'");
$sql = "INSERT INTO qadailyreportremarks (refid,remarks,approver,approvertitle,approvername)
					VALUE('". $SelectedUser ."','". $_POST['remarks'] ."',3,'SQAO Reason For Rejection','". $_SESSION['UsersRealName'] ."')";
...
echo '<a href="' . $RootPath . '/QADailyReportSQAO.php">' . _('Back to Main Menu') . '</a>';
...
if (!isset($SelectedUser)) {
	$sql = "SELECT *
				FROM qadailyreport  where process_level>=2 ORDER BY id DESC";
	$result = DB_query($sql);
	...
	if ($myrow['date']=='') {
		...
		if($myrow['process_level'] >=3 && $myrow['process_level'] != 11){
