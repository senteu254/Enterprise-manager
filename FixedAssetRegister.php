...
if (isset($_POST['submit']) OR isset($_POST['pdf']) OR isset($_POST['csv'])) {
	...
	$sql = "SELECT fixedassets.assetid,
					...
					SUM(CASE WHEN (fixedassettrans.transdate >='" . $DateFrom ."'  AND fixedassettrans.transdate <='" . $DateTo . "' AND fixedassettrans.fixedassettranstype='cost') THEN fixedassettrans.amount ELSE fixedassets.cost END) AS periodadditions,
		...
		*/
		if (Date1GreaterThanDate2(ConvertSQLDate($myrow['disposaldate']),$_POST['FromDate']) OR $myrow['disposaldate']='0000-00-00') {
			...
			if (isset($_POST['pdf'])) {
				...
				$LeftOvers = $pdf->addTextWrap($XPos, $YPos, 100 - $Left_Margin, $FontSize, $myrow['assetid']);
				...
				* */
				...
				$LeftOvers = $pdf->addTextWrap($XPos + 220, $YPos, 100 - $Left_Margin, $FontSize, ConvertSQLDate($myrow['datepurchased']));
