...
/* END Brought from class.pdf.php constructor */
	...
	nobble the invoice reprints */
		...
		if (DB_num_rows($result)==1) {
			...
			$neww ="SELECT deliverynotenumber FROM deliverynotes WHERE salesorderno='".$myrow['orderno']."'";
			$resultx=DB_query($neww);
			$myr = DB_fetch_array($resultx);
			...
			if ($InvOrCredit=='Invoice') {
				...
				$sql = "SELECT stockmoves.stockid,
								...
								((1 - stockmoves.discountpercent) * stockmoves.price * " . $ExchRate . "* SUM(-stockmoves.qty)) AS fxnet,
								(stockmoves.price * " . $ExchRate . ") AS fxprice,
								stockmoves.narrative,
								SUM(stockmoves.linetax) AS linetax,
								stockmaster.units,
							...
							AND stockmoves.show_on_inv_crds=1 GROUP BY stockmoves.stockid limit 16";
			} else {
		/* only credit notes to be retrieved */
			...
			if (DB_error_no()!=0 OR DB_num_rows($result)==0) {
				...
				while ($myrow2=DB_fetch_array($result)) {
					...
					$DisplayLineTax=locale_number_format($myrow2['linetax'],$myrow['decimalplaces']);
					...
					$LeftOvers = $pdf->addTextWrap($Left_Margin+292,$YPos,96,$FontSize,$myrow2['units'],'right');
					$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,95,$FontSize,$DisplayQty,'right');
					$LeftOvers = $pdf->addTextWrap($Left_Margin+520,$YPos,35,$FontSize,$DisplayPrice,'centre');
					$LeftOvers = $pdf->addTextWrap($Left_Margin+558,$YPos,50,$FontSize,$DisplayDiscount,'right');
					$LeftOvers = $pdf->addTextWrap($Left_Margin+620,$YPos,50,$FontSize,$DisplayLineTax,'right');
			...
			if (($YPos-$Bottom_Margin)<(2*$line_height+32)) {
				PrintLinesToBottom ();
				include ('includes/PDFTransPageHeader.inc');
			}
			...
			$pdf->line($Left_Margin+97, $TopOfColHeadings+12,$Left_Margin+97,$Bottom_Margin+(4*$line_height+32));

			/* Print a column vertical line */
			$pdf->line($Left_Margin+350, $TopOfColHeadings+12,$Left_Margin+350,$Bottom_Margin+(4*$line_height+32));

			/* Print a column vertical line */
			$pdf->line($Left_Margin+390, $TopOfColHeadings+12,$Left_Margin+390,$Bottom_Margin+(4*$line_height+32));

			/* Print a column vertical line */
			$pdf->line($Left_Margin+470, $TopOfColHeadings+12,$Left_Margin+470,$Bottom_Margin+(4*$line_height+32));
			
			/* Print a column vertical line */
			$pdf->line($Left_Margin+550, $TopOfColHeadings+12,$Left_Margin+550,$Bottom_Margin+(4*$line_height+32));

			/* Print a column vertical line */
			$pdf->line($Left_Margin+610, $TopOfColHeadings+12,$Left_Margin+610,$Bottom_Margin+(4*$line_height+32));

			$pdf->line($Left_Margin+670, $TopOfColHeadings+12,$Left_Margin+670,$Bottom_Margin+(4*$line_height+32));

			/* Rule off at bottom of the vertical lines */
			$pdf->line($Left_Margin, $Bottom_Margin+(4*$line_height+32),$Page_Width-$Right_Margin,$Bottom_Margin+(4*$line_height+32));
			...
			$pdf->addTextWrap($Left_Margin+5,$YPos+36,280,$FontSize,_('Payment Terms') . ': ' . $myrow['terms']);
			$pdf->addText($Left_Margin+5, $YPos-30, $FontSize, _('Sales Person') . ':');
			$pdf->addText($Left_Margin+70, $YPos-30, $FontSize, $myrow['salesmanname']);
		//      $pdf->addText($Page_Width-$Right_Margin-392, $YPos - ($line_height*3)+22,$FontSize, _('Bank Code:***** Bank Account:*****'));
		//	$FontSize=10;
		
			$pdf->addText($Left_Margin+220, $YPos-30, $FontSize, _('Official Stamp') . ':___________________');
			$pdf->addText($Left_Margin+380, $YPos-30, $FontSize, _('Signature') . ':_________________________');
			...
			$pdf->addText($Page_Width-$Right_Margin-270, $YPos+47,$FontSize, _('Sub Total'));
			$LeftOvers = $pdf->addTextWrap($Left_Margin+642,$YPos+37,120,$FontSize,$DisplaySubTot, 'right');
			$LeftOvers = $pdf->addTextWrap($Left_Margin+550,$YPos+37,120,$FontSize,$DisplayTax, 'right');

			$pdf->addText($Page_Width-$Right_Margin-270, $YPos+34,$FontSize, _('Freight'));
			$LeftOvers = $pdf->addTextWrap($Left_Margin+642,$YPos+26,120,$FontSize,$DisplayFreight, 'right');

			$pdf->addText($Page_Width-$Right_Margin-270, $YPos+22,$FontSize, _('Tax'));
			$LeftOvers = $pdf->addTextWrap($Left_Margin+642,$YPos-($line_height)+27,120, $FontSize,$DisplayTax, 'right');
			...
			$pdf->line($Page_Width-$Right_Margin-222, $YPos+$line_height+32,$Page_Width-$Right_Margin-222,$Bottom_Margin);
			...
			if ($InvOrCredit=='Invoice') {
				...
				$FontSize=9;
				$YPos-=4;
				//$LeftOvers = $pdf->addTextWrap($Left_Margin+5,$YPos+15,220,$FontSize,$_SESSION['RomalpaClause']);
				//while (mb_strlen($LeftOvers)>0 AND $YPos > $Bottom_Margin) {
				//	$YPos-=12;
				//	$LeftOvers = $pdf->addTextWrap($Left_Margin+5,$YPos+15,220,$FontSize,$LeftOvers);
				//}
				...
				$BankResult = DB_query("SELECT bankaddress,
												...
												bankaccountcode
										FROM bankaccounts
										WHERE accountcode='" . $myrow['bankacc'] . "'");
				...
				if ($PrintBankDetails){
					...
					$YPos-=4;
					$LeftOvers = $pdf->addTextWrap($Left_Margin+5,$YPos+15,280,$FontSize, _('Account Name :') . ' ' . $BankDetailsRow['bankaccountname']);
					$YPos-=12;
					$LeftOvers = $pdf->addTextWrap($Left_Margin+5,$YPos+15,220,$FontSize,'Account No.     : '.$BankDetailsRow['bankaccountnumber']);
					$YPos-=12;
					$LeftOvers = $pdf->addTextWrap($Left_Margin+5,$YPos+15,280,$FontSize,_('Bank                 :') . ' ' . $BankDetailsRow['bankaddress']);
					$YPos-=12;
					$LeftOvers = $pdf->addTextWrap($Left_Margin+5,$YPos+15,220,$FontSize,_('Swift Code       :') . ' '.$BankDetailsRow['bankaccountcode']);
				}
