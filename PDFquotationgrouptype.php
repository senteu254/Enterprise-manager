...
/* END Brought from class.pdf.php constructor */
	...
	$quotation="AND f.groupid='". $_POST['StockCat'] ."'";
	}
		
			   $sql = "SELECT *,f.groupid,f.groupname
				        FROM tenders a
			...
			if (DB_error_no()!=0 OR DB_num_rows($result)==0){
				...
				$FontSize = 10;
				$PageNumber = 1;
				include('includes/PDFquotationgrouptypeheader.php');
				$FirstPage = False;
				$i=0;
				while ($myrow2=DB_fetch_array($result)){
				$FormatedrequiredDate = ConvertSQLDate($myrow2['requiredbydate']);
                $i++;
				 $LeftOvers = $pdf->addTextWrap($Left_Margin+0,$YPos,300,$FontSize,$i);
                 $LeftOvers = $pdf->addTextWrap($Left_Margin+50,$YPos,300,$FontSize,$myrow2['quotation']);
				 $LeftOvers = $pdf->addTextWrap($Left_Margin+230,$YPos,300,$FontSize,$FormatedrequiredDate);
				 $LeftOvers = $pdf->addTextWrap($Left_Margin+300,$YPos,300,$FontSize,$myrow2['suppname']);
				 $LeftOvers = $pdf->addTextWrap($Left_Margin+460,$YPos,300,$FontSize,$myrow2['address6']);
				 $LeftOvers = $pdf->addTextWrap($Left_Margin+544,$YPos,300,$FontSize,$myrow2['telephone']);
				 $LeftOvers = $pdf->addTextWrap($Left_Margin+624,$YPos,300,$FontSize,$myrow2['email']);
            ...
            /* Print a column vertical line */
			$pdf->line($Left_Margin+229, $TopOfColHeadings+16,$Left_Margin+229,$Bottom_Margin);
			/* Print a column vertical line */
			$pdf->line($Left_Margin+299, $TopOfColHeadings+16,$Left_Margin+299,$Bottom_Margin);
			/* Print a column vertical line */
			$pdf->line($Left_Margin+460, $TopOfColHeadings+16,$Left_Margin+460,$Bottom_Margin);
			/* Print a column vertical line */
			$pdf->line($Left_Margin+540, $TopOfColHeadings+16,$Left_Margin+540,$Bottom_Margin);
			/* Print a column vertical line */
			$pdf->line($Left_Margin+615, $TopOfColHeadings+16,$Left_Margin+615,$Bottom_Margin);
			/* Print a column vertical line */
		...
		//	$FontSize=10;
			...
			$YPos+=10;
			 ...
			 /*   
				...
				$pdf->addText($Page_Width-$Right_Margin-140, $YPos - ($line_height*2)+20,$FontSize,locale_number_format($commpayments,2),'right');
				$pdf->addText($Page_Width-$Right_Margin-220, $YPos - ($line_height*2)+20,$FontSize,locale_number_format($totalpayables,2),'right');
				$pdf->addText($Page_Width-$Right_Margin-279, $YPos - ($line_height*2)+20,$FontSize,locale_number_format($Totalcommitements,2),'right');
				$pdf->addText($Page_Width-$Right_Margin-380, $YPos - ($line_height*2)+20,$FontSize,locale_number_format($totalallocations,2),'right');
				$pdf->addText($Page_Width-$Right_Margin-425, $YPos - ($line_height*2)+20,$FontSize,locale_number_format($Mainsuppallocation,2),'right');
				$pdf->addText($Page_Width-$Right_Margin-532, $YPos - ($line_height*2)+20,$FontSize,locale_number_format($Mainallocation,2),'right');
				$pdf->addText($Page_Width-$Right_Margin-912, $YPos - ($line_height*2)+20,$FontSize,$MainTot,'right');
...
function PrintLinesToBottom () {
	...
	global $line_height;
			...
			$pdf->line($Left_Margin+49, $TopOfColHeadings+18,$Left_Margin+49,$Bottom_Margin+(0.0*$line_height));
            /* Print a column vertical line */
			$pdf->line($Left_Margin+229, $TopOfColHeadings+18,$Left_Margin+229,$Bottom_Margin+(0.0*$line_height));
			/* Print a column vertical line */
			$pdf->line($Left_Margin+299, $TopOfColHeadings+18,$Left_Margin+299,$Bottom_Margin+(0.0*$line_height));
			/* Print a column vertical line */
			$pdf->line($Left_Margin+460, $TopOfColHeadings+18,$Left_Margin+460,$Bottom_Margin+(0.0*$line_height));
			/* Print a column vertical line */
			$pdf->line($Left_Margin+540, $TopOfColHeadings+18,$Left_Margin+540,$Bottom_Margin+(0.0*$line_height));
			/* Print a column vertical line */
			$pdf->line($Left_Margin+615, $TopOfColHeadings+18,$Left_Margin+615,$Bottom_Margin+(0.0*$line_height));
			/* Print a column vertical line */
