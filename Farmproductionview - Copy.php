...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
	...
	if ($_POST['Keywords']) {
		...
		if ($_POST['StockCat'] == 'All') {
			$SQL = "SELECT  *,c.stockid as groupid, c.description as desk FROM farmproduction a
					...
					INNER JOIN farmitemsource e ON e.source_Id=b.source
	...
	if (DB_num_rows($SearchResult) == 0) {
		prnMsg(_('No production items were returned by this search please re-enter alternative criteria to try again or Enter production'), 'info');
	}
	unset($_POST['Search']);
}
...
if (isset($SearchResult) AND !isset($_POST['Select'])) {
	...
	if ($ListCount > 0) {
		...
		while (($myrow = DB_fetch_array($SearchResult))) {
		 $So= DB_query("SELECT sum((`farmproductionitems`.`unitcost` * `farmproductionitems`.`quantity`)) AS `SUM_TOTAL` FROM  farmproductionitems  GROUP BY'".$myrow['groupid']."','" . $myrow1['stockid'] ."'");      
		 ...
		 echo '<tr><td colspan="7"></td><th>Total</th><th>'.$myrow2['SUM_TOTAL'].'</th></tr>';
		...
		$SQL=" select
				  sum((`farmproductionitems`.`unitcost` * `farmproductionitems`.`quantity`)) 
				from
				  `farmproductionitems`
				group by
				  `farmproductionitems`.`groupid`";
...
/*
		...
		if($z >0){
		 //echo '<tr><td colspan="7"></td><th>Total</th><th>40000</th></tr>';
		 }
		//end of while loop
		echo '</table>
              </div>
