...
if (isset($_POST['EnterLines']) OR isset($_POST['AllowRePrint'])) {
	...
	$sql = "SELECT orderno FROM purchorders
				WHERE purchorders.orderno='" . $_POST['PONo'] . "'";
...
echo '</td></tr></table><br />';
   ...
   $ListCount = DB_num_rows($result_SuppSelect);
	...
	if (isset($result_SuppSelect)) {
		echo '<br /><table  style="width: 700px; cellpadding="3" class="selection">';
	...
	<tr>';
	################################################last lpo entered to system#############################################################
	$sql1 = "SELECT MAX(orderno) as lastLPO FROM  purchorders ORDER BY orderno";
		$result1 = DB_query($sql1);
		$Row = DB_fetch_array($result1);
	echo '<tr>';
	echo'<td>' . _('Last LPO No.') . ':</td>
	<td><input type="text" disabled="disabled"  name="lastLPO" class="number" size="16" value="'.$Row['lastLPO'].'" /></td>';
	echo'</tr>';
	####################################################end#################################################################################
	/********************************************************end*********************************************************************************/
	echo '<tr>
	<td>' . _('LPO No.') . ':</td>
	<td><input type="text" required="required" autofocus="autofocus" name="PONo" class="number" size="16" value="'.$_POST['PONo'].'" /></td>
	</tr>';
	}
	echo '<tr>
			<td>' . _('LPO Date') . ':</td>
