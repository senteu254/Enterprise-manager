...
if (isset($_POST['Submit'])) {
	...
	if (isset($_POST['SelectedMeasureID']) AND $_POST['SelectedMeasureID']!='' AND $InputError !=1) {
		...
		$sql = "SELECT count(*) FROM wocalibre
				WHERE id <> '" . $SelectedMeasureID ."'
				AND calibre ".LIKE." '" . $_POST['MeasureName'] . "'";
		$result = DB_query($sql);
		$myrow = DB_fetch_row($result);
		if ( $myrow[0] > 0 ) {
			$InputError = 1;
			prnMsg( _('The calibre can not be renamed because another with the same name already exist.'),'error');
		} else {
			...
			if ( DB_num_rows($result) != 0 ) {
				...
				$sql = array();
				$sql[] = "UPDATE wocalibre
					SET calibre='" . $_POST['MeasureName'] . "'
					WHERE calibre ".LIKE." '".$OldMeasureName."'";
				$sql[] = "UPDATE workorders
					SET calibre='" . $_POST['MeasureName'] . "'
					WHERE calibre ".LIKE." '" . $OldMeasureName . "'";
			} else {
				$InputError = 1;
				prnMsg( _('The calibre no longer exist.'),'error');
			}
		}
		$msg = _('Calibre changed');
	} elseif ($InputError !=1) {
		/*SelectedMeasureID is null cos no item selected on first time round so must be adding a record*/
		$sql = "SELECT count(*) FROM wocalibre
				WHERE calibre " .LIKE. " '".$_POST['MeasureName'] ."'";
		$result = DB_query($sql);
		$myrow = DB_fetch_row($result);
		if ( $myrow[0] > 0 ) {
			$InputError = 1;
			prnMsg( _('The calibre can not be created because another with the same name already exists.'),'error');
		} else {
			$sql = "INSERT INTO wocalibre (calibre )
					VALUES ('" . $_POST['MeasureName'] ."')";
		}
		$msg = _('New Calibre added');
	}
	...
	if ($InputError!=1){
		...
		if (is_array($sql)) {
			$result = DB_Txn_Begin();
			$tmpErr = _('Could not update Calibre');
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'stockmaster'
	...
	if ( DB_num_rows($result) == 0 ) {
		// This is probably the safest way there is
		prnMsg( _('Cannot delete this calibre because it no longer exist'),'warn');
	} else {
		$myrow = DB_fetch_row($result);
		$OldMeasureName = $myrow[0];
		$sql= "SELECT COUNT(*) FROM workorders WHERE calibre ".LIKE." '" . $OldMeasureName . "'";
		$result = DB_query($sql);
		$myrow = DB_fetch_row($result);
		if ($myrow[0]>0) {
			prnMsg( _('Cannot delete this calibre because workorder have been created using this calibre'),'warn');
			echo '<br />' . _('There are') . ' ' . $myrow[0] . ' ' . _('workorder that refer to this calibre') . '</font>';
		} else {
			$sql="DELETE FROM wocalibre WHERE calibre ".LIKE."'" . $OldMeasureName . "'";
			$result = DB_query($sql);
			prnMsg( $OldMeasureName . ' ' . _('calibre has been deleted') . '!','success');
		}
...
/* An unit of measure could be posted when one has been edited and is being updated
  ...
  or deletion of the records*/
	...
	$ErrMsg = _('Could not get calibre because');
	$result = DB_query($sql,$ErrMsg);

	echo '<table class="selection">
			<tr>
				<th class="ascending">' . _('Calibre') . '</th>
			</tr>';
	...
	while ($myrow = DB_fetch_row($result)) {
		...
		echo '<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?SelectedMeasureID=' . $myrow[0] . '&amp;delete=1" onclick="return confirm(\'' . _('Are you sure you wish to delete this Calibre?') . '\');">' . _('Delete')  . '</a></td>';
		echo '</tr>';
...
if (isset($SelectedMeasureID)) {
	echo '<div class="centre">
			<a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">' . _('Review Calibre') . '</a>
		</div>';
}
...
if (! isset($_GET['delete'])) {
	...
	if (isset($SelectedMeasureID)) {
		...
		$result = DB_query($sql);
		if ( DB_num_rows($result) == 0 ) {
			prnMsg( _('Could not retrieve the requested calibre, please try again.'),'warn');
			unset($SelectedMeasureID);
		} else {
			$myrow = DB_fetch_array($result);

			$_POST['MeasureID'] = $myrow['id'];
			$_POST['MeasureName']  = $myrow['calibre'];
