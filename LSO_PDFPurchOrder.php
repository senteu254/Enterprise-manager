...
if (!isset($_GET['OrderNo']) AND !isset($_POST['OrderNo'])) {
	$Title = _('Select a Service Order');
	include('includes/header.inc');
	echo '<div class="centre"><br /><br /><br />';
	prnMsg(_('Select a Service Order Number to Print before calling this page'), 'error');
	echo '<br />
				<br />
				<br />
				<table class="table_index">
					<tr><td class="menu_group_item">
						<li><a href="' . $RootPath . '/LSO_SelectOSPurchOrder.php">' . _('Outstanding Service Orders') . '</a></li>
						<li><a href="' . $RootPath . '/LSO_SelectPurchOrder.php">' . _('Service Order Inquiry') . '</a></li>
						</td>
					</tr></table>
				</div>
				<br />
...
$Title = _('Print Service Order Number') . ' ' . $OrderNo;
...
if (isset($OrderNo) AND $OrderNo != '' AND $OrderNo > 0 AND $OrderNo != 'Preview') {
	...
	$ErrMsg = _('There was a problem retrieving the Service order header details for Order Number') . ' ' . $OrderNo . ' ' . _('from the database');
	$sql = "SELECT	lsorders.supplierno,
					suppliers.suppname,
					suppliers.address1,
					suppliers.address2,
					suppliers.address3,
					...
					lsorders.paymentterms,
					suppliers.currcode,
					lsorders.status,
					lsorders.stat_comment,
				...
				INNER JOIN www_users
					ON lsorders.initiator=www_users.userid
				...
				WHERE lsorders.orderno='" . $OrderNo . "'";
	...
	if (DB_num_rows($result) == 0) {
		...
		include('includes/header.inc');
		echo '<div class="centre"><br /><br /><br />';
		prnMsg(_('Unable to Locate Service Order Number') . ' : ' . $OrderNo . ' ', 'error');
		echo '<br />
			<br />
			<br />
			<table class="table_index">
		...
		if ($POHeader['status'] != 'Authorised' AND $POHeader['status'] != 'Printed') {
			include('includes/header.inc');
			prnMsg(_('Service orders can only be printed once they have been authorised') . '. ' . _('This order is currently at a status of') . ' ' . _($POHeader['status']), 'warn');
			include('includes/footer.inc');
			exit;
		}
		...
		if ($ViewingOnly == 0) {
			if ($POHeader['allowprint'] == 0) {
				$Title = _('Service Order Already Printed');
				include('includes/header.inc');
				echo '<p>';
				prnMsg(_('Service Order Number') . ' ' . $OrderNo . ' ' . _('has previously been printed') . '. ' . _('It was printed on') . ' ' . ConvertSQLDate($POHeader['dateprinted']) . '<br />' . _('To re-print the order it must be modified to allow a reprint') . '<br />' . _('This check is there to ensure that duplicate purchase orders are not sent to the supplier resulting in several deliveries of the same supplies'), 'warn');

				echo '<div class="centre">
 					<li><a href="' . $RootPath . '/LSO_PDFPurchOrder.php?OrderNo=' . $OrderNo . '&ViewingOnly=1">' . _('Print This Order as a Copy') . '</a>
 					<li><a href="' . $RootPath . '/LSO_Header.php?ModifyOrderNumber=' . $OrderNo . '">' . _('Modify the order to allow a real reprint') . '</a>
					<li><a href="' . $RootPath . '/LSO_SelectPurchOrder.php">' . _('Select another order') . '</a>
...
if (isset($MakePDFThenDisplayIt) or isset($MakePDFThenEmailIt)) {
	...
	$pdf->addInfo('Title', _('Service Order'));
	$pdf->addInfo('Subject', _('Local Service Order Number') . ' ' . $OrderNo);
	...
	if ($OrderNo != 'Preview') { // It is a real order
		...
		$sql = "SELECT a.itemcode,
						...
						a.suppliers_partno,
						b.description_Id 	
				FROM lsorderdetails a
				LEFT JOIN farmdescriptions b ON a.itemcode= b.description_Id 	
	...
	if ($OrderNo == 'Preview' or DB_num_rows($result) > 0) {
		...
		include('includes/LSO_PDFServiceOrderPageHeader.inc');
		...
		while ((isset($OrderNo) AND $OrderNo == 'Preview') OR (isset($result) AND !is_bool($result) AND $POLine = DB_fetch_array($result))) {
			...
			$DisplayQty = locale_number_format($POLine['quantityord']);
			if ($_POST['ShowAmounts'] == 'Yes') {
				$DisplayPrice = locale_number_format($POLine['unitprice'],2);
			} else {
				$DisplayPrice = '----';
			}
	...
	if ($ViewingOnly == 0 AND $Success == 1) {
		...
		$sql = "UPDATE lsorders	SET	allowprint =  0,
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$SQL="SELECT requestid FROM irq_request
					WHERE requestid='".$POHeader['requisitionno']."'";
					$Result=DB_query($SQL);			
		if(DB_num_rows($Result)>0){
...
/* There was enough info to either print or email the purchase order */
	...
	if ($_POST['PrintOrEmail'] == 'Email') {
		...
		$SQL = "SELECT suppliercontacts.contact,
				...
				FROM suppliercontacts INNER JOIN lsorders
				ON suppliercontacts.supplierid=lsorders.supplierno
				WHERE lsorders.orderno='" . $OrderNo . "'";
