...
if (isset($_POST['submit'])) {
	...
	if (!is_numeric(filter_number_format($_POST['LPrice'])) OR $_POST['LPrice']=='') {
		$InputError = 1;
		prnMsg( _('The price entered must be numeric'),'error');
	}
	...
	if (isset($_POST['OldTypeAbbrev']) AND isset($_POST['OldCurrAbrev']) AND mb_strlen($Item)>1 AND $InputError !=1) {
		...
		$sql = "UPDATE prices SET
					...
					lprice='" . filter_number_format($_POST['LPrice']) . "',
	...
	/*Selected price is null cos no item selected on first time round so must be adding a	record must be submitting new entries in the new price form */
		...
		$sql = "INSERT INTO prices (stockid,
							...
							VALUES ('" . $Item . "',
								...
								'" . filter_number_format($_POST['LPrice']) . "')";
...
if (DB_num_rows($result) > 0) {
	...
	echo '<tbody><tr><th class="ascending">' . _('Currency') . '</th>
				...
				<th class="ascending">' . _('L.Price') . '</th>
	...
	while ($myrow = DB_fetch_array($result)) {
		...
		echo   '<td>' . $CurrencyName[$myrow['currabrev']] . '</td>
				...
				<td class="number">' . locale_number_format($myrow['lprice'], $myrow['currdecimalplaces']+2) . '</td>
		...
		if (in_array(5, $_SESSION['AllowedPageSecurityTokens'])) {
			echo '<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Item=' . $myrow['stockid'] . '&amp;TypeAbbrev=' . $myrow['typeabbrev'] . '&amp;CurrAbrev=' . $myrow['currabrev'] . '&amp;Price=' . locale_number_format($myrow['price'],$myrow['currdecimalplaces']) . '&amp;LPrice=' . locale_number_format($myrow['lprice'],$myrow['currdecimalplaces']) . '&amp;StartDate=' . $myrow['startdate'] . '&amp;EndDate=' . $myrow['enddate'] . '&amp;Edit=1">' . _('Edit') . '</a></td>
...
echo '<tr><td>' . _('Least Price') . ':</td>
          <td>
          <input type="text" class="number" required="required" name="LPrice" size="12" maxlength="11" value="';
