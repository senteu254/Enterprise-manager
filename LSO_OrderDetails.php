...
if (isset($_GET['OrderNo'])) {
	$Title = _('Reviewing  Local Service Order Number').' ' . $_GET['OrderNo'];
	$_GET['OrderNo']=(int)$_GET['OrderNo'];
} else {
	$Title = _('Reviewing A Local Service Order');
}
include('includes/header.inc');

if (isset($_GET['FromGRNNo'])){

	$SQL= "SELECT lsorderdetails.orderno
			FROM lsorderdetails INNER JOIN grns
			ON lsorderdetails.podetailitem=lsogrns.podetailitem
			WHERE lsogrns.grnno='" . $_GET['FromGRNNo'] ."'";
...
if (!isset($_GET['OrderNo'])) {
	...
	echo '<br /><br />';
	prnMsg( _('This page must be called with a service order number to review'), 'error');

	echo '<table class="table_index">
		<tr><td class="menu_group_item">
                <li><a href="'. $RootPath . '/LSO_SelectPurchOrder.php">' . _('Outstanding Service Orders') . '</a></li>
		</td></tr></table>';
...
$OrderHeaderSQL = "SELECT lsorders.*,
					...
					ON locations.loccode=lsorders.intostocklocation
					...
					WHERE lsorders.orderno = '" . $_GET['OrderNo'] ."'";
...
if (DB_num_rows($GetOrdHdrResult)!=1) {
	...
	if (DB_num_rows($GetOrdHdrResult) == 0){
		prnMsg ( _('Unable to locate this LSO Number') . ' '. $_GET['OrderNo'] . '. ' . _('Please look up another one') . '. ' . _('The order requested could not be retrieved') . ' - ' . _('the SQL returned either 0 or several Service  orders'), 'error');
	} else {
		prnMsg ( _('The order requested could not be retrieved') . ' - ' . _('the SQL returned either several Service orders'), 'error');
	}
        echo '<table class="table_index">
                <tr>
...
echo '<p class="page_title_text"><img src="'.$RootPath.'/css/'.$Theme.'/images/supplier.png" title="' .
		_('Service Order') . '" alt="" />' . ' ' . $Title . '</p>';
echo '<a href="' . $RootPath . '/LSO_SelectPurchOrder.php">' . _('Outstanding Setrvice Orders') . '</a>';
...
if ($myrow['dateprinted'] == ''){
	...
	echo '[<a href="LSO_PDFPurchOrder.php?OrderNo='. $_GET['OrderNo'] .'">' .  _('Print')  . '</a>]';
} else {
	echo _('Printed on').' '. ConvertSQLDate($myrow['dateprinted']). '&nbsp; &nbsp;';
	echo '[<a href="LSO_PDFPurchOrder.php?OrderNo='. $_GET['OrderNo'] .'">' .  _('Print a Copy')  . '</a>]';
}
...
$ErrMsg = _('The line items of the Servei order could not be retrieved');
$LineItemsSQL = "SELECT lsorderdetails.*,
						stockmaster.decimalplaces
				FROM lsorderdetails
				LEFT JOIN stockmaster
				ON lsorderdetails.itemcode=stockmaster.stockid
				WHERE lsorderdetails.orderno = '" . $_GET['OrderNo'] ."'
