...
if(empty($source_Id)) {
	prnMsg(_('source ID cannot be Empty '),'error');
	$InputError = 1;
	}elseif(empty($stockID)) {
	prnMsg(_('Stock Id Number cannot be Empty '),'error');
	$InputError = 1;
	}elseif(empty($source_Name)) {
	prnMsg(_('Production Source Name cannot be Empty '),'error');
	$InputError = 1;
	}if ($InputError == 0){
	DB_query("INSERT INTO farmitemsource(source_Id,stockid,source_Name) 
					 VALUES ('$source_Id','$stockID','$source_Name')");
							 $InsResult = DB_query($sql,$ErrMsg,$DbgMsg,true);
								DB_Txn_Commit();
								if (DB_error_no() ==0) {
						prnMsg( _('New Source ') .' ' . '<a href="Farm_Description.php?description_Id=' . $description_Id . '">' . $description_Id . '</a> '. _(' has been added to      Production Service'),'success'); 
     }
     }
	 }
...
//echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/inventory.png" title="' . _('Inventory Items') . '" alt="" />' . ' ' . _('Item Source Maintanance') . '</p>';
...
if (DB_num_rows($result1) == 0) {
	echo '<p class="bad">' . _('Problem Report') . ':<br />' . _('There are no production currently in the system') . '</p>';
...
if (!isset($_POST['Search']) AND (isset($_POST['Select']) OR isset($_SESSION['SelectedStockItem']))) {
 ...
 echo '</table></td>';
/********************************************************************************************************************************/
 echo'<table align="center" style="width:45%" class="selection table table-hover">';
...
echo'<input type="hidden" size="10" maxlength="60" value="'.$StockID.'" name="stockid" />';
	echo'<tr>
	<td style="font-size:10pt">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Source Id</td><td><input type="text" size="10" maxlength="60" placeholder="Source Id" name="source_Id" /><td><center></center>
	</td> 
	</tr>		
	
   <tr><td style="font-size:10pt">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Source Name</td><td><input type="text" size="40" placeholder="Source Name" maxlength="60" name="source_Name" /><td><td><center></center></td></tr>
...
echo '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Application=FA2&Ref=Farm_Serices" method="post">';
echo '<div>';
	...
	if ($_POST['Keywords']) {
		...
		if ($_POST['StockCat'] == 'All') {
			...
			$SQL = "SELECT stockmaster.stockid,
						...
						AND stockmaster.stockid='" . $_POST['StockCat'] . "'
						AND stockmaster.mbflag!='B'
					    AND stockmaster.mbflag!='A'
						AND stockmaster.mbflag!='M'
					    AND stockmaster.mbflag!='K'
		...
		if ($_POST['StockCat'] == 'All') {
			...
			$SQL = "SELECT stockmaster.stockid,
					...
					FROM stockmaster INNER JOIN locstock
					ON stockmaster.stockid=locstock.stockid
					WHERE stockmaster.stockid='" . $_POST['StockCat'] . "'
					AND stockmaster.mbflag!='B'
					    AND stockmaster.mbflag!='A'
						AND stockmaster.mbflag!='M'
					    AND stockmaster.mbflag!='K'
