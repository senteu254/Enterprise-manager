...
foreach ($_POST as $key => $value) {
	if (mb_strstr($key,'StockID')) {
		...
		if (filter_number_format($_POST['Quantity'.$Index])>0) {
			...
			$factor=$_POST['factor'.$Index];
			$_SESSION['FP']->AddLine($StockID, $ItemDescription, $NewItem_array[$StockID],$NewItem_array1[$StockID], $_POST['units'.$StockID], $Source,$DecimalPlaces,$cost,$factor);
		}
	}
...
if (isset($_POST['Submit'])) {
	...
	if ($InputError==0) {		
		$HeaderSQL="INSERT INTO farmproduction (Fid,
		                                        service_id,
												contractor,
												field,
												date)
										VALUES('$ProductionID',
											...
											'" . $_SESSION['FP']->Field . "',
											'" . FormatDateForSQL($_SESSION['FP']->prod_date) . "')";
...
if (isset($_GET['Edit'])) {
	echo '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?Application=FA2&Ref=default&Link=kofc_service" method="post">';
    echo '<div>';
	...
	echo'<tr>
			...
			<td><input type="text" class="number" name="Quantity" value="' . locale_number_format($_SESSION['FP']->LineItems[$_GET['Edit']]->AreaCovered, $_SESSION['FP']->LineItems[$_GET['Edit']]->DecimalPlaces) . '" /></td>
		</tr>';
...
if($_SESSION['FP']->SP =='CO'){
 ...
 if (!isset($_SESSION['FP']->prod_date)) {
		$_SESSION['FP']->prod_date = date('Y-m-d');
	}else if (isset($_SESSION['FP']->prod_date)) {
	$_SESSION['FP']->prod_date=	$_SESSION['FP']->prod_date;
	}
 echo '<tr>
<td>' . _('Production Date') . ':</td>
<td><input type="text" required="required" autofocus="autofocus" class="date" alt="' . $_SESSION['DefaultDateFormat'] . '" name="prod_date" size="11" value="' . $_SESSION['FP']->prod_date . '" /></td>
</tr>';
 ////////////////////////////////////////////////////////////////////
  echo '<tr>
    <td>Field Name</td>';
  ...
  /* echo '<tr>
    <td><input type="checkbox" style="width:15px; height:15px;" class="form-control" name="tax" '.((isset($_SESSION['FP']->litres_acre) && $_SESSION['FP']->litres_acre==1)? 'checked="true"' : '').' value="1">Litres Per Acre</td>';
  echo '<td><input type="text" autofocus="autofocus" name="litres_acre" size="20" value="' . locale_number_format($_SESSION['FP']->litres_acre,2) . '" /></td>';
   // echo '<td><input type="text" autofocus="autofocus" name="litres_acre" size="11" value="' . locale_number_format($_SESSION['FP']->litres_acre,2) . '" /></td>
   echo '</tr>';*/
...
echo '<br />
	...
	<tr>
		<th colspan="9"><h6>' . _('Details of Farm Service') . '</h6></th>
	</tr>
...
$i++;
	...
	echo '<td>' . $i  . '</td>
			...
			<td>' . $LineItems->UOM . '</td>
			<td>' . locale_number_format($LineItems->cost*($LineItems->Quantity), $_SESSION['StandardCostDecimalPlaces']) . '</td>
			<td><a href="'. htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?Edit='.$LineItems->LineNumber.'">' . _('Edit') . '</a>
...
?><table style="widows:30%;" class="selection table table-hover">
<?php 
	echo '<tr><td>' . _('Select type of service') . ':';
...
echo '<td></h3>' . _('Enter partial') . ' <b>' . _('Stock Code') . '</b>:</td>';
...
if (isset($SearchResult)) {
	...
	echo '<br />
		...
		<br />
		<form action="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?Application=FA2&Ref=default&Link=kofc_service" method="post" id="orderform">
	...
	while ($myrow=DB_fetch_array($SearchResult)) {
		...
		echo '<td>' . $myrow['description_Id'] . '</td>
				...
				<td><select name="Source'.$i.'">';
                           $SourceResult= DB_query("SELECT a.source_Id,
														   a.source_Name,
														   b.stockid
														   FROM  farmitemsource a,stockmaster b
														   WHERE a.stockid=b.stockid 
														   AND b.stockid='". $_POST['StockCat'] ."'");
							while ($myrow2 = DB_fetch_array($SourceResult)){
							echo '<option value="' . $myrow2['source_Id'] . '">' . $myrow2['source_Name'] . '</option>';
							}
					  echo'</select></td>
				<td>' . $myrow['units'] . '</td>
				<td><input class="number" ' . ($i==0 ? 'autofocus="autofocus"':'') . ' tabindex="'.($j+7).'" type="text" size="6" name="cost'.$i.'" value="0" />
