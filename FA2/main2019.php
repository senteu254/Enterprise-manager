...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
	...
	if ($_POST['Keywords']) {
		...
		if ($_POST['StockCat'] == 'All') {
			$SQL = "SELECT  *,a.service_Id,c.stockid as groupid,b.units, c.description as desk FROM farmproduction a
	...
	if (DB_num_rows($SearchResult) == 0) {
		prnMsg(_('No Farm Production were returned by this search please re-enter alternative criteria to try again'), 'info');
	}
	unset($_POST['Search']);
}
...
if (isset($SearchResult) AND !isset($_POST['Select'])) {
	...
	if ($ListCount > 0) {
		...
		?>
		<table id="myTable2"  style="width:100%; font-size:12px;" class="selection table table-hover">
		<thead>		
		<?php
		$TableHeader = '<tr>
		...
		while ($myrow = DB_fetch_array($SearchResult)) {    
			...
			$i++;
			if($myrow['service_Id']= 'KF'){
			$Totalcost=$myrow['unitcost']*$myrow['quantity'];
			}elseif($myrow['service_Id']= 'CO'){
...
/*
		...
		?>
            ...
            <?php
		echo'<br />';
			 echo'<a href="PDFFarmproduction.php?datef='.$DateFrom.'&datet='.$DateTo.'" target="_blank"><button type="button" class="btn btn-default"><i class="fa fa-print"></i> Print</button></a>';
