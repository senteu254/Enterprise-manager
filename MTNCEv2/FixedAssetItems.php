	...
	if ($InputError !=1){
		...
		if ($_POST['submit']==_('Update')) { /*so its an existing one */
			...
			if (DB_error_no() ==0) {
				...
				$TransNo = GetNextTransNo( 41, $db); /* transaction type is asset category change */
				
				$resultx = DB_query("SELECT costact,
										accumdepnact
								FROM fixedassetcategories
								WHERE categoryid='" . $_POST['AssetCategoryID'] . "'");
				$OldDetails = DB_fetch_array($resultx);

				//credit cost for the old category
				$CreditorsAct = $_SESSION['CompanyRecord']['creditorsact'];
				$SQL = "INSERT INTO gltrans (type,
											typeno,
											trandate,
											periodno,
							...
							VALUES ('41',
								...
								'" . $PeriodNo . "',
								'" . $CreditorsAct . "',
								'". _('Asset Addition') . ' - ' . $NewAssetID . ": ".$_POST['Description']."',
								'" . -$_POST['cost']. "'
								)";
				$ErrMsg = _('Cannot insert a GL entry for the change of asset category because');
				$DbgMsg = _('The SQL that failed to insert the cost GL Trans record was');
...
echo '<form id="AssetForm" enctype="multipart/form-data" method="post" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Application=FA&Link=NewAsset">
      <div>';
...
/*If the page was called without $AssetID passed to page then assume a new asset is to be entered other wise the form showing the fields with the existing entries against the asset will show for editing with a hidden AssetID field. New is set to flag that the page may have called itself and still be entering a new asset, in which case the page needs to know not to go looking up details for an existing asset*/
	...
	$_POST['datepurchased']  = ConvertSQLDate($AssetRow['datepurchased']);
...
echo '<tr>
		<td>' . _('Asset Code') . ':</td>
		<td><input ' . (in_array('mcno',$Errors) ?  'class="inputerror"' : '' ) .' type="text" required="required" title="' . _('Enter the Asset Code. Up to 20 characters can be used.') . '" name="mcno" size="22" maxlength="20" value="' . $mcno . '" /></td>
	</tr>';
...
echo '<tr>
		...
		<td><textarea ' . (in_array('LongDescription',$Errors) ?  'class="texterror"' : '' ) .'  name="LongDescription" required="required" title="' . _('Enter the lond description of the asset including specs etc. Up to 255 characters are allowed.') . '" cols="52" rows="2">' . stripslashes($LongDescription) . '</textarea></td>
	</tr>';
...
echo '</select><a href="'. $RootPath . '/index.php?Application=FA&Link=AssetCategories">' . ' ' . _('Add or Modify Asset Categories') . '</a></td></tr>';
...
echo '</select>
	<a href="'. $RootPath . '/index.php?Application=FA&Link=AssetLocation">' . ' ' . _('Add Asset Location') . '</a></td>
	</tr>';
	if (isset($AssetRow) AND ($AssetRow['datepurchased']!='0000-00-00' AND $AssetRow['datepurchased']!='')){
	echo '<tr>
			<td>' . _('Cost') . ':</td>
			<td>' . locale_number_format($AssetRow['cost'],2) . '</td>
		</tr>';
	echo '<tr>
...
echo '<tr>
		<td>' . _('Cost') . ':</td>
		<td><input required type="text" name="cost" size="32" maxlength="30" value="' . $_POST['cost'] . '" /></td>
	</tr>
	
	<tr>
		<td>' . _('Date of Purchase') . ':</td>
		<td><input required type="text" class="date" alt="'.$_SESSION['DefaultDateFormat'].'" autocomplete="off" name="datepurchased" size="32" maxlength="30" value="' . $_POST['datepurchased'] . '" /></td>
	</tr>';	
	}
echo '<tr>
		<td>' . _('Serial Number') . ':</td>
		<td><input ' . (in_array('SerialNo',$Errors) ?  'class="inputerror"' : '' ) .' required type="text" name="SerialNo" size="32" maxlength="30" value="' . $_POST['SerialNo'] . '" /></td>
	</tr>
	<tr>
