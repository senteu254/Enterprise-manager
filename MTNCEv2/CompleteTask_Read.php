...
$sqlz = "UPDATE fixedassettasks SET user='" . $_SESSION['UserID'].' - '.$_SESSION['UsersRealName'] . "',
										userremarks='" . $_POST['remarks'] . "',
										userremarksdate='" . date('Y-m-d H:i:s') . "',
										mcdowntime='" . $_POST['downtime'] . "',
	...
	$_SESSION['msg'] = 'Machine handed over to user successfully';
}			
...
<div class="mailbox-read-message">
		...
		echo '</table>';
		$requesting_officer = $rows['requesting_officer'];
			?>
              </div>
				<?php
			...
			<?php
			$sqlx= "SELECT *, TIME_TO_SEC(TIMEDIFF(enddate,startdate)) as diff, (SELECT COUNT(initiator) FROM irq_request WHERE initiator='".$_SESSION['UserID']."' AND requestid='" . $LID . "') as userauthorized FROM fixedassettasks
							WHERE requestid='" . $LID . "'";
			$resx = DB_query($sqlx);
		...
		echo '<div id="div" class="alert alert-warning alert-dismissible">
                <h4><i class="icon fa fa-warning"></i> Alert!</h4>
                Machine is not yet handed over to user. Please contact '.$requesting_officer.' to accept this machine.
              </div>';
		}else{
		echo '<tr><td colspan="4">';
		echo '<strong style="text-decoration:underline;">HANDING OVER TO USER</strong>';
