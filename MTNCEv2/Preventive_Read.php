...
$sqlz = "DELETE FROM fixedassettasks WHERE taskid='".$_GET['Delete']."'";
	$ErrMsg = _('The fixed asset task details cannot be inserted because');
	$Resultz=DB_query($sqlz,$ErrMsg);
...
<div class="mailbox-read-message">
		...
		echo '</table>';
			...
			$results = DB_query($sqls);
					...
					if($roww['accepted']!=1){
					echo '<span class="pull-right"><a href="index.php?Application=FA&Link=PreventiveRead&VID='.$LID.'&amp;Edit='.$roww['taskid'].'"><i style="color:blue" class="fa fa-edit"></i> </a> || <a href="index.php?Application=FA&Link=PreventiveRead&VID='.$LID.'&amp;Delete='.$roww['taskid'].'"><i style="color:red" class="fa fa-trash"></i></a></span>';
					}
		...
		<div class="panel-heading">Task Register</div>
			...
			<form enctype="multipart/form-data" action="<?php echo 'index.php?Application=FA&Link=PreventiveRead&VID='.$LID.''; ?>" onsubmit="return document.getElementById('loadingbackground').style.display = 'block';" method="post" class="form-horizontal">
			...
			<?php echo '<input type="hidden" name="month" value="' . $asset . '" />'; ?>
