...
$sql="SELECT fixedassets.assetid,
			...
			SUM(CASE WHEN fixedassettrans.fixedassettranstype='cost' THEN fixedassettrans.amount ELSE fixedassets.cost END) AS costtotal,
...
echo '<table class="table" style="font-size:11px;width:100%;">';
$Heading = '<tr>
				<th>' . _('Asset ID') . '</th>
				<th>' . _('Description') . '</th>
...
if (isset($_POST['CommitDepreciation']) AND $InputError==false){
	...
	echo '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Application=FA&Link=Journal" method="post" id="form">';
    echo '<div>';
