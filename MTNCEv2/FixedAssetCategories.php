...
or deletion of the records*/
	...
	while ($myrow = DB_fetch_array($result)) {
		...
		printf('<td>%s</td>
					...
					$myrow['disposalact'],
					$myrow['accumdepnact'],
					htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Application=FA&Link=AssetCategories&',
					$myrow['categoryid'],
					htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Application=FA&Link=AssetCategories&',
					$myrow['categoryid']);
	}
...
if (isset($SelectedCategory)) {
	echo '<br /><div class="centre"><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Application=FA&Link=AssetCategories">' ._('Show All Fixed Asset Categories') . '</a></div>';
}

echo '<form id="CategoryForm" method="post" action="">';
echo '<div>';
...
if (isset($SelectedCategory) and !isset($_POST['submit'])) {
	...
	echo '<table class="table">
			<tr>
				<td>' . _('Category Code') . ':</td>
				<td><input type="text" name="CategoryID" required="required" title="' . _('Enter the asset category code. Up to 6 alpha-numeric characters are allowed') . '" size="7" maxlength="6" value="' . $_POST['CategoryID'] . '" /></td>
			</tr>';
}
