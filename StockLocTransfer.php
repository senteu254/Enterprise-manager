...
if(isset($_POST['Submit']) AND $InputError==False){
	...
	if (isset($_SESSION['RequestID'])) {
					...
					$Result=DB_query($SQL);
				$sql="SELECT level FROM irq_authorize_state WHERE requisitionid='" . $_SESSION['RequestID'] . "'";
				$result=DB_query($sql);
		...
		$insert = "UPDATE irq_request SET closed='1' WHERE requestid=". $_SESSION['RequestID'] ."";	
...
unset($_SESSION['Locode']);
	...
	if(isset($_POST['LinesCounter'])){
		...
		for ($i=0;$i < $_POST['LinesCounter'];$i++){
			...
			echo '<tr>
					<td><input type="text" name="StockID' . $j .'" size="21"  maxlength="20" value="' . str_replace("'"," ",$_POST['StockID' . $i]) . '" /></td>
	...
	while($j < $z) {
		...
		echo '<tr>
				<td><input type="text" name="StockID' . $j .'" ' . ($j==0 OR $j==$z-9 ? 'autofocus="autofocus"' : '') . ' size="21"  maxlength="20" value="' . str_replace("'","",$_POST['StockID' . $j]) . '" /></td>
