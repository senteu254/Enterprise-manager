...
If ((isset($_POST['PrintPDF']))
	...
	$sql = "SELECT suppliers.supplierid,
			...
			AND trandate <='" . FormatDateForSQL($_POST['PaymentDatet']) . "'
...
/*then print the report */
	...
	$pdf->addInfo('Subject',_('Remittance Advice') . ' - ' . _('suppliers from') . ' ' . $_POST['FromCriteria'] . ' ' . _('to') . ' ' . $_POST['ToCriteria'] . ' ' . _('and Paid On') . ' ' .  $_POST['PaymentDate']. ' ' . _('to') . ' '. $_POST['PaymentDatet']);
	...
	if (!isset($_POST['PaymentDatet'])){
		$DefaultDate = Date($_SESSION['DefaultDateFormat'], Mktime(0,0,0,Date('m')+1,0 ,Date('y')));
	} else {
		$DefaultDate = $_POST['PaymentDatet'];
	}

	echo '<tr>
			<td>' . _('Date Of Payment From') . ':</td>
			<td><input type="text" class="date" alt="'.$_SESSION['DefaultDateFormat'].'" name="PaymentDate" maxlength="11" size="12" value="' . $DefaultDate . '" /></td>
		</tr>';
		
		echo '<tr>
			<td>' . _('Date Of Payment To') . ':</td>
			<td><input type="text" class="date" alt="'.$_SESSION['DefaultDateFormat'].'" name="PaymentDatet" maxlength="11" size="12" value="' . $DefaultDate . '" /></td>
		</tr>';
