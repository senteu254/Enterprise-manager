...
If (isset($_POST['PrintPDF'])) {
 ...
 if($myrow['controlled']==1){
		  $sql = "SELECT serialno,
			...
			WHERE stockserialitems.loccode='" . $myrow['loccode'] . "'
...
$LocStockResult = DB_query($sql, $ErrMsg);
			...
			// and False to set to transparent
				...
				$pdf->addTextWrap(450,$YPos,50,$FontSize,locale_number_format($myrow['quantity'],
				                                    $myrow['decimalplaces']),'right',0);
				$pdf->addTextWrap(510,$YPos,50,$FontSize,locale_number_format($myrow['reorderlevel'],
				                                    $myrow['decimalplaces']),'right',0);
				if($myrow['controlled']==1){
				while ($myr=DB_fetch_array($LocStockResult)) {
				$pdf->addTextWrap(350,$YPos,50,$FontSize,$myr['serialno'],'right',0);
				$pdf->addTextWrap(395,$YPos,50,$FontSize,locale_number_format($myr['quantity'],$myrow['decimalplaces']),'right',0);
				 $YPos -=($line_height);
...
/*Print out the grand totals */
	...
	$pdf->OutputI($_SESSION['DatabaseName'] . '_Inventory_Quantities_' . Date('Y-m-d') . '.pdf');
	$pdf->__destruct();
...
function PrintHeader(&$pdf,&$YPos,&$PageNumber,$Page_Height,$Top_Margin,$Left_Margin,
	...
	$pdf->addTextWrap(345,$YPos,50,$FontSize,_('Lot'), 'right');
	$pdf->addTextWrap(395,$YPos,50,$FontSize,_('Qty'), 'right');
	$pdf->addTextWrap(450,$YPos,50,$FontSize,_('Quantity'), 'right');
	$pdf->addTextWrap(480,$YPos,100,$FontSize,_('Reorder Level'), 'right');
	$YPos -=$line_height;
	//$pdf->addTextWrap(475,$YPos,50,$FontSize,_('Level'), 'right');
