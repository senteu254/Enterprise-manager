...
header('location:index.php');
							...
							INNER JOIN irq_documents f ON z.doc_id = f.doc_id
							INNER JOIN locations g ON a.loccode = g.loccode
							WHERE closed=1 AND z.Requesteddate >= DATE_SUB(NOW(), INTERVAL 1 MONTH) AND z.doc_id='".$_SESSION['Doc_Store']."' AND CASE WHEN d.userid ='HOD' THEN e.authoriser='".$_SESSION['UserID']."' WHEN d.userid ='ISSUE' THEN g.authoriser='".$_SESSION['UserID']."' ELSE d.userid='".$_SESSION['UserID']."' END GROUP BY z.requestid";
							$result= mysqli_query($conn,$query) or die (mysqli_error($conn));
							...
							INNER JOIN irq_documents f ON z.doc_id = f.doc_id
							INNER JOIN locations g ON a.loccode = g.loccode
							WHERE closed=1 AND z.Requesteddate >= DATE_SUB(NOW(), INTERVAL 1 MONTH) AND z.doc_id='".$_SESSION['Document_id']."' AND CASE WHEN d.userid ='HOD' THEN e.authoriser='".$_SESSION['UserID']."' WHEN d.userid ='ISSUE' THEN g.authoriser='".$_SESSION['UserID']."' ELSE d.userid='".$_SESSION['UserID']."' END GROUP BY z.requestid";
							$result= mysqli_query($conn,$query) or die (mysqli_error($conn));
							$cnum=mysqli_num_rows($result);
							}
							$Lang = $_SESSION['Lang'];
