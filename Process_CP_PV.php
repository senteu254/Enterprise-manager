...
$InputError = 0;
$pv=$_POST['pv'];
for($i=0; $i < count($_POST['pv']); $i++)
{
$sql = "UPDATE payment_voucher SET process_level=6 WHERE voucherid = '". $_POST['pv'][$i] . "'";

		$result = DB_query($sql);	
$SQL = "SELECT voucherno FROM  payment_voucher_approval where voucherno='".$_POST['pv'][$i]."' and process_level=6";
$resu=DB_query($SQL);
...
$sqlq = "INSERT INTO payment_voucher_approval (voucherno,
	...
	prnMsg( _('The selected Payment Voucher Has been  Authorized ready to be Paid'), 'success' );
	unset($_POST['approve']);
	}	
...
$sql = "UPDATE payment_voucher SET process_level=14, comment='$comment' WHERE voucherid = '". $_POST['PV'] . "'";
...
echo '<a href="' . $RootPath . '/Process_CP_PV.php">' . _('Back to Payment Voucher') . '</a>';
...
if ($myrow['process_level'] ==5) {
	...
	<input type="hidden" name="amts" value="' . $myrow['amount'] . '" />';
		...
		echo '<form method="post" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" id="form">
		   <a href="' . $RootPath . '/SuppliersPayments.php">' . _('Generate Report') . '</a>';
		echo'<input type="hidden" name="pv[]" value="' . $_GET['SelectedUser'] . '" />
			<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
		echo '</form>';
		}
...
echo '<input type="hidden" name="Submit" value="' . _('Update') . '" />';
	$sql = "SELECT voucherid,
					authorityref,
					datereq,
					label,
					...
					process_level,
					total
				FROM payment_voucher WHERE process_level >=5 AND process_level NOT BETWEEN 10 AND 13
...
echo'<input type="hidden" name="suppid" value="' . $myrow['supplierid'] . '" />
	<input type="hidden" name="amts" value="' . $myrow['amount'] . '" />';
		...
		printf('<td>%s</td>
					...
					<td>%s</td>
					'.$check.'
					<td><a href="%s&amp;SelectedUser=%s&amp;view=1">' . _('View') . '</a>'.($myrow['process_level']==6 ? '||<a href="%s'.($myrow['supplierid']=="" ? '' : '?SupplierID=%s').'">' . _('Make Payment') . '</a>' : '' ).'</td>
					</tr>',
					$myrow['voucherid'],
					$myrow['authorityref'],
					$myrow['label'],
					...
					$myrow['voucherid'],
					($myrow['process_level']==6 ? 'Payments.php' : ''),
					$myrow['supplierid'],
					htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?',
					$myrow['voucherid']);
