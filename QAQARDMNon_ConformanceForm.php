...
$sql = "UPDATE qanonconformingproducts SET process_level='" . 6 . "'
					WHERE id = '". $SelectedUser . "'";

		$result = DB_query($sql);
$sql = "INSERT INTO qanonconformingremarks (refid,remarks,approver,approvertitle,approvername)
					VALUE('". $SelectedUser ."','". $_POST['remarks'] ."',6,'QARDM REMARKS','". $_SESSION['UsersRealName'] ."')";

		$result = DB_query($sql);
		prnMsg( _('The selected Record has been for processed'), 'info' );
...
echo '<a href="' . $RootPath . '/QAQARDMNon_ConformanceForm.php">' . _('Back to Main Menu') . '</a>';
...
if (!isset($SelectedUser)) {
	$sql = "SELECT *
				FROM qanonconformingproducts where process_level >=5 ORDER BY id DESC";
	$result = DB_query($sql);
	...
	if ($myrow['date']=='') {
		...
		$report='<td><a href="%sid=%s">' . _('Print PDF') . '</a></td>';
		$status = '<td><b class="jj" style="background: #3399FF; font-size:12px;" id="">Processed</b></td>';
		}else{
		$report='';
