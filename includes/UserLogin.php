...
function userLogin($Name, $Password, $SysAdminEmail = '', $db) {
	...
	/* if not logged in */
		...
		if ($PasswordVerified) {
			...
			$_SESSION['CanViewWorkOrder'] = $myrow['canviewworkorder'];
			$_SESSION['CanEditFlow'] = $myrow['caneditflow'];
			$_SESSION['CanEditHR'] = $myrow['caneditHR'];
			...
			$sql = "UPDATE www_users SET lastvisitdate='". date('Y-m-d H:i:s') ."', OnlineStatus=1
							WHERE www_users.userid='" . $Name . "'";
			...
			$sql='SELECT GROUP_CONCAT(description) as des,
				 GROUP_CONCAT(activity) as act,
				 GROUP_CONCAT(maintenance_date) as mdate,
				email
...
foreach ($emailList as $email => $itemList)
{
 ...
 $EmailText.= _('Please make a requisition for those assets  ')  . "\n";
	 foreach($itemList as $itemInfo)
    {
		...
		$EmailText.= _('Asset : '.$itemInfo['des'].'') . "\n";
		$EmailText.= _('Activity : '.$itemInfo['act'] .'') . "\n";
		$EmailText.= _('Maintenance Date : '. $itemInfo['mdate'].'') . "\n";
		$EmailText .= "------\n\n";
