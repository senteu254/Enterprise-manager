...
$sql1 = "SELECT * FROM systypes a,debtortrans b
		WHERE a.typeid = '".$_POST['TransType']."'";
...
$FontSize = 10;
$pdf->addText($Page_Width-268, $YPos-12, $FontSize, _('Transaction Type: - '.$_POST['TransType'].''));
//$pdf->addText($Page_Width-180, $YPos-39, $FontSize, ConvertSQLDate($myrow['Requesteddate']));
	$pdf->addText($Page_Width-268, $YPos-26, $FontSize, _('Order Type - Customer '));
	//$pdf->addText($Page_Width-268, $YPos-39, $FontSize, _('Date '));
	$pdf->addText($Page_Width-268, $YPos-39, $FontSize, _('Date Range - '.ConvertSQLDate($SQL_FromDate).' to '.ConvertSQLDate($SQL_ToDate)));
	//$pdf->addText($Page_Width-268, $YPos-39, $FontSize, _('Transactions After- '.ConvertSQLDate($DateAfterCriteria).''));
	...
	$pdf->addText($Page_Width-268, $YPos-52, $FontSize, _('Customer Inquiry:Detail By Date'));
	$pdf->addText($Page_Width-180, $YPos-52, $FontSize, $myrow['year']);
	//$pdf->addText($Page_Width-268, $YPos-65, $FontSize, _('Balance:Admin'));
...
$YPos=$YPos+6;
$pdf->addText($Left_Margin+9, $YPos, $FontSize, _('Type'));
$pdf->addText($Left_Margin+80, $YPos, $FontSize, _('Number'));
$pdf->addText($Left_Margin+130, $YPos, $FontSize, _('Date') );
$pdf->addText($Left_Margin+210, $YPos, $FontSize, _('Customer Name') . '');
$pdf->addText($Left_Margin+420, $YPos, $FontSize, _('Refferencee'));
$pdf->addText($Left_Margin+560, $YPos, $FontSize, _('Order'));
$pdf->addText($Left_Margin+670, $YPos, $FontSize, _('Amount'));
