...
if (isset($_POST['Save'])) {
	...
	//$_SESSION['tender'.$identifier]->date=$_POST['date'];
...
if (isset($_POST['SelectedSupplier'])) {
	...
	if (mb_strlen($myrow['email'])>=0) {
		...
		prnMsg( _('The supplier must have an email set up or they cannot be part of a Quotation'), 'warn');
	}
	}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
if (isset($_SESSION['RFQStockID'])
	AND isset($_SESSION['RFQReq_No'])){

	foreach ($_SESSION['RFQStockID'] as $rqt) {
		$Quantity =$rqt['QTY'];
		$StockID =$rqt['SID'];
		$sql = "SELECT description,
						units,
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	echo'<td>' . _('Quotation Number') . '</td>
			<td><input required="required" type="text" name="quotationNo"  title="'._('The quotation should not be over 4 characters').'" size="31" maxlength="50" value="' . $_SESSION['tender'.$identifier]->quotationNo . '" /></td>';	
	...
	* the form loaded set up defaults */
	...
	echo '<tr>
			<td>' . _('Warehouse') . ':</td>
			<td><select name="loccode" onchange="ReloadForm(form1.LookupDeliveryAddress)">';
...
if (isset($_POST['SearchSupplier']) or isset($_POST['Go'])
	...
	if ($_POST['Partname'] == '' and $_POST['SupplierCode'] == '') {
		...
		if (mb_strlen($_POST['Partname']) > 0) {
			$_POST['Partname'] = mb_strtoupper($_POST['Partname']);
			//insert wildcard characters in spaces
			$SearchString = '%' . str_replace(' ', '%', $_POST['Partname']) . '%';
			$SQL = "SELECT supplierid,
							suppname,
							currcode,
							address1,
...
if (isset($_POST['Partname'])) {
	echo '<input type="text" name="Partname" value="' . $_POST['Partname'] . '" size="20" maxlength="25" />';
} else {
	echo '<input type="text" name="Partname" size="20" maxlength="25" />';
}
echo '</td>
		<td><b>' . _('OR') . '</b></td>
...
echo '</td></tr>
	</table><br /><div class="centre"><input type="submit" name="SearchSupplier" value="' . _('Search Now') . '" /></div>';
	echo '</div>
		</form>';
}
...
if (isset($_POST['Search'])){  /*ie seach for stock items */
	...
	echo '<div>';
	echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
	echo '<p class="page_title_text noPrint" ><img src="' . $RootPath . '/css/' . $Theme . '/images/supplier.png" title="' . _('Tenders') . '" alt="" />' . ' ' . _('Select items required on this Quotation') . '</p>';
