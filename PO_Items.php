 ...
 */
	if ($InputError!=1){
		...
		if ($_SESSION['ExistingOrder']==0){ /*its a new order to be inserted */
			...
			if(isset($_SESSION['Requisition_No']) && $_SESSION['Requisition_No']!=""){
			$sql="SELECT level FROM irq_authorize_state WHERE requisitionid='" . $_SESSION['Requisition_No'] . "'";
				$result=DB_query($sql);
				...
				$doc =1;
					$HSQL="INSERT INTO irq_authorize_state (requisitionid,
											...
											approver_comment)
										VALUES(
											'" . $_SESSION['Requisition_No'] . "',
											'" . $rowcount.$doc . "',
											'" . date('Y-m-d H:i:s') . "',
											'" . $_SESSION['UsersRealName']. "',
											'Purchase Order Raised (Ref:" . $_SESSION['PO'.$identifier]->OrderNo . ")')";
		$insert=DB_query($HSQL);
			DB_query("UPDATE irq_request SET order_ref='PO Raised (Ref:" . $_SESSION['PO'.$identifier]->OrderNo . ")', closed=1 WHERE requestid='". $_SESSION['Requisition_No'] ."'");
			...
			$sql = "INSERT INTO purchorders ( orderno,
							...
							VALUES(	'" . $_SESSION['PO'.$identifier]->OrderNo . "',
									...
									'" . $_SESSION['PO'.$identifier]->Ordertype . "',
...
if (count($_SESSION['PO'.$identifier]->LineItems)>0 and !isset($_GET['Edit'])){
	...
	echo '<tr><td colspan="9" class="number">' . _('TOTAL') . _(' including Tax') . '</td>
