...
if (!isset($StockID)) {
	...
	echo '</select></td>
		...
		}elseif($_GET['Quotations']=='Proformainvoice_Only'){
			$_POST['Quotations']='Proformainvoice_Only';
		}
		...
		if ($_POST['Quotations']=='Quotes_Only'){
			...
			echo '<option value="Proformainvoice_Only">' . _('Proforma invoice Only') . '</option>';
		} elseif ($_POST['Quotations']=='Proformainvoice_Only'){
			echo '<option value="Proformainvoice_Only">' . _('Proforma invoice Only') . '</option>';
			echo '<option value="Orders_Only">' . _('Orders Only')  . '</option>';
...
//end if stock search results to show
	...
	if (isset($_POST['Quotations']) AND $_POST['Quotations']=='Orders_Only'){
		$Quotations = 0;
	}elseif (isset($_POST['Quotations']) AND $_POST['Quotations']=='Proformainvoice_Only'){
		$Quotations = 2;
	}else {
		$Quotations =1;
	}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		if (isset($_POST['Quotations']) AND $_POST['Quotations']=='Orders_Only'){
		  //echo'<th colspan="10"><h4>' . _('Orders and  details') . '</h4></th>';
			$TableHeader = '<tr>
								<th>' . _('Modify') . '</th>
								<th>' . _('Invoice') . '</th>
		...
		} elseif (isset($_POST['Proforma']) AND $_POST['Proforma']=='Proformainvoice_Only'){
			$TableHeader = '<tr>
								...
								<th class="ascending" >' . _('Req Del Date') . '</th>
								<th class="ascending" >' . _('Delivery To') . '</th>
								<th class="ascending" >' . _('Profoma Invoice Total') . '<br />' . $_SESSION['CompanyRecord']['currencydefault'] . '</th>';
		    ...
		    // echo'<th colspan="10"><h4>' . _('Quatations and  details') . '</h4></th>';
			$TableHeader = '<tr>
								<th>' . _('Modify') . '</th>
								<th>' . _('Invoice') . '</th>
								<th>' . _('Print Quote') . '</th>
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		while ($myrow=DB_fetch_array($SalesOrdersResult)) {
			...
			$PrintSalesOrder = $RootPath . '/PrintCustOrder_SalesOrder.php?TransNo=' . $myrow['orderno'];

			if ($_SESSION['PackNoteFormat']==1){ /*Laser printed A4 default */
				$PrintDispatchNote = $RootPath . '/PrintCustOrder_PickingNote.php?TransNo=' . $myrow['orderno'];
			...
			$PrintProformaInvoicePortrait = $RootPath . '/PDFProformaInvoicePortrait.php?PINo=' . $myrow['orderno'];
			$PrintQuotationPortrait = $RootPath . '/PDFQuotationPortrait.php?QuotationNo=' . $myrow['orderno'];
			 ...
			 /*Check authority to create POs if user has authority then show the check boxes to select sales orders to place POs for otherwise don't provide this option */
				if ($AuthRow['cancreate']==0 AND $myrow['poplaced']==0){ //cancreate==0 if the user can create POs and not already placed
					printf('<td><a href="%s">%s</a></td>
						...
						<td><a target="_blank" href="%s">' . $PrintOrder . ' <img src="' .$RootPath.'/css/'.$Theme.'/images/pdf.png" title="' . _('Click for PDF') . '" alt="" /></a></td>
		...
		if ($_POST['Quotations']=='Orders_Only'){
			...
			echo '<b>' . _('Profoma invoice(s) Value in');
		} else {
