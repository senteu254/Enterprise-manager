<?php
/* $Id: SupplierQuotationCreate.php 4821 2012-01-20 13:48:53Z tim_schofield $ */
...
if (isset($_GET['New']) and $_SESSION['CanCreateTender']==0) {
	...
	prnMsg( _('You do not have authority to create supplier Quotation for this company.') . '<br />' .
...
if (isset($_GET['Edit']) and $_SESSION['CanCreateTender']==0) {
	...
	include('includes/header.inc');
	echo '<p class="page_title_text noPrint" ><img src="'.$RootPath.'/css/'.$Theme.'/images/supplier.png" title="' . $Title . '" alt="" />  '.$Title . '</p>';
	prnMsg( _('You do not have authority to amend supplier Quotation for this company.') . '<br />' .
			_('Please see your system administrator'), 'warn');
	include('includes/footer.inc');
	exit;
}
...
if (isset($_GET['Edit'])) {
	$Title = _('Edit an Existing Supplier Quotation Request');
	include('includes/header.inc');
	echo '<p class="page_title_text noPrint" ><img src="'.$RootPath.'/css/'.$Theme.'/images/supplier.png" title="' . _('Purchase Order Quoting') . '" alt="" />  '.$Title . '</p>';
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/********************************************************************************************************************************/	
...
$sql = "SELECT * FROM tenders a
	...
	$Title = _('Edit an Existing Supplier Quotation Request');
	include('includes/header.inc');
	echo '<p class="page_title_text noPrint" ><img src="' . $RootPath . '/css/' . $Theme . '/images/supplier.png" title="' . _('Quotation') . '" alt="" />' . $Title . '</p>';
} else {
	$Title = _('Create a New Supplier Quotation Request');
	include('includes/header.inc');
	echo '<p class="page_title_text noPrint" ><img src="' . $RootPath . '/css/' . $Theme . '/images/supplier.png" title="' . _('Quotation') . '" alt="" />' . $Title . '</p>';
}
...
if (isset($_POST['Save'])) {
	...
	$_SESSION['tender'.$identifier]->quotationNo=$_POST['quotationNo'];
	//$_SESSION['tender'.$identifier]->gen_pass=$gen_pass;
	//$_SESSION['tender'.$identifier]->gen_user=$gen_user;
	$_SESSION['tender'.$identifier]->save($db);//save quotation to system 
	$_SESSION['tender'.$identifier]->portalsave($db);//save quotation to portal 
	$_SESSION['tender'.$identifier]->EmailSuppliers();
	prnMsg( _('The Quotation has been successfully saved'), 'success');
	include('includes/footer.inc');
	exit;
}	
...
if (isset($_POST['SelectedSupplier'])) {
	    ######################################################username#############################################################
        $Uuppr_case = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
		$Ulower_case = "abcdefghijklmnopqrstuvwxyz";
		
		$Ugenerated_uppr_case = substr(str_shuffle($Uuppr_case),0,2);
		$Ugenerated_lower_case = substr(str_shuffle($Ulower_case),0,2);
		
		$user ="$Ugenerated_uppr_case$Ugenerated_lower_case";
		$gen_user = substr(str_shuffle($user),0,16);
    ############################################################################################################################
		...
		$pgenerated_uppr_case = substr(str_shuffle($uppr_case),0,2);
		$pgenerated_lower_case = substr(str_shuffle($lower_case),0,2);
		$pgenerated_numbers = substr(str_shuffle($numbers),0,2);
		$pgenerated_special = substr(str_shuffle($special),0,2);
		
		$pass ="$pgenerated_uppr_case$pgenerated_lower_case$pgenerated_numbers$pgenerated_special";
...
if (isset($_POST['Suppliers']) || isset($_POST['Items']) || isset($_POST['LookupDeliveryAddress'])) {
$_SESSION['tender'.$identifier]->RequiredByDate=$_POST['RequiredByDate'];
$_SESSION['tender'.$identifier]->quotationNo=$_POST['quotationNo'];
$_SESSION['tender'.$identifier]->Location=$_POST['LookupDeliveryAddress'];
}
...
if (!isset($_SESSION['tender'.$identifier])
	...
	echo '<table class="selection">';
	echo '<tr>
			<th colspan="4"><h3>' . _('Quotation header details') . '</h3></th>
		</tr>
		<tr>
			<td>' . _('Delivery Must Be Made Before') . '</td>
			<td><input type="text" class="date" required="required" alt="' . $_SESSION['DefaultDateFormat'] . '" name="RequiredByDate" autofocus="autofocus" size="11" value="' . $_SESSION['tender'.$identifier]->RequiredByDate . '" /></td>
		</tr>';

	echo'<td>' . _('Quotation Number') . '</td>';
			echo'<td><input required="required" type="text" name="quotationNo"  title="'._('The quotation should not be over 4 characters').'" size="31" maxlength="50" value="' . $_SESSION['tender'.$identifier]->quotationNo . '" /></td>';
	...
	* the form loaded set up defaults */
	...
	echo '<tr>
			<th colspan="4"><h3>' . _('Suppliers To Send Quotation') . '</h3></th>
		</tr>';
	echo '<tr>
	...
	echo '<tr>
			<th colspan="6"><h3>' . _('Items in Quotation') . '</h3></th>
		</tr>
		<tr>
	...
	if ($_SESSION['tender'.$identifier]->LinesOnTender > 0
		...
		echo '<input type="submit" name="Close" value="' . _('Close This Quotation') . '" />';
	}
	echo '</div>
		<br />';
	if ($_SESSION['tender'.$identifier]->LinesOnTender > 0
		...
		echo '<div class="centre">
				<input type="submit" name="Save" value="' . _('Save Quotation') . '" />
			</div>';
	}
	echo '</div>
		</form>';
...
if (isset($_POST['Search'])){  /*ie seach for stock items */
	...
	echo '<div>';
	echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
	echo '<p class="page_title_text noPrint" ><img src="' . $RootPath . '/css/' . $Theme . '/images/supplier.png" title="' . _('Quotation') . '" alt="" />' . ' ' . _('Select items required on this tender') . '</p>';
	...
	if (isset($SearchResult)) {
		...
		echo '<a name="end"></a>
			<br />
			<div class="centre">
				<input type="submit" name="NewItem" value="' . _('Add to Quotation') . '" />
			</div>';
