...
header('location:../IRQ_TransportRequest.php');
}
...
$q="SELECT * FROM irq_request a 
							INNER JOIN irq_transport b on b.TransportID = a.requestid 
...
$comment= mysqli_real_escape_string($conn,$comm);
		...
		$a = "UPDATE irq_transport SET vregno='".$_POST['vregno']."' WHERE TransportID=".$id."";	
		mysqli_query($conn,$a) or die('Could not run Query: ' . mysqli_error($conn));
		}
		if($final==TRUE){
		...
		header('location:../IRQ_TransportRequest.php?Ref=Inbox');
		}else{
$HSQL="INSERT INTO irq_authorize_state (requisitionid,
											level,
											approvaldate,
...
<?php
		...
		<td align="left"><strong>'.$row['requestid'].'</strong></td>
		</tr><tr>
		<th>Required From</th>
		<td align="left">'.date("d, M Y",strtotime($row['required_from'])).'</td>
		<td><strong>Time:</strong> '. date("h:i A",strtotime($row['required_from'])) .'</td>
		</tr><tr>
		<th>Required To</th>
		<td align="left">'.date("d, M Y",strtotime($row['required_to'])).'</td>
		<td><strong>Time:</strong> '. date("h:i A",strtotime($row['required_to'])) .'</td>
		</tr><tr>
		<th>Transport Type</th>
		<td align="left">'.$row['transport_type'].'</td>
		</tr><tr>
		<th>Destination</th>
		<td align="left">'.$row['destination'].'</td>
		</tr><tr>
		<th>Nature of Duty</th>
		<td align="left" >'.$row['nature_of_duty'].'</td>
		</tr><tr>
		<th>Requesting Officer</th>
		<td align="left" >'.$row['requesting_officer'].'</td>
		<td align="left" ><strong>Department:</strong> '.$row['description'].'</td>
		</tr><tr>
		...
		echo '<br />';
		if($level==12 or $level==13){
		echo '<div style="font-size: large; width:90%" align="left" >Vehicle Reg No. : <input name="vregno" style="height:30px; background:#CCCCCC; font-size:14px;" type="text" /></div>';
		}
