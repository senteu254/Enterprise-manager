...
function displaySubmit(){
			document.form.action="Inbox_Main.php"; 
...
<table width="100%" border="0" style="height:60px;">
  <tr >
    <td colspan="2"><span class="title">Inbox</span></td>
  </tr>
  <tr>
    <td>
...
$query="SELECT * FROM irq_request z 
							...
							WHERE draft=0 AND closed=0 AND z.doc_id='". $_SESSION['Doc_Store'] ."' AND CASE WHEN d.userid ='HOD' THEN e.authoriser='".$_SESSION['UserID']."' WHEN d.userid ='ISSUE' THEN g.authoriser='".$_SESSION['UserID']."' WHEN d.userid ='PROCURE' THEN g.purchasing_officer='".$_SESSION['UserID']."' ELSE d.userid='".$_SESSION['UserID']."' END ".$search." ORDER BY Requesteddate DESC";
}else{
/*$query="SELECT * FROM irq_request z 
							INNER JOIN irq_stockrequest a on z.requestid = a.dispatchid 
							INNER JOIN irq_authorize_state b on z.requestid = b.requisitionid 
							...
							INNER JOIN departments e ON a.departmentid = e.departmentid
							INNER JOIN irq_documents f ON z.doc_id = f.doc_id
							WHERE draft=0 AND closed=0 AND d.userid='".$_SESSION['UserID']."' AND z.doc_id='". $_SESSION['Document_id'] ."' ORDER BY Requesteddate DESC"; */
$query="SELECT * FROM irq_request z 
							INNER JOIN irq_stockrequest a on z.requestid = a.dispatchid 
							INNER JOIN irq_authorize_state b on z.requestid = b.requisitionid 
							INNER JOIN irq_levels c ON b.level = c.level_id
							...
							INNER JOIN irq_documents f ON z.doc_id = f.doc_id
							INNER JOIN locations g ON a.loccode = g.loccode
							WHERE draft=0 AND closed=0 AND z.doc_id='". $_SESSION['Document_id'] ."' AND CASE WHEN d.userid ='HOD' THEN e.authoriser='".$_SESSION['UserID']."' WHEN d.userid ='ISSUE' THEN g.authoriser='".$_SESSION['UserID']."' WHEN d.userid ='PROCURE' THEN g.purchasing_officer='".$_SESSION['UserID']."' ELSE d.userid='".$_SESSION['UserID']."' END ".$search." ORDER BY Requesteddate DESC";
...
$query="SELECT * FROM irq_request z 
							...
							INNER JOIN departments e ON a.departmentid = e.departmentid
							INNER JOIN irq_documents f ON z.doc_id = f.doc_id
							WHERE draft=0 AND closed=0 AND CASE WHEN d.userid ='HOD' THEN e.authoriser='".$_SESSION['UserID']."' ELSE d.userid='".$_SESSION['UserID']."' END ".$search." ORDER BY Requesteddate DESC";
...
$query="SELECT * FROM irq_request z 
							...
							INNER JOIN irq_documents f ON z.doc_id = f.doc_id
							WHERE draft=0 AND closed=0 AND CASE WHEN d.userid ='HOD' THEN e.authoriser='".$_SESSION['UserID']."' ELSE d.userid='".$_SESSION['UserID']."' END ".$search." ORDER BY Requesteddate DESC";
}elseif($_SESSION['Document_id'] ==7){ //maintenance request 5 breakdown 6 preventive
$query="SELECT * FROM irq_request z 
...
if($num >0){
	...
	if (date("d, M Y",strtotime($row['Requesteddate'])) != $current_cat) 
					...
					?>
		<div class="hov" style="border-bottom:inset; width:100%"><a style="text-decoration:none;" href="<?php echo $inbox.'?id='.$row['requestid'].'&LV='.$row['level'] ?>" target="Content" onclick="setVisibility('<?php echo $row['requestid'].''.$row['level']; ?>');">
		<?php
		echo '<table width="100%" border="0"><tr><td><span class="subject">'.strtoupper(substr($row['doc_name'], 0, 28)).'</span></td><td align="right"><span class="time">'. date("h:i:s A",strtotime($row['Requesteddate'])).'</span></td></tr>';
		echo '<tr><td><span class="subject">'.substr('From Dept:'.strtoupper($row['description']), 0, 28).'</span></td><td align="right">';
		if($row['Unread']==0){ echo '<b style="background: #8AC007; font-size:9px;" id="'.$row['requestid'].''.$row['level'].'">new</b>';}else{
		if($row['Unread']==1 && $row['Sent']==0){ echo '<b style="background-color:#CC0000; font-size:9px;">Pending</b>';}
		else{echo '<b style="background-color:#3399FF; font-size:9px;">Sent</b>';}
		}
		echo '</td></tr></table>';
		echo '</a></div>';
	}
		...
		echo '<table width="100%"><tr><td><span class="subject">No Inbox</span></td></tr></table>';
	}
?>
