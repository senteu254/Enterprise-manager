...
<?php
		...
		echo '<tr class="strikeout">
					...
					<td align="center"><select style="width:80px" disabled="true" name="'. $row['dispatchid'] . 'Tag'. $row['dispatchitemsid'] . '">';
...
// End select tag
		...
		echo '<tr>
					...
					<td align="center">';
			if( $row['stkbal']>0){		
		echo '<input type="text" class="number" style="width:80px" name="'. $row['dispatchid'] . 'Qty'. $row['dispatchitemsid'] . '" value="0" />';
			}else{
			if($row['on_order']==1){
		...
		echo '</td>
					...
					<td align="center"><select style="width:80px" name="'. $row['dispatchid'] . 'Tag'. $row['dispatchitemsid'] . '">';
...
// End select tag
		...
		echo '<td align="center">'.$row['longdescription'].'</td>';
		...
		if($decision == 1){
		echo '<input name="DeclineRequestID" type="hidden" value="'.$id.'" />';
		$decline = '<input style="background:#FF3300; width:130px;" name="Decline" type="submit" value="<< Decline" /> &nbsp;';
		} //end if decision
		if($closed == FALSE){
		...
		echo $decline. ' <input name="SubmitType" type="submit" value="Continue >>" />';
		echo '</span>';
		}
		}else{
		if($final == TRUE && isset($_SESSION['Doc_Store'])){
		echo '<fieldset style="width:30%; border-radius:6px;"><legend><strong>Select Fulfil Type</strong></legend>';
		echo '<table><tr><td>';
		echo '<label>
		...
		echo '</div><br />';
		echo '<span id="submitButton">';
		echo $decline. ' <input name="Approve" type="submit"  value="Continue >>" />';
		echo '</span>';
		}else{
		echo '<span id="submitButton">';
		echo $decline. ' <input name="Approve" type="submit"  value="Continue >>" />';
