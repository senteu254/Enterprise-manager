...
function displaySubmit(){
			document.form.action="Completed_Menu.php"; 
...
<table width="100%" border="0" style="height:60px;">
  <tr >
    <td colspan="2"><span class="title">Completed</span></td>
  </tr>
  <tr>
    <td>
...
$query="SELECT * FROM irq_request z 
							...
							WHERE closed=1 AND z.Requesteddate >= DATE_SUB(NOW(), INTERVAL 1 MONTH) AND z.doc_id='".$_SESSION['Doc_Store']."' AND CASE WHEN d.userid ='HOD' THEN e.authoriser='".$_SESSION['UserID']."' WHEN d.userid ='ISSUE' THEN g.authoriser='".$_SESSION['UserID']."' WHEN d.userid ='PROCURE' THEN g.purchasing_officer='".$_SESSION['UserID']."' ELSE d.userid='".$_SESSION['UserID']."' END ".$search." GROUP BY z.requestid ORDER BY Requesteddate DESC";
}else{
$query="SELECT * FROM irq_request z 
							INNER JOIN irq_stockrequest a  on a.dispatchid = z.requestid 
							INNER JOIN irq_authorize_state b on z.requestid = b.requisitionid 
							...
							INNER JOIN irq_documents f ON z.doc_id = f.doc_id
							INNER JOIN locations g ON a.loccode = g.loccode
							WHERE closed=1 AND z.Requesteddate >= DATE_SUB(NOW(), INTERVAL 1 MONTH) AND z.doc_id='".$_SESSION['Document_id']."' AND CASE WHEN d.userid ='HOD' THEN e.authoriser='".$_SESSION['UserID']."' WHEN d.userid ='ISSUE' THEN g.authoriser='".$_SESSION['UserID']."' WHEN d.userid ='PROCURE' THEN g.purchasing_officer='".$_SESSION['UserID']."' ELSE d.userid='".$_SESSION['UserID']."' END ".$search." GROUP BY z.requestid ORDER BY Requesteddate DESC";
}
...
if($num >0){
	...
	if (date("d, M Y",strtotime($row['Requesteddate'])) != $current_cat) 
		...
		echo '<table width="100%"><tr><td><span class="subject">No Completed Requests</span></td></tr></table>';
	}
?>
