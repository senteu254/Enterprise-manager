...
$Title = _('Search Outstanding Oversea Orders');
...
if (isset($_POST['SearchParts'])) {
	...
	if ($_POST['Keywords']) {
		...
		$SQL = "SELECT stockmaster.stockid,
					SUM(overseaorderdetails.quantityord-overseaorderdetails.quantityrecd) AS qord
					stockmaster.description,
					SUM(locstock.quantity) AS qoh,
					stockmaster.units,
				FROM stockmaster INNER JOIN locstock
					INNER JOIN overseaorderdetails
					ON stockmaster.stockid = locstock.stockid
						ON stockmaster.stockid=overseaorderdetails.itemcode
					INNER JOIN overseaoders on overseaoders.orderno=overseaorderdetails.orderno
					INNER JOIN locationusers ON locationusers.loccode=overseaoders.intostocklocation AND locationusers.userid='" .  $_SESSION['UserID'] . "' AND locationusers.canview=1
...
if (!isset($OrderNumber) or $OrderNumber == '') {
	if (isset($SelectedSupplier)) {
		echo '<a href="' . $RootPath . '/Overseas_Header.php?NewOrder=Yes&amp;SupplierID=' . $SelectedSupplier . '">' . _('Add Purchase Order') . '</a>';
	} else {
		echo '<a href="' . $RootPath . '/Overseas_Header.php?NewOrder=Yes">' . _('Add Oversea Order') . '</a>';
	}
	echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . $Title . '</p>';
	echo '<table class="selection">
			<tr>
				<td>' . _('Contract Number') . ': <input type="text" name="OrderNumber" autofocus="autofocus" maxlength="27" size="30" />  ' . _('Into Stock Location') . ':
...
if (isset($StockItemsResult)) {
	...
	if (!isset($_POST['Status']) OR $_POST['Status'] == 'Pending_Authorised') {
		$StatusCriteria = " AND (overseaoders.status='Pending' OR overseaoders.status='Authorised' OR overseaoders.status='Printed') ";
	} elseif ($_POST['Status'] == 'Authorised') {
		$StatusCriteria = " AND (overseaoders.status='Authorised' OR overseaoders.status='Printed')";
	} elseif ($_POST['Status'] == 'Pending') {
		$StatusCriteria = " AND overseaoders.status='Pending' ";
	} elseif ($_POST['Status'] == 'Rejected') {
		$StatusCriteria = " AND overseaoders.status='Rejected' ";
	} elseif ($_POST['Status'] == 'Cancelled') {
		$StatusCriteria = " AND overseaoders.status='Cancelled' ";
	}
	if (isset($OrderNumber) AND $OrderNumber != '') {
		$SQL = "SELECT overseaoders.orderno,
						overseaoders.realorderno,
						suppliers.suppname,
						...
						SUM(overseaorderdetails.unitprice*overseaorderdetails.quantityord) AS ordervalue
				FROM overseaoders INNER JOIN overseaorderdetails
				ON overseaoders.orderno=overseaorderdetails.orderno
				INNER JOIN suppliers
				ON overseaoders.supplierno = suppliers.supplierid
				INNER JOIN currencies
				ON suppliers.currcode=currencies.currabrev
				WHERE overseaorderdetails.completed=0
				AND overseaoders.orderno='" . $OrderNumber . "'
				GROUP BY overseaoders.orderno ASC,
					suppliers.suppname,
					overseaoders.orddate,
					overseaoders.status,
					overseaoders.initiator,
		...
		if (isset($SelectedSupplier)) {
			...
			if (isset($SelectedStockItem)) {
				$SQL = "SELECT overseaoders.realorderno,
							overseaoders.orderno,
							suppliers.suppname,
							overseaoders.orddate,
							overseaoders.deliverydate,
							overseaoders.status,
						...
						INNER JOIN locationusers ON locationusers.loccode=overseaoders.intostocklocation AND locationusers.userid='" .  $_SESSION['UserID'] . "' AND locationusers.canview=1
						WHERE overseaorderdetails.completed=0
						AND overseaoders.orddate>='" . FormatDateForSQL($_POST['TransAfterDate']) . "'
						AND overseaoders.orddate<='" . FormatDateForSQL($_POST['TransToDate']) . "'
						AND overseaorderdetails.itemcode='" . $SelectedStockItem . "'
						AND overseaoders.supplierno='" . $SelectedSupplier . "'
						AND overseaoders.intostocklocation = '" . $_POST['StockLocation'] . "'
						" . $StatusCriteria . "
						GROUP BY overseaoders.orderno ASC,
							overseaoders.realorderno,
							suppliers.suppname,
	...
	while ($myrow = DB_fetch_array($PurchOrdersResult)) {
		...
		$ModifyPage = $RootPath . '/Overseas_Header.php?ModifyOrderNumber=' . $myrow['orderno'];
		if ($myrow['status'] == 'Printed') {
			$ReceiveOrder = '<a href="' . $RootPath . '/GoodsReceivedOverseas.php?PONumber=' . $myrow['orderno'] . '">' . _('Receive') . '</a>';
		} else {
			$ReceiveOrder = '';
		}
		if ($myrow['status'] == 'Authorised' AND $myrow['allowprint'] == 1) {
			$PrintPurchOrder = '<a target="_blank" href="' . $RootPath . '/PO_PDFPurchOrderOverseas.php?OrderNo=' . $myrow['orderno'] . '">' . _('Print') . '</a>';
		} elseif ($myrow['status'] == 'Authorisied' AND $myrow['allowprint'] == 0) {
			$PrintPurchOrder = _('Printed');
		} elseif ($myrow['status'] == 'Printed') {
			$PrintPurchOrder = '<a target="_blank" href="' . $RootPath . '/PO_PDFPurchOrderOverseas.php?OrderNo=' . $myrow['orderno'] . '&amp;realorderno=' . $myrow['realorderno'] . '&amp;ViewingOnly=2">
				' . _('Print Copy') . '</a>';
		} else {
