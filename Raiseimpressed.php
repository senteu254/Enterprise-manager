...
$sql = "UPDATE impressed SET label='" . $_POST['label'] ."',
...
include('includes/Level_Tracking_Impressed.php');
	...
	<div style="padding-left:70px; padding-top:15px; font-weight:bold;">'.$label.' &nbsp;Department &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Manager</div>
	<div style="padding-left:70px; padding-top:15px; font-weight:bold;">'.$label2.' &nbsp;AIE Holder</div>
	<div style="padding-left:70px; padding-top:15px; font-weight:bold;">'.$label3.' &nbsp;Finance Office</div
...
if (!isset($SelectedUser)) {
	$sql = "SELECT voucherid,
					...
					total
				FROM impressed
				WHERE process_level =0 OR process_level =10 OR process_level =11 ";
	$result = DB_query($sql);
	...
	if ($myrow['datereq']=='') {
		...
		$del= '<td><a href="%s&amp;SelectedUser=%s&amp;delete=1" onclick="return confirm(\'' . _('Are you sure you wish to delete this Imprest?') . '\');">' . _('Delete') . '</a></td>';
		$process='<td><a href="%s&amp;SelectedUser=%s&amp;process=1" onclick="return confirm(\'' . _('Are you sure you wish to Process this Imprest?') . '\');">' . _('Process') . '</a></td>';
		$view='';
		}
...
echo '<a href="' . $RootPath . '/Raiseimpressed.php">' . _('Back to Imprest Voucher') . '</a>';
}
#############################################################################################
...
echo '<td><select name="departmentid" onchange="ReloadForm(departmentForm.Updatedepartmentid)">';
...
/***************************************************************************************************************/
		echo'<tr>
		<td>' .  _('Select Employee') . '</td>';
	echo '<td>
	<div class="supplierfield"><select name="userid"  onchange="this.myform.submit">';
     $SQL = "SELECT userid,
	                 emp_id,
					 realname,
					 phone			
...
echo '</td>
	...
	<td>Description</td>
				<td>
				<textarea required name="shortdescription" value="'.$shortdescription.'" cols="30" rows="4"></textarea>
				</td>
			</tr>
	</table>
	<br/>';
