<?php
/* $Id: WorkOrderCosting.php 6945 2014-10-27 07:20:48Z daintree $*/
...
$RequirementsResult = DB_query("SELECT worequirements.stockid,
									   ...
									   SUM(worequirements.qtypu*(woitems.qtyrecd+woitems.qtyrejected)) AS requiredqty,
									   SUM(worequirements.stdcost*worequirements.qtypu*(woitems.qtyrecd+woitems.qtyrejected)) AS expectedcost,
...
while ($RequirementsRow = DB_fetch_array($RequirementsResult)){
	...
	if (DB_num_rows($IssuesResult)>0){
		while ($IssuesRow = DB_fetch_array($IssuesResult)){
			...
			echo '<td colspan="3"></td><td>' . $IssuesRow['serialno'] . '</td>
