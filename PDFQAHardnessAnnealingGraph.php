	...
	nobble the invoice reprints */
		...
		if (DB_error_no()!=0) {
			...
			//Change the language to the customer's language			
				...
				$pdf->addJpegFromFile('companies/' .$_SESSION['DatabaseName'] .  '/reports/hardnessgraph_'.$id.'_0.png',$Left_Margin+3,$YPos-300,0,299);
				$pdf->addJpegFromFile('companies/' .$_SESSION['DatabaseName'] .  '/reports/hardnessgraph_'.$id.'_1.png',$Left_Margin+255,$YPos-300,0,299);
				$pdf->addJpegFromFile('companies/' .$_SESSION['DatabaseName'] .  '/reports/hardnessgraph_'.$id.'_2.png',$Left_Margin+508,$YPos-300,0,299);
				}else{
				$pdf->addJpegFromFile('companies/' .$_SESSION['DatabaseName'] .  '/reports/hardnessgraph_'.$id.'.png',$Left_Margin+120,$YPos-300,0,300);
				}
...
$ListCount = 0;
$YPos -= 310;// Increment a line down for the next line item.
...
if (DB_num_rows($result)>0){
	while ($myrow2=DB_fetch_array($result)){
		...
		$LeftOvers = $pdf->addTextWrap($Left_Margin+210,$YPos+$FontSize,540,$FontSize, ucwords(strtolower($myrow2['remarks'])));
		while(mb_strlen($LeftOvers) > 1) {
		$YPos -= $FontSize;
		$LeftOvers = $pdf->addTextWrap($Left_Margin+220,$YPos+$FontSize,540,$FontSize, ucwords(strtolower($LeftOvers)));
	}

		$pdf->SetFont('','B');
		//$YPos -= 8;
		//$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos+$FontSize,250,8, ucwords(strtolower($myrow2['approvername'])));
		$LeftOvers = $pdf->addTextWrap($Left_Margin+670,$YPos+$FontSize,100,8, ConvertSQLDateTime($myrow2['remarkdate']),'right');
		$pdf->SetFont('','');
	...
	$YPos = 480;
if($myrow['hardness'] !=""){
$pdf->line($Page_Width-520, $YPos+$FontSize, $Left_Margin+120, $YPos+$FontSize);
$pdf->line($Page_Width-520, $YPos-50, $Page_Width-520, $YPos+$FontSize);
$pdf->line($Left_Margin+120, $YPos-50, $Left_Margin+120, $YPos+$FontSize);
$LeftOvers = $pdf->addTextWrap($Left_Margin+50,$YPos,220,8, 'HARDNESS: '.$myrow['hardness'],'right');	
$control = unserialize($myrow['base_control']);
$line =12;
foreach($control as $key=>$val){
$LeftOvers = $pdf->addTextWrap($Left_Margin+50,$YPos-$line,180,8, $key.'             '.$val,'right');
$line +=12;
}	
}
