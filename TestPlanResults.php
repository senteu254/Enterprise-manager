...
if (isset($_POST['submit'])) {
	...
	if($myrow[0]>0) {
		$sql = "UPDATE qrsampleremarks SET remark='" . $_POST['remark1'] . "'
				WHERE sampleid = '".$SelectedSampleID."' AND approver_level=1";
		$ErrMsg = _('The update of the QA Sample Remarks failed because');
		$DbgMsg = _('The SQL that was used and failed was');
		$result = DB_query($sql,$db,$ErrMsg, $DbgMsg);
	}else{
...
$groupby = $myrow['groupby'];
	...
	echo '<td><input type="hidden" name="ResultID' .$x. '" value="' . $myrow['resultid'] . '" /> ' . $myrow['name'] . '
			...
			/*
	echo '<td><input type="text" class="date" name="TestDate' .$x. '" size="10" maxlength="10" value="' . $TestDate . '" /> </td>
...
echo '<tr><td></td><td colspan="2"><input name="agree" type="checkbox" value="'.$_SESSION['UserID'].'" /> I <b>'.$_SESSION['UsersRealName'].'</b> Agree that I was Part of this Quality Assurance Technicians perform this tests.</td></tr>';
}
...
if ($CanCert==1){
	echo '<div class="centre"><a target="_blank" href="'. $RootPath . '/PDFCOA.php?LotKey=' .$LotKey .'&ProdSpec=' . $ProdSpec. '&QASampleID=' .$SelectedSampleID. '">' . _('Print COA') . '</a></div>';
}
