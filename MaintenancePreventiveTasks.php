...
$Title = _('Fixed Asset Preventive  Maintenance Tasks');

$ViewTopic = 'FixedAssets';
$BookMark = 'PreventiveAssetMaintenance';
...
if (isset($_POST['Submit'])) {
	if (!is_numeric(filter_number_format($_POST['FrequencyDays'])) OR filter_number_format($_POST['FrequencyDays']) < 0){
		...
		DB_query("UPDATE fixedassets SET pm_stat='1' where assetid='".$_POST['AssetID']."'");
		unset($_POST['AssetID']);
...
$sql="SELECT taskid,
		...
		WHERE fixedassettasks.doc_id='6' ";
...
AND irq_documents.doc_id='6'
AND irq_maintenance.status='0'
AND fixedassets.pm_stat='0'
";
$Result=DB_query($SQL);
while ($myrow=DB_fetch_array($Result)) {
...
$AssetSQL="SELECT assetid, description FROM fixedassets where pm_stat='1'";
$AssetResult=DB_query($AssetSQL);
