 ...
 */
	if ($InputError!=1){
		...
		if ($_SESSION['ExistingOrder']==0){ /*its a new order to be inserted */
			...
			$sql = "INSERT INTO overseaoders ( orderno,
											supplierno,
											comments,
											orddate,
											rate,
			...
			foreach ($_SESSION['PO'.$identifier]->LineItems as $POLine) {
				if ($POLine->Deleted==False) {
					$sql = "INSERT INTO overseaorderdetails (orderno,
														itemcode,
														deliverydate,
														itemdescription,
														glcode,
			...
			prnMsg(_('Purchase Order') . ' ' . $_SESSION['PO'.$identifier]->OrderNo . ' ' . _('on') . ' ' . $_SESSION['PO'.$identifier]->SupplierName . ' ' . _('has been created'),'success');
				...
				OR $_SESSION['PO'.$identifier]->Status=='Printed')){
			      ...
			      echo '<br /><div class="centre"><a target="_blank" href="'.$RootPath.'/PO_PDFPurchOrderOverseas.php?OrderNo=' . $_SESSION['PO'.$identifier]->OrderNo . '">' . _('Print Purchase Order') . '</a></div>';
			}
			...
			$sql = "UPDATE  overseaoders SET supplierno = '" . $_SESSION['PO'.$identifier]->SupplierID . "' ,
			...
			foreach ($_SESSION['PO'.$identifier]->LineItems as $POLine) {
				...
				if ($POLine->Deleted==true) {
					if ($POLine->PODetailRec!='') {
						$sql="DELETE FROM overseaorderdetails WHERE podetailitem='" . $POLine->PODetailRec . "'";
					...
					if ($POLine->Quantity==$POLine->QtyReceived){
						$sql = "UPDATE overseaorderdetails SET itemcode='" . $POLine->StockID . "',
...
if (count($_SESSION['PO'.$identifier]->LineItems)>0 and !isset($_GET['Edit'])){
	...
	echo '<table style="font-size:12px;"  cellpadding="2" class="selection">';
	echo '<tr>
...
if (!isset($_GET['Edit'])) {
	...
	echo '<table style="font-size:12px;" class="selection">
			<tr>
...
if (isset($SearchResult)) {
	...
	echo '<table style="font-size:12px;" cellpadding="1" class="selection">';
	echo $PageBar;
	$TableHeader = '<tr>
