...
If (!isset($_GET['id']) || $_GET['id']==""){
        $Title = _('Select Sick Leave To Print');
        include('includes/header.inc');
        echo '<div class="centre">
				<br />
				<br />
				<br />';
        prnMsg( _('Select a Sick Leave to Print before calling this page') , 'error');
        echo '<br />
				<br />
				<br />
				...
				<tr>
					<td class="menu_group_item">
						<ul><li><a href="index.php?Application=HR&Ref=LeaveAll' . _(' Sick All Approve') . '</a></li>
						</ul>
					</td>
				</tr>
				</table>
...
$ErrMsg = _('There was a problem retrieving the Sick header details for this Sick Leave Number') . ' ' . $_GET['id'] . ' ' . _('from the database');
$sql = "SELECT *,b.emp_id as id FROM leaves a
...
if (DB_num_rows($result)==0){
	$Title = _('Print Sick Leave Request Error');
	include('includes/header.inc');
	 echo '<div class="centre">
			<br />
			<br />
			<br />';
	prnMsg( _('Unable to Locate Sick Leave Number') . ' : ' . $_GET['id'] . ' ', 'error');
	echo '<br />
			<br />
			<br />
			<table class="table_index">
			<tr>
				<td class="menu_group_item">
					<ul><li><a href="index.php?Application=HR&Ref=LeaveAll">' . _(' Sick All Approve') . '</a></li></ul>
				</td>
			</tr>
			</table>
			</div>
...
$pdf->addInfo('Title', _('Sick Leave') );
$pdf->addInfo('Subject', _('Sick Leave') . ' ' . $_GET['id']);
$FontSize = 12;
$line_height = 12;// Recommended: $line_height = $x * $FontSize.

	/*Yes there are line items to start the ball rolling with a page header */
	include ('PDFSLHeader.php');
		...
		$LeftOvers = $pdf->addTextWrap(180,$YPos-120,100,$FontSize, $myrow['edate']);
		$LeftOvers = $pdf->addTextWrap(350,$YPos-120,100,$FontSize, $myrow['endate']);
		$LeftOvers = $pdf->addTextWrap(180,$YPos-140,300,$FontSize, $myrow['no_days']);
