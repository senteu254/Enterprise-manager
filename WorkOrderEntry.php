...
if (DB_num_rows($WOResult)==1){
	...
	$_POST['User'] = $myrow['operator'];
	$_POST['ForeMan'] = $myrow['foreman'];
...
echo '<tr><td class="label">' . _('Calibre') .':</td>
	<td><select name="Calibre" onChange="ReloadForm(form1.submit)">';
$Result = DB_query("SELECT id,calibre
						FROM wocalibre");
while ($Rows = DB_fetch_array($Result)){
	if ($_POST['Calibre']==$Rows['calibre']){
		echo '<option selected="True" value="' . $Rows['calibre'] .'">' . $Rows['calibre'] . '</option>';
	} else {
		echo '<option value="' . $Rows['calibre'] .'">' . $Rows['calibre'] . '</option>';
	}
}
echo '</select></td></tr>';
...
echo '</select></td></tr>';
echo '<tr><td class="label">' . _('ForeMan') .':</td>
	<td><select name="ForeMan" onChange="ReloadForm(form1.submit)">';
$Result = DB_query("SELECT www_users.userid, realname
						FROM www_users");
while ($Row = DB_fetch_array($Result)){
	if ($_POST['ForeMan']==$Row['userid']){
		echo '<option selected="True" value="' . $Row['userid'] .'">' . $Row['userid'] .'-' . $Row['realname'] . '</option>';
	} else {
		echo '<option value="' . $Row['userid'] .'">' . $Row['userid'] .'-' . $Row['realname'] . '</option>';
	}
}
echo '</select></td></tr>';
echo '<tr><td class="label">' . _('Operator') .':</td>
	<td><select name="User" onChange="ReloadForm(form1.submit)">';
$Result = DB_query("SELECT www_users.userid, realname
						FROM www_users");
while ($Row = DB_fetch_array($Result)){
