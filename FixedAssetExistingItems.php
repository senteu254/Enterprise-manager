...
/* $Id: FixedAssetItems.php 6945 2014-10-27 07:20:48Z daintree $ */
...
if (isset($_POST['submit'])) {
	...
	if (!is_numeric(filter_number_format($_POST['cost']))
		OR filter_number_format($_POST['cost'])<0){

		$InputError = 1;
		prnMsg(_('The cost is expected to be a number above 0 '),'error');
		$Errors[$i] = 'cost';
		$i++;
	}
	...
	if ($InputError !=1){
		...
		if ($_POST['submit']==_('Update')) { /*so its an existing one */
			...
			$datepurchased=date('Y-m-d', strtotime($date));
		...
		$date=addslashes($_POST['date']);
			...
			if($result){
			$SQL=DB_query("SELECT assetid FROM fixedassets where serialno= '" . $_POST['SerialNo'] . "' ");
			$RES=DB_fetch_array($SQL);
			$assetid=$RES['assetid']; 
			}
			if (is_numeric(filter_number_format($_POST['cost'])) && filter_number_format($_POST['cost'])>0){
...
/*If the page was called without $AssetID passed to page then assume a new asset is to be entered other wise the form showing the fields with the existing entries against the asset will show for editing with a hidden AssetID field. New is set to flag that the page may have called itself and still be entering a new asset, in which case the page needs to know not to go looking up details for an existing asset*/
	...
	$_POST['date']  = $AssetRow['datepurchased'];
...
echo '</select>
	...
	<tr>
		...
		<td><input ' . (in_array('SerialNo',$Errors) ?  'class="inputerror"' : '' ) .'  type="text" name="SerialNo" required="required" size="32" maxlength="30" value="' . $_POST['SerialNo'] . '" /></td>
	</tr>';
	...
	echo'<tr>
		<td>' . _('Date Purchased') . ':</td>
		<td><input ' . (in_array('date',$Errors) ?  'class="inputerror"' : '' ) .'  type="text" name="date"  id="date" value="' . $_POST['date'] . '"  required="required" /></td>
	</tr>';
	}
	else{
	echo'<tr>
		<td>' . _('Date Purchased') . ':</td>
		<td><input ' . (in_array('date',$Errors) ?  'class="inputerror"' : '' ) .'  type="text" name="date"  id="date" value="' . $_POST['date'] . '" /></td>
	</tr>';
	}
	...
	echo'<tr>
		<td>' . _('Cost') . ':</td>
		<td><input ' . (in_array('cost',$Errors) ?  'class="inputerror"' : '' ) .'  type="text" name="cost"  size="32" maxlength="30" value="' . $_POST['cost'] . '" required="required"/></td>
	</tr>';
	}
	else{
	echo'<tr>
		<td>' . _('Cost') . ':</td>
		<td><input ' . (in_array('cost',$Errors) ?  'class="inputerror"' : '' ) .'  type="text" name="cost"  size="32" maxlength="30" value="' . $_POST['cost'] . '"  /></td>
	</tr>';
	}
	echo'<tr>
...
if (isset($New)) {
	echo '<div class="centre">
			<br />
			<input type="submit" name="submit" value="' . _('Insert Existing Fixed Asset') . '" />';
} else {
	echo '<br />
		<div class="centre">
