...
$pdf->addInfo('Title', _('Paternity/Maternity Leave Report') );
$pdf->addInfo('Subject', _('Paternity/Maternity Leave Report') );
$FontSize = 12;
$line_height = 12;// Recommended: $line_height = $x * $FontSize.

/* Now ... Has the order got any line items still outstanding to be invoiced */

$ErrMsg = _('There was a problem retrieving the Paternity/Maternity Leave Report line details for Paternity/Maternity Leave Report Number') . ' ' .
...
if (DB_num_rows($result)>0){
	...
	include('PDFMPLeaveReportHeader.php');
...
if ($ListCount == 0){
        $Title = _('Print Paternity/Maternity Leave Report Error');
        include('includes/header.inc');
        echo '<p>' .  _('There were no Paternity/Maternity Leave Reports') . '. ' . _('The Paternity/Maternity Leave Report cannot be printed').
                '<br /><a href="index.php?Application=HR&Ref=SLReports">' .  _('Print Another Paternity/Maternity Leave Report');
        include('includes/footer.inc');
	exit;
} else {
    $pdf->OutputI($_SESSION['DatabaseName'] . '_Paternity/Maternity Leave Report_'. '_' . date('Y-m-d') . '.pdf');
    $pdf->__destruct();
}
ob_end_flush();
?>
