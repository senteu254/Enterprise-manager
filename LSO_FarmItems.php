...
$ViewTopic= 'Local Service Ordering';
$BookMark = 'Local service Ordering';
include('includes/header.inc');

if (!isset($_POST['Commit'])) {
	echo '<a href="'.$RootPath.'/LSO_Header.php?identifier=' . $identifier. '">' ._('Back To LSO Header') . '</a><br />';
}
 ...
 */
	...
	if ($_SESSION['PO'.$identifier]->DelAdd1=='' or mb_strlen($_SESSION['PO'.$identifier]->DelAdd1)<3){
		prnMsg( _('The Service order cannot be committed to the database because there is no delivery street address specified'),'error');
		$InputError=1;
	} elseif ($_SESSION['PO'.$identifier]->Location=='' or ! isset($_SESSION['PO'.$identifier]->Location)){
		prnMsg( _('The Service order can not be committed to the database because there is no location specified to book any stock items into'),'error');
		$InputError=1;
	} elseif ($_SESSION['PO'.$identifier]->LinesOnOrder <=0){
		prnMsg( _('The Service order can not be committed to the database because there are no lines entered on this order'),'error');
		$InputError=1;
	}
	...
	if ($InputError!=1){
		...
		if ($_SESSION['AutoAuthorisePO']==1) {
			...
			if (DB_num_rows($AuthResult) > 0 AND $AuthRow['authlevel'] > $_SESSION['PO'.$identifier]->Order_Value()) { //user has authority to authrorise as well as create the order
				...
				if (DB_num_rows($AuthResult) ==0){
					$AuthMessage = _('Your authority to approve Service orders in') . ' ' . $_SESSION['PO'.$identifier]->CurrCode . ' ' . _('has not yet been set up') . '<br />';
				} else {
					$AuthMessage = _('You can only authorise up to').' '.$_SESSION['PO'.$identifier]->CurrCode.' '.$AuthRow['authlevel'] .'.<br />';
				}

				prnMsg( _('You do not have permission to authorise this Service order').'.<br />' .  _('This order is for').' '.
			...
			foreach ($_SESSION['PO'.$identifier]->LineItems as $POLine) {
				...
				if ($POLine->Deleted==true) {
					if ($POLine->PODetailRec!='') {
						...
						$DbgMsg = _('The SQL statement used to delete the Service order detail record, that failed was');
						$result = DB_query($sql,$ErrMsg,$DbgMsg,true);
					}
				...
				$DbgMsg = _('The SQL statement used to update the Service order detail record that failed was');
				$result =DB_query($sql,$ErrMsg,$DbgMsg,true);
		...
		if ($_SESSION['PO'.$identifier]->Status == 'Authorised'
                   AND in_array($_SESSION['PageSecurityArray']['GoodsReceived.php'], $_SESSION['AllowedPageSecurityTokens'])){
                	...
                	echo '<a href="LSO_SupplierInvoice.php?SupplierID=' . $_SESSION['PO'.$identifier]->SupplierID . '&amp;ReceivePO=' . $_SESSION['PO'.$identifier]->OrderNo . '&amp;DeliveryDate=' . $_SESSION['PO'.$identifier]->DeliveryDate . '">' . _('Receive and Enter Service Invoice') . '</a>';
		}
		...
		$AlreadyOnThisOrder = 0;
			...
			if ($AlreadyOnThisOrder!=1 AND filter_number_format($Quantity) > 0){
				$sql = "SELECT description,
						...
						WHERE  farmdescriptions.description_Id = '". $ItemCode . "'";
...
/************************************************************************************************************************/
			if ($AlreadyOnThisOrder!=1 AND filter_number_format($Quantity) > 0){
			  	 ...
			  	 $sql = "SELECT a.description,
							...
							c.categoryid,
							d.accountname
						FROM farmdescriptions a
					    INNER JOIN stockmaster b ON a.stockid=b.stockid
						INNER JOIN stockcategory c ON b.categoryid=c.categoryid
						INNER JOIN chartmaster d ON c.stockact=d.accountcode
						WHERE a.description_Id = '". $ItemCode . "'";
			 $sql = "SELECT a.description,
							a.stockid,
							a.description_Id,
							a.units,
...
if (count($_SESSION['PO'.$identifier]->LineItems)>0 and !isset($_GET['Edit'])){
	...
	echo '<tr>
			...
			<th class="ascending">' . _('No./Quantity Our Units') . '</th>
			<th>' . _('Our Unit')  . '</th>
			<th class="ascending">' . _('Price Our Units') .' (' . $_SESSION['PO'.$identifier]->CurrCode .  ')</th>
			<th>' . _('Unit Conversion Factor') . '</th>
			<th class="ascending">' . _('No./Quantity') . '<br />' . _('Contractor Units') . '</th>
			<th>' .  _('Contractor Unit') . '</th>
			<th class="ascending">' . _('Price Units') . ' ('.$_SESSION['PO'.$identifier]->CurrCode.  ')</th>
	...
	echo '&nbsp;<input type="submit" name="Commit" value="' . _('Process LSO') . '" />
			</div>';
	...
	if ($_POST['Keywords']) {
		...
		if ($_POST['StockCat']=='All'){
			if ($_POST['SupplierItemsOnly']=='on'){
				 $sql = "SELECT farmdescriptions.description_Id,
		...
		if ($_POST['StockCat']=='All'){
			if ($_POST['SupplierItemsOnly']=='on'){
				$sql = "SELECT farmdescriptions.description_Id,
						...
						AND description_Id " . LIKE . " '" . $_POST['StockCode'] . "'
						";
			} else {
				$sql = "SELECT farmdescriptions.description_Id,
								farmdescriptions.description,
			...
			if ($_POST['SupplierItemsOnly']=='on'){
				...
				$sql = "SELECT farmdescriptions.description_Id,
								...
								farmdescriptions.units
						FROM farmdescriptions
						WHERE description_Id " . LIKE  . " '" . $_POST['StockCode'] . "'
						AND categoryid='" . $_POST['StockCat'] . "'
						 ";
			}
		}
...
if (!isset($_GET['Edit'])) {
	...
	echo ' /></td>
		...
		<tr><td></td>
		<td><b>' . _('OR') . ' </b><a target="_blank" href="'.$RootPath.'/Farm_Description.php">' . _('Insert New Service Description') . '</a></td></tr>
		</table>
		<br />
	...
	while ($myrow=DB_fetch_array($SearchResult)) {
		...
		*/
		echo '<td>' . $myrow['description_Id']  . '</td>
			...
			<input type="hidden" name="StockID' . $j .'" . value="' . $myrow['description_Id'] . '" />
			</tr>';
		$j++;
		$PartsDisplayed++;
