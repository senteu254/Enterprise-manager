...
if (isset($_GET['StaffID'])) {
	$StaffID = mb_strtoupper($_GET['StaffID']);
} elseif (isset($_POST['StaffID'])) {
	$StaffID = mb_strtoupper($_POST['StaffID']);
} else {
	unset($StaffID);
}
...
if(!isset($MaterialID) and !isset($StaffID)){
	echo '<a href="' . $RootPath . '/Sec_Staff.php">Back to Search Visitor</a>';
	include('includes/footer.inc');
	exit;
}
echo '<p class="page_title_text"><img src="'.$RootPath.'/css/'.$Theme.'/images/visitor.png" title="' . _('Search') . '" alt="" />' . ' ' . _('Staff Materials Booking Register') . '</p>';
$InputError = 0;
...
if (isset($_POST['submit'])) {
	...
	if ($InputError != 1) {
		...
		if (!isset($_POST['New'])) {
				$sql = "UPDATE staff_material_register SET description='" . $_POST['name'] . "',
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'SuppTrans' , PurchOrders, SupplierContacts
	...
	if ($CancelDelete == 0) {
		$sql="DELETE FROM staff_material_register WHERE id='" . $MaterialID . "'";
		$result = DB_query($sql);
...
$sql = "SELECT *, b.description as gatename, a.description as name
			...
			WHERE staffid=".$StaffID." and booked_out=0";

		$result = DB_query($sql);
			echo '<table class="selection">
					<tr><th>Description</th><th>Destination</th><th>Gate</th><th>Date</th><th>Purpose</th><th></th></tr>';
					while($row = DB_fetch_array($result)){
			echo '<tr><td>'.$row['name'].'</td><td>'.$row['destination'].'</td><td>'.$row['gatename'].'</td><td>'.ConvertSQLDateTime($row['booked_in_time']).'</td><td>'.$row['purpose'].'</td><td><a href="'.$link.'?MaterialID=' . $row['id'] .'&StaffID='.$StaffID.'">Edit</a> ||'; ?> <a onclick="return confirm('Are you sure you want to book out this item?');" href="<?php echo $link; ?>?MaterialID=<?php echo $row['id']; ?>&StaffID=<?php echo $StaffID; ?>&BookedOut=True">Book Out</a></td></tr><?php
			}
			echo '</table>';
...
/*If the page was called without $SupplierID passed to page then assume a new supplier is to be entered show a form with a Supplier Code field other wise the form showing the fields with the existing entries against the supplier will show for editing with only a hidden SupplierID field*/
	...
	echo '<form method="post" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?StaffID=' . $StaffID . '">';
	echo '<div>';
	echo '<a href="' . $RootPath . '/Sec_Staff.php">Back to Search Staff</a>';
...
//StaffID exists - either passed when calling the form or from the form itself
