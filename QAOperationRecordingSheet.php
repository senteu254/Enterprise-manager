...
if (isset($_POST['Submit'])) {
	...
	if (isset($_POST['SelectedMeasureID']) AND $_POST['SelectedMeasureID']!='' AND $InputError !=1) {
		...
		if ( $myrow[0] > 0 ) {
			...
			if ( DB_num_rows($result) != 0 ) {
				...
				$sql[] = "UPDATE qarecordingsheet
					SET operationid='" . $_POST['opid'] . "', typeid='" . $_POST['typeid'] . "', sheetname='" . $_POST['MeasureName'] . "', max_limit='" . $_POST['max'] . "', min_limit='" . $_POST['min'] . "', graphs='".$_POST['graph']."',basecontrol='".$_POST['bcontrol']."',hardness='".$_POST['hardness']."',description='" . $_POST['description'] ."',sheets='".$_POST['sheets']."'
					WHERE id = '" . $SelectedMeasureID . "'";
		...
		if ( $myrow[0] > 0 ) {
			...
			$sql = "INSERT INTO qarecordingsheet (operationid, typeid,sheetname,max_limit,min_limit,description,graphs,basecontrol,hardness,sheets)
					VALUES ('" . $_POST['opid'] ."','" . $_POST['typeid'] ."','" . $_POST['MeasureName'] ."','" . $_POST['max'] ."','" . $_POST['min'] ."','" . $_POST['description'] ."','".$_POST['graph']."','".$_POST['bcontrol']."','".$_POST['hardness']."','".$_POST['sheets']."')";
		}
  ...
  or deletion of the records*/
	...
	echo '<table class="selection">
			<tr>
				...
				<th class="ascending">' . _('Graphs') . '</th>
			</tr>';
...
if (! isset($_GET['delete'])) {
	...
	if (isset($SelectedMeasureID)) {
		...
		if ( DB_num_rows($result) == 0 ) {
			...
			$_POST['graph']  = $myrow['graphs'];
			$_POST['bcontrol']  = $myrow['basecontrol'];
			$_POST['hardness']  = $myrow['hardness'];
			$_POST['sheets']  = $myrow['sheets'];
	...
	echo '<tr>
		<td>' . _('Max Limit') . ':' . '</td>
		<td><input required="required" type="text" name="max" title="'._('Cannot be blank or contains illegal characters').'" placeholder="'._('More than one character').'" size="60" maxlength="100" value="' . $_POST['max'] . '" /></td>
		</tr>';
	echo '<tr>
		<td>' . _('Min Limit') . ':' . '</td>
		<td><input required="required" type="text" name="min" title="'._('Cannot be blank or contains illegal characters').'" placeholder="'._('More than one character').'" size="60" maxlength="100" value="' . $_POST['min'] . '" /></td>
		</tr>';
	echo '<tr>
		<td>' . _('Description') . ':' . '</td>
	...
	echo '<tr>
		<td>' . _('Graphs') . ':' . '</td>
		<td><input required="required" type="text" name="graph" title="'._('Cannot be blank or contains illegal characters').'" placeholder="'._('No of graphs drawn in one sheet').'" size="30" maxlength="30" value="' . $_POST['graph'] . '" /></td>
		</tr>';
		echo '<tr>
		<td>' . _('Base Control') . ':' . '</td>
		<td><input type="text" name="bcontrol" title="'._('Cannot be blank or contains illegal characters').'" placeholder="'._('Base Control').'" size="30" maxlength="30" value="' . $_POST['bcontrol'] . '" /></td>
		</tr>';
		echo '<tr>
		<td>' . _('Hardness') . ':' . '</td>
		<td><input type="text" name="hardness" title="'._('Cannot be blank or contains illegal characters').'" placeholder="'._('Hardness').'" size="30" maxlength="30" value="' . $_POST['hardness'] . '" /></td>
		</tr>';
		echo '<tr>
		<td>' . _('Sheets') . ':' . '</td>
		<td><input type="text" class="integer" name="sheets" title="'._('Cannot be blank or contains illegal characters').'" placeholder="'._('Sheets').'" size="30" maxlength="30" value="' . $_POST['sheets'] . '" /></td>
		</tr>';
	echo '</table>';
