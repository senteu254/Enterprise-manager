...
if (isset($_POST['submit'])) {
	...
	if (mb_strlen($_POST['SalesmanCode']) > 20) {
		$InputError = 1;
		prnMsg(_('The salesperson code must be 20 characters or less long'),'error');
	...
	if (isset($SelectedSalesPerson)) {
		...
		echo '<table class="selection">
			...
			$SQL = "SELECT UserID, realname
							FROM www_users";
			$data1 = DB_query($SQL);
			echo '<select name="SalesmanCode" id="SalesmanCode" >';
					while($rows = DB_fetch_row($data1)){
					print '<option value="'.$rows[0].'"'.($rows[0] == $_POST['SalesmanCode'] ? ' selected="selected"' : '').'>'.$rows[1].'</option>';
					}
			echo '</select>';
