...
foreach($_SESSION['CreditItems' . $identifier]->LineItems as $LnItm) {
	...
	foreach ($LnItm->Taxes AS $Tax) {
		$LineTotal = ($LineTotal/(1+$Tax->TaxRate));
		}
...
// Use of htmlspecialchars to convert ampersand, double_quote, single_quote, less_than, and greater_than to html entities to to preserve their meanings.
	...
	if(!isset($_POST['ProcessCredit'])) {
		...
		echo '<tr class="'.$RowClass.'">
	<td colspan="13"><textarea name="Narrative_' . $LnItm->LineNumber . '" cols="100%" style="width:95%" rows="1" tabindex="'.$tabindex++.'">' . $LnItm->Narrative . '</textarea>
				<br />
				<hr /></td>
			</tr>';
		$j++;
...
/*first the cost of sales entry*/
			...
			$TotalAmt = $CreditLine->Price * $CreditLine->QtyDispatched;
		foreach ($LnItm->Taxes AS $Tax) {
		$TotalAmt = ($TotalAmt/(1+$Tax->TaxRate));
		}
...
//Post sales transaction to GL credit sales
				...
				$SQL = "INSERT INTO gltrans(type,
					...
					VALUES(11,
						...
						'" . round($TotalAmt/$_SESSION['CurrencyRate'],$_SESSION['CompanyRecord']['decimalplaces']) . "'
						)";
				...
				if($CreditLine->DiscountPercent !=0) {
					...
					$SQL = "INSERT INTO gltrans(type,
						...
						VALUES(11,
							...
							'" . -round($TotalAmt * $CreditLine->DiscountPercent/$_SESSION['CurrencyRate'],$_SESSION['CompanyRecord']['decimalplaces']) . "'
							)";
