...
If (!isset($_GET['id']) || $_GET['id']==""){
        $Title = _('Select Vacation Leave To Print');
        include('includes/header.inc');
        echo '<div class="centre">
				<br />
				<br />
				<br />';
        prnMsg( _('Select a Vacation Leave to Print before calling this page') , 'error');
        echo '<br />
				<br />
				<br />
				...
				<tr>
					<td class="menu_group_item">
						<ul><li><a href="index.php?Application=HR&Ref=AllVLeave' . _(' Vacation All Approve') . '</a></li>
						</ul>
					</td>
				</tr>
				</table>
...
$ErrMsg = _('There was a problem retrieving the Vacation header details for this Vacation Number') . ' ' . $_GET['id'] . ' ' . _('from the database');
$sql = "SELECT *,b.emp_id as id FROM vacation_log a
...
if (DB_num_rows($result)==0){
	$Title = _('Print Vacation Leave Request Error');
	include('includes/header.inc');
	 echo '<div class="centre">
			<br />
			<br />
			<br />';
	prnMsg( _('Unable to Locate Vacation Number') . ' : ' . $_GET['id'] . ' ', 'error');
	echo '<br />
			<br />
			<br />
			<table class="table_index">
			<tr>
				<td class="menu_group_item">
					<ul><li><a href="index.php?Application=HR&Ref=AllVLeave">' . _(' Vacation All Approve') . '</a></li></ul>
				</td>
			</tr>
			</table>
			</div>
...
$pdf->addInfo('Title', _('Vacation Leave') );
$pdf->addInfo('Subject', _('Vacation Leave') . ' ' . $_GET['id']);
$FontSize = 12;
$line_height = 12;// Recommended: $line_height = $x * $FontSize.

	/*Yes there are line items to start the ball rolling with a page header */
	include ('PDFVacationHeader.php');
		...
		$LeftOvers = $pdf->addTextWrap(180,$YPos-40,300,$FontSize, $myrow['vdate']);
