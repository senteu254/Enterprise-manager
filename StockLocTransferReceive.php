...
//exit;
	...
	/*All inputs must be sensible so make the stock movement records and update the locations stocks */
		...
		/*Insert the StockSerialMovements and update the StockSerialItems  for controlled items*/
				...
				$Result = DB_query($sql, $ErrMsg, $DbgMsg, true);
			/*--------------------------------------------------------------------------------------------------*/	
			$sql = "SELECT requestno FROM loctransfers
			WHERE reference ='" . $_SESSION['Transfer']->TrfID . "'";
			$result = DB_query($sql);
			$req=DB_fetch_row($result);
			if($req[0] !="" && $req[0] >0){
				$SQL="UPDATE irq_stockrequestitems
						SET qtydelivered=qtydelivered+" . round($TrfLine->Quantity, $TrfLine->DecimalPlaces) . "
						WHERE dispatchid='" . $req[0] . "'
							AND stockid = '".  $TrfLine->StockID."'";
...
$sql = "UPDATE loctransfers SET shipqty = recqty
						WHERE reference = '". $_GET['Trf_ID'] . "'
						AND stockid = '".  $_GET['StockID']."'";
				$ErrMsg =  _('CRITICAL ERROR') . '! ' . _('Unable to set the quantity received to the quantity shipped to cancel the balance on this transfer line');
				$Result = DB_query($sql);
prnMsg(_('Line Cancelled Successfull'),'success');		
}
...
if(isset($_GET['Trf_ID'])){
	...
	$Sql="SELECT * FROM transferserialitems WHERE transferid='".$_GET['Trf_ID']."' AND stockid='".$myrow['stockid']."'";
	$rest = DB_query($Sql);
	while ($myr=DB_fetch_array($rest)) {
	$_SESSION['Transfer']->TransferItem[$i]->SerialItems[$myr['serialno']] = new SerialItem ($myr['serialno'], filter_number_format($myr['quantity']),'0000-00-00');
	}
...
if (isset($_SESSION['Transfer'])){
	...
	foreach ($_SESSION['Transfer']->TransferItem AS $TrfLine) {
		...
		echo '<td><a href="'.htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8'). '?Trf_ID='.$_GET['Trf_ID'].'&amp;StockID='.$TrfLine->StockID.'&amp;Cancel=true">' . _('Cancel This Line') . '</a></td>';
		echo '</tr>';
