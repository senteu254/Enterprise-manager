...
$PageSecurity=0;
	...
	DB_query("INSERT INTO commitment(payee_Name,date,lpo_No,commitments,voted_Item,cummulative_Com) 
		 VALUES ('$payee_Name','" . Date('Y-m-d') . "','$lpo_No','$commitments','$voteaccount','$cummulative_Com')"); 
	 ...
	 $SQL = "SELECT purchorders.realorderno,
						...
						WHERE purchorders.orderno='". $_POST['lpo_No'] ."' AND purchorders.status='Printed'";	 
				$rest=DB_query($SQL);
	 ...
	 $SQL = "SELECT lsorders.realorderno,
						...
						WHERE lsorders.orderno='". $_POST['lpo_No'] ."' AND lsorders.status='Printed'";	 
				$rest=DB_query($SQL);
				$r=DB_fetch_array($rest);
				$_POST['OrderSelected'] = $r['orderno'];
				$_POST[$r['orderno'].'SuppSelected'] = $r['suppname'];	
	 ...
	 $SQL = "SELECT purchorders.realorderno,
						...
						FROM purchorders INNER JOIN suppliers
						ON  purchorders.supplierno = suppliers.supplierid
						WHERE purchorders.status='Printed'";
...
$result=DB_query("SELECT a.allocated_Fund, a.suppliementary,
							ifnull((select SUM(b.commitments)
							from commitment b
							where b.voted_Item=a.votecode), 0) as commitment,
							ifnull((select SUM(b.decommitment)
							from commitment b
							where b.voted_Item=a.votecode), 0) as decommitment,
							...
							e.voted_Item=a.votecode), 0) as payments
