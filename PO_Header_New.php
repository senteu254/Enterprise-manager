...
if ($_SESSION['RequireSupplierSelection'] == 1 OR !isset($_SESSION['PO' . $identifier]->SupplierID) OR $_SESSION['PO' . $identifier]->SupplierID == '') {
	...
	echo '<tr>
	<td>' . _('PO No.') . ':</td>
	<td><input type="text" required="required" id="PONo" autofocus="autofocus" onblur="CheckOrder()" name="PONo" class="number" size="16" value="" /><div id="output" style="color:green"></div></td>
	</tr>';
	}
	echo '<tr>
			<td>' . _('PO Date') . ':</td>
...
<script>
function CheckOrder() {
	var state_id = document.getElementsByName('PONo')[0].value;
	if (state_id.length > 0 ) { 
	 $.ajax({
			type: "GET",
			url: "Ajax_OrderControl.php",
			data: "id="+state_id+"&type=LPO",
			cache: false,
			beforeSend: function () { 
