...
function DisplayBOMItems($UltimateParent, $Parent, $Component,$Level, $db) {
		...
		while ($myrow=DB_fetch_array($result)) {
			...
			if ($ParentMBflag!='M' AND $ParentMBflag!='G'){
				$AutoIssue = _('N/A');
			} elseif ($myrow['autoissue']==1){//autoissue and not controlled
				$AutoIssue = _('Yes');
			} elseif ($myrow['autoissue']==0) {
				$AutoIssue = _('No');
			} else {
				$AutoIssue = _('N/A');
			}
...
if (isset($Select)) { //Parent Stock Item selected so display BOM or edit Component
	...
	echo '<tr>
			<th colspan="13"><div class="centre"><b>' . $SelectedParent .' - ' . $ParentDesc . ' ('. $MBdesc. ') </b></div></th>
		</tr>';
    ...
    /* We do want to show the new component entry form in any case - it is a lot of work to get back to it otherwise if we need to add */
		...
		echo '</select></td>
				...
				<tr>
					<td>' . _('Quantity') . ': </td>
					<td><input ' . (in_array('Quantity',$Errors) ?  'class="inputerror"' : '' ) .' tabindex="4" type="text" class="number" required="required" name="Quantity" size="10" maxlength="20" title="' . _('Enter the quantity of this item required for the parent item') . '" value="';
