...
for($i=0; $i < $N; $i++)
{
$sql = "UPDATE impressed SET process_level=5 WHERE voucherid = '". $pv[$i] . "'";
		$result = DB_query($sql);

$SQL = "SELECT voucherno FROM  impressed_approval where voucherno='".$pv[$i]."' and process_level=5";
$resu=DB_query($SQL);
...
$sql = "UPDATE impressed SET process_level=13, comment='$comment' WHERE voucherid = '". $_POST['PV'] . "'";
...
echo '<a href="' . $RootPath . '/Impressed_AI.php">' . _('Back to Petty cash Requisition') . '</a>';
...
if (!isset($SelectedUser)) {
	$sql = "SELECT voucherid,
	               serialNo,
					...
					process_level,
					total
				FROM impressed WHERE process_level >=4 
				" .$sort. "
