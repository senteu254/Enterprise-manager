...
if (!isset($_SESSION['PO'.$identifier])){
	header('Location:' . $RootPath . '/LSO_Header.php');
	exit;
}

/* webERP manual links before header.inc */
$ViewTopic= 'ServiceOrdering';
$BookMark = 'ServiceOrdering';
include('includes/header.inc');

if (!isset($_POST['Commit'])) {
	echo '<a href="'.$RootPath.'/LSO_Header.php?identifier=' . $identifier. '">' ._('Back To Service Order Header') . '</a><br />';
}
	...
	foreach ($_SESSION['PO'.$identifier]->LineItems as $POLine) {
		if ($POLine->Deleted == false) {
			...
			if (!is_numeric(filter_number_format($_POST['SuppPrice'.$POLine->LineNo]))){
				...
				if($_POST['taxtype'.$POLine->LineNo]==1){
				 $_SESSION['PO'.$identifier]->LineItems[$POLine->LineNo]->Price = filter_number_format($_POST['SuppPrice'.$POLine->LineNo])/$_SESSION['PO'.$identifier]->LineItems[$POLine->LineNo]->ConversionFactor*1.16;
				 $_SESSION['PO'.$identifier]->LineItems[$POLine->LineNo]->Price1 = filter_number_format($_POST['SuppPrice'.$POLine->LineNo])/$_SESSION['PO'.$identifier]->LineItems[$POLine->LineNo]->ConversionFactor;
				 $_SESSION['PO'.$identifier]->LineItems[$POLine->LineNo]->TaxType = $_POST['taxtype'.$POLine->LineNo];
				}else{
 ...
 */
	if ($InputError!=1){
		...
		if ($_SESSION['ExistingOrder']==0){ /*its a new order to be inserted */
			...
			//$_SESSION['PO'.$identifier]->OrderNo =  GetNextTransNo(920, $db);

			/*Insert to purchase order header record */
			$sql = "INSERT INTO lsorders ( orderno,
											supplierno,
											comments,
											orddate,
											rate,
			...
			$ErrMsg =  _('The Service order header record could not be inserted into the database because');
			$DbgMsg = _('The SQL statement used to insert the Service order header record and failed was');
			$result = DB_query($sql,$ErrMsg,$DbgMsg,true);

		     /*Insert the purchase order detail records */
			foreach ($_SESSION['PO'.$identifier]->LineItems as $POLine) {
				if ($POLine->Deleted==False) {
					$sql = "INSERT INTO lsorderdetails (orderno,
														itemcode,
														deliverydate,
														itemdescription,
														glcode,
					...
					$ErrMsg =_('One of the Service order detail records could not be inserted into the database because');
					$DbgMsg =_('The SQL statement used to insert the Service order detail record and failed was');

					$result =DB_query($sql,$ErrMsg,$DbgMsg,true);
				}
			} /* end of the loop round the detail line items on the order */
			echo '<p />';
			prnMsg(_('Service Order') . ' ' . $_SESSION['PO'.$identifier]->OrderNo . ' ' . _('on') . ' ' . $_SESSION['PO'.$identifier]->SupplierName . ' ' . _('has been created'),'success');
				...
				OR $_SESSION['PO'.$identifier]->Status=='Printed')){
			      ...
			      echo '<br /><div class="centre"><a target="_blank" href="'.$RootPath.'/LSO_PDFPurchOrder.php?OrderNo=' . $_SESSION['PO'.$identifier]->OrderNo . '">' . _('Print Service Order') . '</a></div>';
			}
			...
			$sql = "UPDATE lsorders SET supplierno = '" . $_SESSION['PO'.$identifier]->SupplierID . "' ,
			...
			$ErrMsg =  _('The Service order could not be updated because');
			$DbgMsg = _('The SQL statement used to update the Service order header record, that failed was');
			$result = DB_query($sql,$ErrMsg,$DbgMsg,true);
			...
			foreach ($_SESSION['PO'.$identifier]->LineItems as $POLine) {
				...
				if ($POLine->Deleted==true) {
					if ($POLine->PODetailRec!='') {
						$sql="DELETE FROM lsorderdetails WHERE podetailitem='" . $POLine->PODetailRec . "'";
						$ErrMsg =  _('The Service order detail line could not be deleted because');
					...
					if ($POLine->Quantity==$POLine->QtyReceived){
						$sql = "UPDATE lsorderdetails SET itemcode='" . $POLine->StockID . "',
				...
				$ErrMsg = _('One of the Service order detail records could not be updated because');
				$DbgMsg = _('The SQL statement used to update the purchase order detail record that failed was');
				$result =DB_query($sql,$ErrMsg,$DbgMsg,true);

			} /* end of the loop round the detail line items on the order */
			echo '<br /><br />';
			prnMsg(_('Service Order') . ' ' . $_SESSION['PO'.$identifier]->OrderNo . ' ' . _('has been updated'),'success');
			if ($_SESSION['PO'.$identifier]->AllowPrintPO==1
					AND ($_SESSION['PO'.$identifier]->Status=='Authorised'
					OR $_SESSION['PO'.$identifier]->Status=='Printed')){

				echo '<br /><div class="centre"><a target="_blank" href="'.$RootPath.'/LSO_PDFPurchOrder.php?OrderNo=' . $_SESSION['PO'.$identifier]->OrderNo . '">' . _('Print Purchase Order') . '</a></div>';
			}
		...
		if ($_SESSION['PO'.$identifier]->Status == 'Authorised'
                   AND in_array($_SESSION['PageSecurityArray']['LSO_ServicedDone.php'], $_SESSION['AllowedPageSecurityTokens'])){

                	echo '<a href="SupplierInvoice.php?SupplierID=' . $_SESSION['PO'.$identifier]->SupplierID . '&amp;ReceivePO=' . $_SESSION['PO'.$identifier]->OrderNo . '&amp;DeliveryDate=' . $_SESSION['PO'.$identifier]->DeliveryDate . '">' . _('Receive and Enter Service Invoice') . '</a>';
		}
  ...
  * there will be an sql error  in SupplierInvoice.php without a valid GL Code
	*/
	...
	if (mb_strlen($_POST['ItemDescription'])<=3){
		$AllowUpdate = false;
		prnMsg(_('Cannot enter this order line') . ':<br />' . _('The description of the item being Service is required where a non-stock item is being ordered'),'warn');
	}
...
if (isset($_SESSION['RequestStockID'])
	...
	foreach ($_SESSION['RequestStockID'] as $rqt) {
		$Quantity =$rqt['QTY'];
		$ItemCode =$rqt['SID'];
		
		if (filter_number_format($Quantity)!=0) { //if the form variable represents a Qty to add to the order
		...
		if (mb_substr($FormVariableName, 0, 6)=='NewQty' AND filter_number_format($Quantity)!=0) { //if the form variable represents a Qty to add to the order
			...
			if ($AlreadyOnThisOrder!=1 AND filter_number_format($Quantity) > 0){
				...
				if (DB_num_rows($ItemResult)==1){
					...
					$ErrMsg = _('The purchasing data for') . ' ' . $ItemCode . ' ' . _('could not be retrieved because');
					$DbgMsg = _('The SQL used to retrieve the Service data but failed was');
					$PurchDataResult = DB_query($sql,$ErrMsg,$DbgMsg);
					if (DB_num_rows($PurchDataResult)>0){ //the purchasing data is set up
						$PurchRow = DB_fetch_array($PurchDataResult);
...
if (count($_SESSION['PO'.$identifier]->LineItems)>0 and !isset($_GET['Edit'])){
	...
	if (isset($_SESSION['PO'.$identifier]->OrderNo)) {
		echo  ' ' . _('Service Order') .' '. $_SESSION['PO'.$identifier]->OrderNo ;
	}
	echo '<br /><b>' . _(' LSO Summary') . '</b></p>';
	...
	echo '<tr>
			...
			<th>' . _('Our Unit')  . '</th>
			<th class="ascending">' . _('Price') .' (' . $_SESSION['PO'.$identifier]->CurrCode .  ')</th>
	...
	foreach ($_SESSION['PO'.$identifier]->LineItems as $POLine) {
		...
		if ($POLine->Deleted==False) {
			...
			if ($POLine->Price > 1) {
				...
				$SuppPrice1 = locale_number_format(round(($POLine->Price1 *$POLine->ConversionFactor),$_SESSION['PO'.$identifier]->CurrDecimalPlaces),$_SESSION['PO'.$identifier]->CurrDecimalPlaces);
				...
				$SuppPrice1 = locale_number_format(round(($POLine->Price1 *$POLine->ConversionFactor),4),4);
			}
			...
			echo '<td>' . $POLine->StockID  . '</td>
				...
				<td><input type="text" class="number" name="ConversionFactor' . $POLine->LineNo .'" size="5" value="' . locale_number_format($POLine->ConversionFactor,'Variable') . '" /></td>
				<td><input type="text" class="number" name="SuppQty' . $POLine->LineNo .'" size="8" value="' . locale_number_format(round($POLine->Quantity/$POLine->ConversionFactor,$POLine->DecimalPlaces),$POLine->DecimalPlaces) . '" /></td>
				<td>' . $POLine->SuppliersUnit . '</td>
				<td><input type="text" class="number" name="SuppPrice' . $POLine->LineNo . '" size="10" value="' . $SuppPrice1 .'" /></td>
				<td class="number">
				<select onchange="ReloadForm(UpdateLines)" name="taxtype'.$POLine->LineNo.'">
				<option value="0" '.($POLine->TaxType==0 ? 'selected="true"' : '').'>Inclusive</option>
				<option value="1" '.($POLine->TaxType==1 ? 'selected="true"' : '').'>Exclusive</option>
				</select></td>
				<td class="number">' . $SuppPrice . '</td>
	...
	echo '<tr><td colspan="11" class="number">' . _('TOTAL') . _(' Value with Tax') . '</td>
						<td class="number"><b>' . $DisplayTotal . '</b></td></tr>';
	echo '<tr>';
	
	if(!isset($DisplayTotal2)){
	$DisplayTotal2 = $DisplayTotal;
	}
	////////////////////////////////////////////////////////////////////////////////////
...
if (!isset($_GET['Edit'])) {
	...
	echo '<table class="selection">
			<tr>
				<th colspan="3"><h3>' .  _('Search For Service Items') . ':</h3></th>';
	...
	echo ' /></td>
		<td><b>' . _('OR') . ' </b>' . _('Enter extract of the Service Code') . ':</td>
...
include('includes/footer.inc');
?>
<script src="<?php echo $RootPath;?>/js/jquery-1.9.1.js"></script>
  <script language="javascript">

function findTotal(){
    var check = document.getElementsByName('taxtype').Checked;
	var tot = document.getElementsByName('totvalue').value;
	var totvat =0;
	if(check==true){
	totvat = (tot);
	}else{
    ...
    document.getElementById('DisplayTotal').value = totvat;
}
