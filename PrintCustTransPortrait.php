...
// gather the invoice data
	   ...
	   if (DB_error_no()!=0) {
		...
		if (DB_num_rows($result)>0){
		    ...
		    while ($myrow2=DB_fetch_array($result)){
				...
				$LeftOvers = $pdf->addTextWrap($Left_Margin+260,$YPos,76,$FontSize,$DisplayPrice,'right');
				...
				if ($myrow2['controlled']==1){
					...
					if ($myrow2['serialised']==1){
						...
						while ($ControlledMovtRow = DB_fetch_array($GetControlMovts)){
							...
							$btmnow -= ($line_height);
							$LeftOvers = $pdf->addTextWrap($Left_Margin+82,$YPos,100,$FontSize,$ControlledMovtRow['serialno'],'left');
							$LeftOvers = $pdf->addTextWrap($Left_Margin+162,$YPos,100,$FontSize,locale_number_format(-$ControlledMovtRow['moveqty']),'left');
							if ($YPos-$line_height <= $Bottom_Margin){
								/* head up a new invoice/credit note page */
								/*draw the vertical column lines right to the bottom */
								PrintLinesToBottom ();
		...
		if (($YPos-$Bottom_Margin)<(6*$line_height)){
			PrintLinesToBottom ();
			include ('includes/PDFTransPageHeaderPortrait.inc');
		}
		
		
		$pdf->line($Left_Margin, $Bottom_Margin+$BoxHeight,$Left_Margin, $btmnow);
		/*Now do the bottom left corner 180 - 270 coming back west*/
		$pdf->partEllipse($Left_Margin+10, $btmnow,180,270,10,10);
		/*Now a line to the bottom right */
		$pdf->line($Left_Margin+10, $btmnow-10,$Page_Width-$Right_Margin-10, $btmnow-10);
		/*Now do the bottom right corner */
		$pdf->partEllipse($Page_Width-$Right_Margin-10, $btmnow,270,360,10,10);
		/*Finally join up to the top right corner where started */
		$pdf->line($Page_Width-$Right_Margin, $btmnow,$Page_Width-$Right_Margin, $Bottom_Margin+$BoxHeight);
		...
		$pdf->line($Left_Margin+78, $TopOfColHeadings+12,$Left_Margin+78,$btmnow+85);

		/*Print a column vertical line */
		$pdf->line($Left_Margin+268, $TopOfColHeadings+12,$Left_Margin+268,$btmnow+85);

		/*Print a column vertical line */
		$pdf->line($Left_Margin+338, $TopOfColHeadings+12,$Left_Margin+338,$btmnow+85);

		/*Print a column vertical line */
		$pdf->line($Left_Margin+388, $TopOfColHeadings+12,$Left_Margin+388,$btmnow+85);

		/*Print a column vertical line */
		$pdf->line($Left_Margin+418, $TopOfColHeadings+12,$Left_Margin+418,$btmnow+85);

		$pdf->line($Left_Margin+448, $TopOfColHeadings+12,$Left_Margin+448,$btmnow+85);

		/*Rule off at bottom of the vertical lines */
		$pdf->line($Left_Margin, $btmnow+85,$Page_Width-$Right_Margin,$btmnow+85);
	...
	/* Print out the payment terms */
		...
		/*rule off for total */
		$pdf->line($Page_Width-$Right_Margin-222, $YPos-(4*$line_height),$Page_Width-$Right_Margin,$YPos-(4*$line_height));

		/*vertical to separate totals from comments and ROMALPA */
		$pdf->line($Page_Width-$Right_Margin-222, $YPos+$line_height,$Page_Width-$Right_Margin-222,$YPos-(5*$line_height));
		
		$LeftOvers = $pdf->addTextWrap($Page_Width-$Left_Margin-72, $YPos-(5*$line_height)+5, 72, $FontSize, $DisplayTotal, 'right');
		$YPos+=10;
		if ($InvOrCredit=='Invoice'){
			$LeftOvers = $pdf->addTextWrap($Page_Width-$Right_Margin-220, $YPos-(5*$line_height)-5, 144, $FontSize, _('TOTAL INVOICE'));
			$FontSize=10;
			...
			$PrintBankDetails = true; //assume we print bank details by default
				...
				$pdf->addText($Left_Margin, $YPos, $FontSize, _('Official Stamp') . ':___________________');
				$pdf->addText($Left_Margin+160, $YPos, $FontSize, _('Signature') . ':___________________');
				
			$pdf->addText(20, $YPos-25,$FontSize,  _('KOFC 55020201'));
			$pdf->addText(220, $YPos-25,$FontSize,  _('ISO 9001:2008 Certified Institution'));
			$pdf->addText(510, $YPos-25,$FontSize,  _('ISSUE 3 REV 0'));
           ...
           // $pdf->addText($Left_Margin, $YPos+22-$line_height*3, $FontSize, $DefaultBankAccountCode . '  ' . $DefaultBankAccountNumber);
			...
			$LeftOvers = $pdf->addTextWrap($Page_Width-$Right_Margin-220, $YPos-(5*$line_height)-5, 144, $FontSize, _('TOTAL CREDIT'));
 		}
	...
	if (isset($_GET['Email'])){ //email the invoice to address supplied
		...
		$pdf->OutputI($_SESSION['DatabaseName'] . '_' . $InvOrCredit . '_' . $FromTransNo . '.pdf');
...
$pdf->addText(20, $Bottom_Margin-5,$FontSize,  _('KOFC 55020201'));
$pdf->addText(220, $Bottom_Margin-5,$FontSize,  _('ISO 9001:2008 Certified Institution'));
$pdf->addText(510, $Bottom_Margin-5,$FontSize,  _('ISSUE 3 REV 0'));
