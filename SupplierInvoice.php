...
if (!isset($_POST['PostInvoice'])){
	...
	if (count( $_SESSION['SuppTrans']->GRNs)>0){   /*if there are any GRNs selected for invoicing then */
		...
		$tableheader = '<tr style="background-color:orange">
							<th>' . _('Seq') . ' #</th>
							<th>' . _('Order#') . '</th>
							<th>' . _('Item Code') . '</th>
							<th>' . _('Description') . '</th>
...
/********************************************8Insert data to the withholding table*********************************************************************************/
		$SQL = "INSERT INTO  withholdingtransactions (typeno,
											trandate,
											supplierid,
											amount)
								VALUES (
		...
		foreach ($_SESSION['SuppTrans']->Contracts as $Contract){
			...
			$SQL = "INSERT INTO contractcharges (contractref,
									...
									VALUES ('" . $Contract->ContractRef . "',
										...
										'" . $Contract->Narrative . "',
										'" . $Anticipated . "')";
          $SQL = "UPDATE contract_payment SET status = 1 WHERE PaymentID='" . $Contract->ContractRef . "'";
