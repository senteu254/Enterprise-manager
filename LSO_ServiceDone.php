...
$Title = _('Receive Service Orders');
include('includes/header.inc');

echo '<a href="'. $RootPath . '/LSO_SelectOSPurchOrder.php">' . _('Back to Service Orders'). '</a>
	<br />';
...
if (isset($_GET['PONumber']) AND $_GET['PONumber']<=0 AND !isset($_SESSION['PO'.$identifier])) {
	...
	echo '<div class="centre">
			<a href= "' . $RootPath . '/LSO_SelectOSPurchOrder.php">' . _('Select a Service order to receive') . '</a>
		</div>
		<br />' .  _('This page can only be opened if a Service order has been selected. Please select a purchase order first');
...
if ($_SESSION['PO'.$identifier]->Status != 'Printed') {
	prnMsg( _('Service orders must have a status of Printed before they can be received').'.<br />' .
...
echo '<p class="page_title_text">
		<img src="'.$RootPath.'/css/'.$Theme.'/images/supplier.png" title="' . _('Receive') . '" alt="" />' . ' ' . _('Receive Service Order') . ' : '. $_SESSION['PO'.$identifier]->OrderNo .' '. _('from'). ' ' . $_SESSION['PO'.$identifier]->SupplierName . '</p>';
...
if (count($_SESSION['PO'.$identifier]->LineItems)>0 and !isset($_POST['ProcessGoodsReceived'])){
	...
	*/
	...
	//Setup & Format values for LineItem display
		...
		$DisplaySupplierQtyOrd = locale_number_format($LnItm->Quantity);
		$DisplaySupplierQtyRec = locale_number_format($LnItm->QtyReceived);
		$DisplayQtyOrd = locale_number_format($LnItm->Quantity);
		$DisplayQtyRec = locale_number_format($LnItm->QtyReceived);
...
if ($_SESSION['PO'.$identifier]->SomethingReceived()==0 AND isset($_POST['ProcessGoodsReceived'])){ /*Then dont bother proceeding cos nothing to do ! */
	...
	prnMsg(_('Entered quantities cannot be greater than the quantity entered on the Service invoice including the allowed over-receive percentage'). ' ' . '(' . $_SESSION['OverReceiveProportion'] .'%)','error');
	echo '<br />';
	prnMsg(_('Modify the ordered items on the Service invoice if you wish to increase the quantities'),'info');
	echo '<div class="centre"><input type="submit" name="Update" value="' . _('Update') . '" />';
...
// Otherwise if you try to fullfill item quantities separately will give error.
	...
	if(DB_num_rows($Result)==0){//Those goods must have been received by another user. So should destroy the session data and show warning to users
		...
		echo '<div class="centre"><a href="' . $RootPath . '/LSO_SelectOSPurchOrder.php">' .
			_('Select a different Service order for receiving goods against') . '</a></div>';
	...
	while ($myrow = DB_fetch_array($Result)) {
		...
		if ($_SESSION['PO'.$identifier]->LineItems[$LineNo]->GLCode != $myrow['glcode'] OR
			...
			prnMsg(_('This order has been changed or invoiced since this delivery was started to be actioned') . '. ' . _('Processing halted') . '. ' . _('To enter a delivery against this Service order') . ', ' . _('it must be re-selected and re-read again to update the changes made by the other user'),'warn');
			...
			echo '<div class="centre"><a href="' . $RootPath . '/LSO_SelectOSPurchOrder.php">' .
				_('Select a different Service order for receiving goods against') . '</a></div>';
			echo '<div class="centre"><a href="' . $RootPath . '/LSO_ServiceDone.php?PONumber=' . $_SESSION['PO'.$identifier]->OrderNo . '">' .  _('Re-read the updated Service order for receiving goods against'). '</a></div>';
			unset($_SESSION['PO'.$identifier]->LineItems);
			unset($_SESSION['PO'.$identifier]);
			unset($_POST['ProcessGoodsReceived']);
            echo '</div>';
	...
	foreach ($_SESSION['PO'.$identifier]->LineItems as $OrderLine) {
		...
		if ($OrderLine->ReceiveQty !=0 AND $OrderLine->ReceiveQty!='' AND isset($OrderLine->ReceiveQty)) {
			...
			if ($OrderLine->StockID!='') { //Its a stock item line
				...
				$SQL = "SELECT cost as stdcost
							FROM farmdescriptions
							WHERE description_Id='" . $OrderLine->StockID . "'";
...
/*Now the SQL to do the update to the PurchOrderDetails */
			...
			$DbgMsg = _('The following SQL to update the Service order detail record was used');
			$Result = DB_query($SQL, $ErrMsg, $DbgMsg, true);
...
/* Need to get the current location quantity will need it later for the stock movement */
	...
	/* Insert stock movements - with unit cost 
					...
					foreach($OrderLine->SerialItems as $Item){
						...
						//need to test if the controlled item exists first already
							...
							if ($_SESSION['QualityLogSamples']==1) {
								CreateQASample($OrderLine->StockID,$Item->BundleRef, '', 'Created from Service Order', 0, 0,$db);
							}	
						}//non blank BundleRef
					} //end foreach
				}
			...
			if ($_SESSION['PO'.$identifier]->GLLink==1 AND $OrderLine->GLCode !=0){
				...
				$SQL = "INSERT INTO lsogltrans (type,
											typeno,
											trandate,
											periodno,
											account,
	...
	echo '<br />
		<div class="centre">
			'. prnMsg(_('SDN number'). ' '. $GRN .' '. _('has been processed'),'success') . '
			<br />
			<br />
			<a href="LSO_PDFGrn.php?GRNNo='.$GRN .'&amp;PONo='.$PONo.'">' .  _('Print to Cornferm Service Done  Note (SDN)') . '</a>
			<br />
			<br />
			<a href="' . $RootPath . '/LSO_SelectOSPurchOrder.php">' . _('Select a different Service order for receiving Service against'). '</a>
		</div>';
/*end of process goods received entry */
	...
	echo '<br />
		<div class="centre">
			<a href="' . $RootPath . '/LSO_Header.php?ModifyOrderNumber=' .$_SESSION['PO'.$identifier]->OrderNo . '">' . _('Modify Order Items'). '</a>
		</div>
		<br />
		<div class="centre">
			...
			<br />
			<br />
			<input type="submit" name="ProcessGoodsReceived" value="' . _('Process Service Received') . '" />
		</div>';
}
echo '</div>';
echo '</form>';
