...
If (!isset($_GET['TransNo']) OR $_GET['TransNo']==""){
	...
	echo '<p class="page_title_text"><img src="'.$RootPath.'/css/'.$Theme.'/images/sales.png" title="' . _('Search') . '" alt="" />' . ' ' . $Title . '</p><br />';
		...
		<tr>
			<td>' . _('Picking Note/Sales Order Number').' : ' . '</td>
			<td>
			<input name="TransNo" type="text" value="'.(isset($_POST['TransNo'])? $_POST['TransNo']:'').'" />
			</td>
		</tr>
		</table>';
	echo '<br />
		...
		FROM deliverynotes
		WHERE (deliverynotenumber LIKE '%".$_POST['TransNo']."%' OR salesorderno LIKE '%".$_POST['TransNo']."%')
		GROUP BY deliverynotenumber
		ORDER BY deliverynotenumber desc LIMIT 30";
...
//for ($i=1;$i<=2;$i++){  //Print it out twice one copy for customer and one for office 
	...
	if (DB_num_rows($result)>0){
		...
		include('includes/PDFOrderPageHeader_deliverynote.inc');
		//$YPos += 30;
		$YPosSUM =0;
		$Dynamic =-80;
		while ($myrow2=DB_fetch_array($result)){
			...
			if ($myrow2['controlled']==1){
					...
					$GetControlMovts = DB_query("SELECT moveqty,
														serialno
												 FROM deliveryserialmoves
												 WHERE deliveryno='" . $myrow2['deliverynotenumber'] . "' AND stockid='" . $myrow2['stkcode'] . "'");
					...
					if ($myrow2['serialised']==1){
						while ($ControlledMovtRow = DB_fetch_array($GetControlMovts)){
							...
							$YPosSUM += ($line_height);
							$Dynamic += ($line_height);
							$LeftOvers = $pdf->addTextWrap($XPos+150,$YPos,200,$FontSize,$ControlledMovtRow['serialno'],'left');
						...
						while ($ControlledMovtRow = DB_fetch_array($GetControlMovts)){
							...
							$Dynamic += ($line_height);
							$LeftOvers = $pdf->addTextWrap($XPos+150,$YPos,200,$FontSize,$ControlledMovtRow['serialno'],'left');
							$LeftOvers = $pdf->addTextWrap($XPos+250,$YPos,200,$FontSize,locale_number_format($ControlledMovtRow['moveqty']),'left');
							if ($YPos-$line_height <= 50){
							/* We reached the end of the page so finsih off the page and start a newy */
								$PageNumber++;
								$Count = 0;
...
include ('includes/PDFOrderPageHeader_deliverynote.inc');
	...
	$pdf->addText($XPos, $YP-15,$FontSize, _('Delivered By'). ':');
