...
if (isset($_POST['submit'])) {
	...
	if (isset($SelectedSampleID) AND $InputError !=1) {
		...
		CreateQASample($_POST['ProdSpecKey'],$_POST['LotKey'], $_POST['Identifier'], $_POST['Comments'], $_POST['Cert'], $_POST['DuplicateOK'],$_POST['Batch'],$_POST['SampleSize'],$_POST['ProductionDate'],$db);
	...
	//end if stock search results to show
		...
		if (DB_num_rows($SampleResult) > 0) {
			...
			while ($myrow = DB_fetch_array($SampleResult)) {
				...
				if ($myrow['cert']==1) {
					$CertAllowed='<a target="_blank" href="'. $RootPath . '/PDFCOA.php?LotKey=' .$myrow['lotkey'] .'&ProdSpec=' .$myrow['prodspeckey'] .'&QASampleID=' .$myrow['sampleid']. '">' . _('Yes') . '</a>';
				} else {
					$CertAllowed=_('No');
				}
...
if (! isset($_GET['delete'])) {
	...
	if (isset($SelectedSampleID)) {
		...
		echo '<tr>
			...
			<tr>
				<td>' . _('Batch Size') . ':</td>
				<td><input type="text" name="Batch" size="15" maxlength="15" value="' . $_POST['Batch']. '" /></td>
			</tr>
			<tr>
				<td>' . _('Sample Size') . ':</td>
				<td><input type="text" name="SampleSize" size="15" maxlength="15" value="' . $_POST['SampleSize']. '" /></td>
			</tr>
			<tr>
				<td>' . _('Identifier') . ':</td>
