	...
	echo '<br />
				...
				<table class="table_index">
					<tr><td class="menu_group_item">
						<li><a href="' . $RootPath . '/PO_SelectOSPurchOrderOverseas.php">' . _('Outstanding Purchase Orders') . '</a></li>
						<li><a href="' . $RootPath . '/PO_SelectPurchOrderOverseas.php">' . _('Purchase Order Inquiry') . '</a></li>
						</td>
					</tr></table>
				</div>
				<br />
...
if (isset($OrderNo)AND $OrderNo != 'Preview') {
	...
	$sql = "SELECT	overseaoders.supplierno,
					suppliers.suppname,
					suppliers.address1,
					suppliers.address2,
					suppliers.address3,
				...
				FROM overseaoders INNER JOIN suppliers
					ON overseaoders.supplierno = suppliers.supplierid
				INNER JOIN currencies
					ON suppliers.currcode=currencies.currabrev
				INNER JOIN www_users
					ON overseaoders.initiator=www_users.userid
				...
				WHERE overseaoders.orderno='" . $OrderNo . "'";
		...
		if ($ViewingOnly == 0) {
			if ($POHeader['allowprint'] == 0) {
				...
				echo '<div class="centre">
 					<li><a href="' . $RootPath . '/PO_PDFPurchOrderOverseas.php?OrderNo=' . $OrderNo . '&ViewingOnly=1">' . _('Print This Order as a Copy') . '</a>
 					<li><a href="' . $RootPath . '/Overseas_Header.php?ModifyOrderNumber=' . $OrderNo . '">' . _('Modify the order to allow a real reprint') . '</a>
					<li><a href="' . $RootPath . '/PO_SelectPurchOrderOverseas.php">' . _('Select another order') . '</a>
...
if (isset($MakePDFThenDisplayIt) or isset($MakePDFThenEmailIt)) {
	...
	if ($OrderNo != 'Preview') { // It is a real order
		...
		$sql = "SELECT itemcode,
						...
						conversionfactor,
						suppliers_partno
				FROM overseaorderdetails LEFT JOIN stockmaster
	...
	if ($OrderNo == 'Preview' or DB_num_rows($result) > 0) {
		...
		include('includes/PO_PDFOrderPageHeaderOverseas.inc');
		...
		$pdf->addText($FormDesign->OrderTotalCaption->x, $Page_Height - $FormDesign->OrderTotalCaption->y, $FormDesign->OrderTotalCaption->FontSize, _('Order Total') . ' ' . $POHeader['currcode']);
	...
	if ($ViewingOnly == 0 AND $Success == 1) {
		...
		$sql = "UPDATE overseaoders	SET	allowprint =  0,
...
/* There was enough info to either print or email the purchase order */
	...
	if ($_POST['PrintOrEmail'] == 'Email') {
		...
		$SQL = "SELECT suppliercontacts.contact,
				...
				FROM suppliercontacts INNER JOIN overseaoders
				ON suppliercontacts.supplierid=overseaoders.supplierno
				WHERE overseaoders.orderno='" . $OrderNo . "'";
