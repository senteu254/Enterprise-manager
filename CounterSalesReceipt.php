...
if ($InputError == false) {
	...
	echo prnMsg( _('Receipt number'). ' '. $ReceiptNumber .' '. _('processed'), 'success');
		...
		echo '<a target="_blank" href="'.$RootPath.'/PDFReceipt.php?BatchNumber='. $ReceiptNumber .'&ReceiptNumber=1"> <img src="'.$RootPath.'/dist/Receipt-Printer_icon.png" title="' . _('Print this Receipt') . '" alt="" /></a><br /><br />';
		
		echo '<br /><br /><a href="' .htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">Back To Enter Receipts</a></div>';
	exit;		
		} //end if $_POST['AmountPaid']!= 0
...
<!-- Top register controls -->
    ...
    <?php echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />'; ?>      
		...
		<span style="font-size:26px; color:#0099FF;"><span class="glyphicon glyphicon-shopping-cart"></span> Sales Register</span>
				...
				<span class="pull-right">
					<a target="_blank" href="<?php echo $RootPath.'/DebtorsAtPeriodEnd.php'; ?>" class="btn btn-success btn-sm" style=" color:#FFFFFF;" id="sales_takings_button" title="Top Sales"><span class="glyphicon glyphicon-print">&nbsp;</span>Debtor Balances</a>&nbsp;&nbsp;</span>
				<span class="pull-right">
		<a target="_blank" href="<?php echo $RootPath.'/Cash_Book.php'; ?>" class="btn btn-primary btn-sm" style=" color:#FFFFFF;" id="sales_takings_button" title="Order Inquiry"><span class="glyphicon glyphicon-print">&nbsp;</span>Cash Book</a>&nbsp;&nbsp;</span>
				</span>&nbsp;&nbsp;
			
				<span class="pull-right">						
					<a target="_blank" href="<?php echo $RootPath.'/DailyBankTransactions.php'; ?>" class="btn btn-danger btn-sm" style=" color:#FFFFFF;" id="sales_takings_button" title="Transactions"><span class="glyphicon glyphicon-list-alt">&nbsp;</span>Bank Transactions</a>&nbsp;&nbsp;</span>
				</span>
		</div>
	...
	echo '<select name="SelectedLoc" class="selectpicker show-menu-arrow" disabled onChange="TransTypes();" data-style="btn-default btn-sm" data-width="fit">';
		while($row = DB_fetch_row($sqlresult)){
		echo '<option '.($row[0] == $_SESSION['Items'.$identifier]->Location ? 'selected' : '').' value="'.$row[0].'">'.$row[1].'</option>';
		}
	echo '</select>';
	echo '<input name="" type="hidden" value="'.$_SESSION['Items'.$identifier]->Location.'">';
		?>
		</span>		
		</div>
	</form>
...
*/
	...
	$QOHResult = DB_query("SELECT sum(quantity)
		...
		echo '<td><input class="number" disabled type="text" autocomplete="off" onBlur="Recalculate2();" onfocus="TextBox();" name="Price_' . $OrderLine->LineNumber . '" required="required" size="10" maxlength="16" value="' . locale_number_format($OrderLine->Price,$_SESSION['Items'.$identifier]->CurrDecimalPlaces) . '" /></td>';
		...
		echo '<td><input class="number" tabindex="2" disabled autocomplete="off" onBlur="Recalculate2();" onfocus="TextBox();" type="text" name="Quantity_' . $OrderLine->LineNumber . '" required="required" size="5" maxlength="7" value="' . locale_number_format($OrderLine->Quantity,$OrderLine->DecimalPlaces) . '" /></td>';
		}

		if (in_array($_SESSION['PageSecurityArray']['OrderEntryDiscountPricing'], $_SESSION['AllowedPageSecurityTokens'])){
			echo '<td><input class="number" disabled onBlur="Recalculate2();" onfocus="TextBox();" type="text" autocomplete="off" name="Discount_' . $OrderLine->LineNumber . '" required="required" size="5" maxlength="5" value="' . locale_number_format(($OrderLine->DiscountPercent * 100),$_SESSION['Items'.$identifier]->CurrDecimalPlaces) . '" />
...
//}
		...
		<?php if($_SESSION['Items'.$identifier]->TRANSACTION =="RECEIPT INVOICE"){ ?>
		<div id="payment_details">
						<table style="width:100%; border:none;" class="sales_table_100">
							...
							echo '<td>Narrative:</td><td><textarea name="Comments" cols="" rows="">' . stripcslashes($_SESSION['Items'.$identifier]->Comments) . '</textarea></td>';
