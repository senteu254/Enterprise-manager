...
If (!isset($_GET['id']) || $_GET['id']==""){
        $Title = _('Select Sick Leave Report To Print');
        include('includes/header.inc');
        echo '<div class="centre">
				<br />
				<br />
				<br />';
        prnMsg( _('Select a Sick Leave Report to Print before calling this page') , 'error');
        echo '<br />
				<br />
				<br />
				...
				<tr>
					<td class="menu_group_item">
						<ul><li><a href="index.php?Application=HR&Ref=SLReports">' . _('Sick Report') . '</a></li>
						</ul>
					</td>
				</tr>
				</table>
...
$ErrMsg = _('There was a problem retrieving the sick leave  header details for Request Number') . ' ' . $_GET['id'] . ' ' . _('from the database');
...
$sql = "(SELECT * ,employee.emp_id as id FROM employee, leaves, departments,general_manager 
									...
									AND employee.emp_id='" . $_GET['id'] . "' )
									UNION
									(SELECT * ,employee.emp_id as id FROM employee, leaves, departments,general_manager 
									WHERE employee.id_dept = departments.departmentid 
									AND employee.emp_id = leaves.emp_id
									AND general_manager.gm = leaves.general_manager
...
$sql = "(SELECT *,SUM(no_days) AS total FROM employee, leaves, departments,general_manager 
									WHERE employee.id_dept = departments.departmentid 
									AND employee.emp_id = leaves.emp_id
									AND general_manager.gm = leaves.general_manager
									AND general_manager.gm ='4'
									...
									UNION
									(SELECT *,SUM(no_days) AS total FROM employee, leaves, departments,program_head 
									WHERE employee.id_dept = departments.departmentid 
									AND employee.emp_id = leaves.emp_id
									AND program_head.prog_head = leaves.prog_head
									AND program_head.prog_head ='4'
									...
									GROUP BY employee.emp_id)
									UNION
									(SELECT *,SUM(no_days) AS total FROM employee, leaves, departments,managing_director 
									WHERE employee.id_dept = departments.departmentid 
									AND employee.emp_id = leaves.emp_id
									AND managing_director.md = leaves.managing_director
									AND managing_director.md ='2'
...
if (DB_num_rows($result)==0){
	$Title = _('Print Sick Leave Report Error');
	include('includes/header.inc');
	 echo '<div class="centre">
			<br />
			<br />
			<br />';
	prnMsg( _('Unable to Locate Sick Leave Report Number') . ' : ' . $_GET['id'] . ' ', 'error');
	echo '<br />
			<br />
			<br />
			<table class="table_index">
			<tr>
				<td class="menu_group_item">
					<ul><li><a href="index.php?Application=HR&Ref=SLReports">' . _('Sick Report') . '</a></li></ul>
				</td>
			</tr>
			</table>
			</div>
...
$pdf->addInfo('Title', _('Sick Leave Report') );
$pdf->addInfo('Subject', _('Sick Leave Report') . ' ' . $_GET['id']);
$FontSize = 12;
$line_height = 12;// Recommended: $line_height = $x * $FontSize.

/* Now ... Has the order got any line items still outstanding to be invoiced */

$ErrMsg = _('There was a problem retrieving the Sick Leave Report line details for Sick Leave Report Number') . ' ' .
...
if ($ListCount == 0){
        $Title = _('Print Sick Leave Report Error');
        include('includes/header.inc');
        echo '<p>' .  _('There were no Sick Leave Reports') . '. ' . _('The Sick Leave Report cannot be printed').
                '<br /><a href="index.php?Application=HR&Ref=SLReports">' .  _('Print Another Sick Leave Report');
        include('includes/footer.inc');
	exit;
} else {
    $pdf->OutputI($_SESSION['DatabaseName'] . '_Sick Leave Report_' . $_GET['id'] . '_' . date('Y-m-d') . '.pdf');
    $pdf->__destruct();
}
ob_end_flush();
?>
