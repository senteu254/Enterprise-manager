...
for($i=0; $i < $N; $i++)
{
$sql = "UPDATE impressed SET process_level=2,controlled='" .$_POST['controlled']. "' WHERE voucherid = '". $pv[$i] . "'";
		$result = DB_query($sql);

$SQL = "SELECT voucherno FROM  impressed_approval where voucherno='".$pv[$i]."' and process_level=2";
$resu=DB_query($SQL);
...
$sqlq = "INSERT INTO impressed_approval (voucherno,
	...
	prnMsg( _('The selected Requistion Has been Certified and Forwarded to Controlling Department'), 'success' );
	unset($_POST['approve']);
	
$sqly = "UPDATE impressed SET controlled=2 WHERE voucherno='".$pv[$i]."'";
...
$sql = "UPDATE impressed SET process_level=10, comment='$comment' WHERE voucherid = '". $_POST['PV'] . "'";
...
echo '<a href="' . $RootPath . '/Imprest_DepartmentProcess.php">' . _('Back to Petty cash Requisition') . '</a>';
...
echo '<table id="dataTable" class="selection">';
echo'<tr>
<td>Controlled Department</td>
                   <td><select required="required" name="controlled">';
				if ($_POST['controlled']=='1'){
				  echo'<option selected="selected" value="1">Procurement</option>
				  <option value="2">Human Resource</option>
				  <option selected="selected">--Select Controlled Department--</option>';
				  }elseif ($_POST['label']=='2'){
				  echo'<option value="1">Procurement</option>
				  <option selected="selected" value="2">Human Resource</option>
				  <option >--Select Label--</option>';
				  }else{
				  echo '<option value="1">Procurement</option>
				  <option value="2">Human Resource</option>
				  <option selected="selected" value="">--Select Controlled Department--</option>';
				  }
	echo '</select></td>';
...
if (!isset($SelectedUser)) {
	$sql = "SELECT voucherid,
					...
					total,
					controlled
				FROM impressed WHERE process_level =1
				".$sort."
