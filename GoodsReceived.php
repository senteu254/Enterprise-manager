...
##############################################################################################################################################
	...
	echo '<div class="centre"><input type="submit" name="Update" value="' . _('Update2') . '" />';
...
/*Need to insert a GRN item */
			$GRN_Numbering = GetNextGRNNo($_SESSION['PO'.$identifier]->Location, $db);
...
/* Need to get the current location quantity will need it later for the stock movement */
	...
	echo '<br />
		<div class="centre">
			'. prnMsg(_('Receipt Voucher(RV) number'). ' '. $GRN_Numbering .' '. _('has been processed'),'success') . '
			<br />
			<br />
...
/*end of process goods received entry */
	...
	echo '<br />
		...
		<div class="centre">
			...
			<br />';
		#############################################################check if commited##############################################################################################	
			/*$sql7 = "SELECT * FROM commitment WHERE lpo_No ='".$_SESSION['PO'.$identifier]->OrderNo."'";
$result7= DB_query($sql7);
if (DB_num_rows($result7)==0){
		$InputError = 1;
		echo "<script  type='text/javascript'>alert('Order Number has not been commited,Consult Votebook Bills for Commitment  !')</script>";
		$Errors[$i] = 'Order No';
		$i++;
	}elseif (DB_num_rows($result7)==1){
		$msg = _(' Order No has been Commited ,proceed to receave  good ');*/
		
			echo'<input type="submit" name="ProcessGoodsReceived" value="' . _('Process Goods Received') . '" />
		</div>';
		//}
}
   ##################################################################end of checking#################################################################################################
