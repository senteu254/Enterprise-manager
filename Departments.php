...
if (! isset($_GET['delete'])) {
	...
	if (isset($SelectedDepartmentID)) {
		...
		if ( DB_num_rows($result) == 0 ) {
			...
			$_POST['initialdept'] = $myrow['initialdept'];
	...
	echo '<tr>
			...
			<tr>
			<td>' . _('Inital Name') . ':' . '</td>
			<td><input type="text" name="initialdept" size="10" required="required" title="' ._('The Initials name is required') . '" maxlength="15" value="' . $_POST['initialdept'] . '" /></td>
		</tr>
		<tr>
			<td>' . _('Authoriser') . '</td>
