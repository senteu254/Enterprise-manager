...
echo '<div class="page_help_text">' . _('There is No.payment voucher select to be payed at the moment,click the button to select voucher.') . '</div>
	<br />';
echo'<br>';
echo '<a href="' . $RootPath . '/index.php?Application=PVM&Ref=default&Link=cheque"><button type="button" name="" class="button"><i class="fa fa-reply"></i> ' . _('View Payment Voucher to Approve') . '</button></a>';
exit;
}*/
#################################################################################################3
...
echo '<div class="page_help_text">' . _('Use this screen to enter payments FROM your bank account.  <br />Note: To enter a payment FROM a supplier, first select the Supplier, click Enter a Payment to, or Receipt from the Supplier, and use a negative Payment amount on this form.') . '</div>
	<br />';
#########################################################################################################################################################################
		$sqli = "SELECT *,a.voucherid,
		                  a.votehead AS PV_Votehead,
						  a.invoice_no,
						  c.VoteHead,
						  a.lpo_no,
						  a.total,
				...
				INNER JOIN funds_allocations b ON a.votehead=b.votecode
				INNER JOIN voteheadmaintenance c ON a.votehead=c.Votecode
				WHERE a.voucherid = '" . $_GET['Vid'] . "' ";
	$resulti = DB_query($sqli);
	while ($myr=DB_fetch_array($resulti)){
	$tax = $myr['tax'];
	$VoteHead = $myr['VoteHead'];
	$Votecode = $myr['Votecode'];
	$PV_Votehead = $myr['PV_Votehead'];
	$VoucherID = $myr['voucherid'];
	$PV_total = $myr['total'];
	$_POST['VoteHead'] = $myr['VoteHead'];
	$lpono = unserialize($myr['lpo_no']);
   }
...
$_SESSION['PaymentDetail' . $identifier]->VID;
...
if (isset($_POST['VoteHead']) AND $_POST['VoteHead']!=''){
	$_SESSION['PaymentDetail' . $identifier]->VoteCode = $VoteHead;
	$_SESSION['PaymentDetail' . $identifier]->Votecode = $Votecode;
	$_SESSION['PaymentDetail' . $identifier]->VoucherID =$VoucherID;
	$_SESSION['PaymentDetail' . $identifier]->PV_Votehead =$PV_Votehead;
	$_SESSION['PaymentDetail' . $identifier]->PV_Total =$PV_total;
	$_SESSION['PaymentDetail' . $identifier]->tax =$tax;
	$_SESSION['PaymentDetail' . $identifier]->Cheque = $_POST['ChequeNum'];
	
	$_SESSION['PaymentDetail' . $identifier]->Amount_Total = str_replace(',','',$_SESSION['PaymentDetail' . $identifier]->PV_Total);
	$sqls = "SELECT *,b.tax_name,a.tax_percentage FROM  tax_deducted a 
	         INNER JOIN pv_tax b ON a.tax_percentage=b.ptid WHERE a.Vid=" .  $_SESSION['PaymentDetail' . $identifier]->VoucherID . "";
	$results = DB_query($sqls);
	echo '<tr>';	
		while ($myrowq = DB_fetch_array($results)) {
			$_SESSION['PaymentDetail' . $identifier]->tax =$myrowq['percentage'];
				if($_SESSION['PaymentDetail' . $identifier]->tax==2){
				$_SESSION['PaymentDetail' . $identifier]->taxname6 =$myrowq['tax_name'];
		$_SESSION['PaymentDetail' . $identifier]->TaxTotals6 += (round(($_SESSION['PaymentDetail' . $identifier]->Amount_Total*(14/114) * (2/14)),2));
				}
				if($_SESSION['PaymentDetail' . $identifier]->tax==5){
				$_SESSION['PaymentDetail' . $identifier]->taxname5 =$myrowq['tax_name'];
		$_SESSION['PaymentDetail' . $identifier]->TaxTotals5 += (round(($_SESSION['PaymentDetail' . $identifier]->Amount_Total*(5/100)),2));
				}
				if($_SESSION['PaymentDetail' . $identifier]->tax==10){
				$_SESSION['PaymentDetail' . $identifier]->taxname10 =$myrowq['tax_name'];
		$_SESSION['PaymentDetail' . $identifier]->TaxTotals10 += (round(($_SESSION['PaymentDetail' . $identifier]->Amount_Total*(10/100)),2));
				}
				if($_SESSION['PaymentDetail' . $identifier]->tax==3){
				$_SESSION['PaymentDetail' . $identifier]->taxname3 =$myrowq['tax_name'];
		$_SESSION['PaymentDetail' . $identifier]->TaxTotals3 += (round(($_SESSION['PaymentDetail' . $identifier]->Amount_Total*(3/100)),2));
				} 
				if($_SESSION['PaymentDetail' . $identifier]->tax==8){
				$_SESSION['PaymentDetail' . $identifier]->taxname8 =$myrowq['tax_name'];
		$_SESSION['PaymentDetail' . $identifier]->TaxTotals8 += (round(($_SESSION['PaymentDetail' . $identifier]->Amount_Total*(8/108) * (6/8)),2));
				}	
			echo'</tr>';
			$_SESSION['PaymentDetail' . $identifier]->TOT_TAX=$_SESSION['PaymentDetail' . $identifier]->TaxTotals6+$_SESSION['PaymentDetail' . $identifier]->TaxTotals5+$_SESSION['PaymentDetail' . $identifier]->TaxTotals10+$_SESSION['PaymentDetail' . $identifier]->TaxTotals3+$_SESSION['PaymentDetail' . $identifier]->TaxTotals8;
			
	}
	################################################################## finance tax#################################################################################
 /*if ($_SESSION['PaymentDetail' . $identifier]->tax==6) {
   $TaxTotals6 += (round(($_SESSION['PaymentDetail' . $identifier]->Amount_Total*(16/116) * (6/16)),2));
   //$_SESSION['PaymentDetail' . $identifier]->AmountPaid = ($_SESSION['PaymentDetail' . $identifier]->Amount_Total) - ($_SESSION['PaymentDetail' . $identifier]->TaxTotals); 
 }else if ($_SESSION['PaymentDetail' . $identifier]->tax == 8){
   $TaxTotals8 += (round(($_SESSION['PaymentDetail' . $identifier]->Amount_Total*(8/108) * (6/8)),2));
   //$_SESSION['PaymentDetail' . $identifier]->AmountPaid = ($_SESSION['PaymentDetail' . $identifier]->Amount_Total) - ($_SESSION['PaymentDetail' . $identifier]->TaxTotals); 

 }else if ($_SESSION['PaymentDetail' . $identifier]->tax==5) {
   $TaxTotals5 += (round(($_SESSION['PaymentDetail' . $identifier]->Amount_Total*(5/100)),2));
   //$_SESSION['PaymentDetail' . $identifier]->AmountPaid = ($_SESSION['PaymentDetail' . $identifier]->Amount_Total) - ($_SESSION['PaymentDetail' . $identifier]->TaxTotals); 
 }else if ($_SESSION['PaymentDetail' . $identifier]->tax==3) {
   $TaxTotals3 += (round(($_SESSION['PaymentDetail' . $identifier]->Amount_Total*(3/100)),2));
   //$_SESSION['PaymentDetail' . $identifier]->AmountPaid = ($_SESSION['PaymentDetail' . $identifier]->Amount_Total) - ($_SESSION['PaymentDetail' . $identifier]->TaxTotals); 
 }else if ($_SESSION['PaymentDetail' . $identifier]->tax==10) {
   $TaxTotals10 += (round(($_SESSION['PaymentDetail' . $identifier]->Amount_Total*(10/100)),2));
   //$_SESSION['PaymentDetail' . $identifier]->AmountPaid = ($_SESSION['PaymentDetail' . $identifier]->Amount_Total) - ($_SESSION['PaymentDetail' . $identifier]->TaxTotals); 
 }else if($_SESSION['PaymentDetail' . $identifier]->tax==0) {
   $TaxTotals0 += 0;
   //$_SESSION['PaymentDetail' . $identifier]->AmountPaid = ($_SESSION['PaymentDetail' . $identifier]->Amount_Total) - ($_SESSION['PaymentDetail' . $identifier]->TaxTotals); 
 }
  $total_tax_deducted=$TaxTotals6+$TaxTotals8+$TaxTotals5+$TaxTotals3+$TaxTotals10;*/
	//$Total2 = str_replace(',','',$_SESSION['PaymentDetail' . $identifier]->PV_Total);
//$tax_rate = $_SESSION['PaymentDetail' . $identifier]->tax;
######################################################## finance tax###########################################################################################
//$TaxTotals += round(($tax_rate/(100+$tax_rate) * ($Total2)),2);
//$AmountPaid = $Total2 - $TaxTotals; 
}
##################################################################################################
if (isset($_SESSION['PaymentDetail' . $identifier]->VoucherID) AND $_SESSION['PaymentDetail' . $identifier]->VoucherID!=''){
	$_SESSION['PaymentDetail' . $identifier]->VoucherID=$_SESSION['PaymentDetail' . $identifier]->VoucherID;
}else{
echo '<div class="page_help_text">' . _('There is No.payment voucher select to be payed at the moment,click the button to select voucher.') . '</div>
	<br />';
echo'<br>';
echo '<a href="' . $RootPath . '/index.php?Application=PVM&Ref=default&Link=inbox"><button type="button" name="" class="button"><i class="fa fa-reply"></i> ' . _('View Payment Voucher to Approve') . '</button></a>';
exit;
}
#################################################################################################3/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
...
if (isset($_POST['ChequeNum']) AND $_POST['ChequeNum']!=''){
	$_SESSION['PaymentDetail' . $identifier]->ChequeNum = $_POST['ChequeNum'];
}
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
...
if (isset($_POST['ChequeNum']) AND $_POST['ChequeNum']!=''){
	$_SESSION['PaymentDetail' . $identifier]->Narrative=$_POST['ChequeNum'];
}
...
/*if (isset($_POST['ChequeNum']) AND $_POST['ChequeNum']!=''){
$InputError = 0;
$sql_cheque = "SELECT chequeid,
						fy,
						bank,
						cheque_from,
						cheque_to,
						close
				FROM cheque_control WHERE bank='".$_POST['BankAccount']."' 
				AND close = 0 ";

	$resultir = DB_query($sql_cheque);
	while ($myrow3=DB_fetch_array($resultir)){
	$chfrom = $myrow3['cheque_from'];
	$chto = $myrow3['cheque_to'];
	}
	$cheque = $_SESSION['PaymentDetail' . $identifier]->ChequeNum;
	$cheque_From = $chfrom;
	$cheque_To = $chto;		
if($cheque >= $cheque_From && $cheque <= $cheque_To){
	$InputError = 0;
	$prnMsg = _('Cheque Posted is between the Range ,proceed with Payment');
	}else{
		//echo '<a href="' . $RootPath . '/Payments.php?'.$_SESSION['PaymentDetail' . $identifier]->VoucherID.'&'.$_SESSION['PaymentDetail' . $identifier]->SupplierID.'">' . _('Back to Payments ') . '</a>';
	$InputError = 1;
	prnMsg( _('Cheque posted is outside range, enter another cheque Number to proceed with payment OR Either range has not been defined.'),'warn');
	include('includes/footer.inc');
	exit;
	}
	}*/
/*||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||end of cheque posting||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||*/
  ...
  */
	...
	// it is a supplier payment by cheque and haven't printed yet so print cheque
		...
		echo '<br />
			<a href="' . $RootPath . '/PrintCheque.php?ChequeNum=' . $_POST['ChequeNum'] . '&amp;identifier=' . $identifier . '">' . _('Print Cheque using(KCB)') . '</a><br>
			<a href="' . $RootPath . '/PrintCheque_Equity.php?ChequeNum=' . $_POST['ChequeNum'] . '&amp;identifier=' . $identifier . '">' . _('Print Cheque (EQUITY)') . '</a>
			<br>
			<a href="' . $RootPath . '/PrintCheque_National.php?ChequeNum=' . $_POST['ChequeNum'] . '&amp;identifier=' . $identifier . '">' . _('Print Cheque using(NATIONAL BANK)') . '</a><br>
			<a href="' . $RootPath . '/PrintCheque_Cooperative.php?ChequeNum=' . $_POST['ChequeNum'] . '&amp;identifier=' . $identifier . '">' . _('Print Cheque using (CO-OPERATIVE BANK OF KENYA)') . '</a>
			<br />
			<br />';
	  ...
	  /*Its a supplier payment type 22 */
			...
			/* Now debit creditors account with payment + discount */
				...
				if ($_SESSION['PaymentDetail' . $identifier]->Discount !=0){
					...
					$DbgMsg = _('Cannot insert a GL transaction for the payment discount creFinance Voucher number has already been useddit using the SQL');
					$result = DB_query($SQL,$ErrMsg,$DbgMsg,true);
...
/*============================================================================================================================================*/
$SQL = "INSERT INTO votepaymenttrans ( `RefNo`,`Voucherid`, `VoteCode`, `BankAcc`, `SuppNo`, `Amount`,Fy) ";
			$SQL = $SQL . "VALUES ('" . $_SESSION['PaymentDetail' . $identifier]->Cheque . "',		         	
					'" . $_SESSION['PaymentDetail' . $identifier]->VoucherID . "',
					'" . $_SESSION['PaymentDetail' . $identifier]->Votecode . "',
			...
			$result = DB_query($SQL,$ErrMsg,$DbgMsg,true);
/*============================================================================================================================================*/
$SQL = "INSERT INTO cash (`type`,TransNo,Payment_Type,FVoucherNo,VoucherID,Account, date,`descs`, `debit`, `credit`) ";
			$SQL = $SQL . "VALUES (1,
			        '" . $TransNo . "',
			        '" . $_SESSION['PaymentDetail' . $identifier]->Paymenttype . "',
					'". $PaymentItem->Cheque ."',
			...
			$SQL = "INSERT INTO paidtax (Vid,`taxed_from`,tax_percentage,amount,transdate) ";
			$SQL = $SQL . "VALUES ('" . $_SESSION['PaymentDetail' . $identifier]->VoucherID . "',
			                      '" . $_SESSION['PaymentDetail' . $identifier]->SupplierID . "',
			                       '" . $_SESSION['PaymentDetail' . $identifier]->tax. "',
								   '" . $_SESSION['PaymentDetail' . $identifier]->TOT_TAX. "',								  
								   '" .date('Y.m.d')."')";
...
################################update pv tables###########################################################################################################################
/*
$header1 = "SELECT process_level FROM payment_voucher WHERE voucherid = '". $_SESSION['PaymentDetail' . $identifier]->VoucherID . "'";
		$resultheader = DB_query($header1);
$myrowH = DB_fetch_array($resultheader);
if($myrowH['process_level']==8){		
$sql_update = "UPDATE payment_voucher SET process_level=process_level+1 WHERE voucherid = '". $_SESSION['PaymentDetail' . $identifier]->VoucherID . "'";
	$result = DB_query($sql_update);
	
	$SQL = "SELECT voucherno FROM  payment_voucher_approval  WHERE voucherno='". $_SESSION['PaymentDetail' . $identifier]->VoucherID . "' AND process_level=process_level+1";
  if(DB_num_rows($resu) ==0){
$sql2 = "SELECT process_level FROM payment_voucher WHERE voucherid = '". $_SESSION['PaymentDetail' . $identifier]->VoucherID . "'";
		$result2 = DB_query($sql2); 
		$rows = DB_fetch_row($result2);
$sqlq = "INSERT INTO payment_voucher_approval (voucherno,
						process_level,
						approver)
					VALUES ('". $_SESSION['PaymentDetail' . $identifier]->VoucherID . "',
						'".$rows[0]."',
						'" . $_SESSION['UserID'] ."')";
...
$sql = "UPDATE payment_voucher SET process_level=process_level+1,comment='' WHERE voucherid= '". $_SESSION['PaymentDetail' . $identifier]->VoucherID . "'";
		$result = DB_query($sql);
			...
			$SupplierSQL="SELECT suppname FROM suppliers
			 ...
			 $SupplierResult = DB_query($SupplierSQL);
                         ...
                         echo '<br /><a href="' . $RootPath . '/Payments.php?SupplierID=' . $LastSupplier . '">' . _('Enter another Payment for') . ' ' . $SupplierRow['suppname'] . '</a>';
						  echo '<br /><a href="' . $RootPath . '/index.php?Application=PVM&Ref=default&Link=inbox">' . _('Back to Payment Voucher') . '</a>';
		} else {
		echo '<br /><a href="' . $RootPath . '/index.php?Application=PVM&Ref=default&Link=inbox">' . _('Approve Another Payment Voucher (PV') . '</a><br />';
  ...
  /* User hit delete the receipt entry from the batch */
	...
	if (is_numeric($_POST['GLManualCode'])){
		...
		if (DB_num_rows($Result)==0){
			...
			prnMsg( _('Finance Voucher number has already been used') . ' - ' . _('This GL analysis item could not be added'),'error');
		}*/ else {
			$myrow = DB_fetch_array($Result);
...
echo ' ' . _('on') . ' ' . $_SESSION['PaymentDetail' . $identifier]->DatePaid . '</h3>';
#############################################################################################################################
	$headers = "SELECT *,a.voucherid,
		                  a.votehead,
						  a.invoice_no,
						  c.VoteHead,
						  a.particulars,
				...
				AND a.voucherid = '" . $_GET['Vid'] . "'";
				
	$res = DB_query($headers);
	while ($my=DB_fetch_array($res)){
	$pat  = unserialize($my['particulars']);
	$lpono = unserialize($my['lpo_no']);
	$invoiceno = unserialize($my['invoice_no']);
	$amount = unserialize($my['amount']);
	}
#############################################################################################################################
...
for($i=0;$i<=count($lpono);$i++)
{
	...
	$invoice = $invoiceno[$i];
	$amnt = $amount[$i];
$_SESSION['PaymentDetail' . $identifier]->amnt =$amnt;

	echo '<td colspan="4">'.$particulars.'</td>
	      <td colspan="4">'.$lpo.'</td>
		 <td colspan="50">'.$invoice.'</td>
		 <td colspan="50">'.$_SESSION['PaymentDetail' . $identifier]->amnt.'</td>';
$_SESSION['PaymentDetail' . $identifier]->amnt44=$amnt;
		}
...
echo '<tr>
		<td>' . _('Vote Head') . ':</td>
		<td><input type="hidden" disabled="true" name="VoteHead" maxlength="65" size="65" value="' .$_SESSION['PaymentDetail' . $identifier]->PV_Votehead. '" /><input type="text" disabled="true" name="VoteHead" maxlength="65" size="65" value="' .$_SESSION['PaymentDetail' . $identifier]->VoteCode. '" /></td>';
/*if (DB_num_rows($resulti)==0){
	echo '</select></td>
		</tr>
		</table>
		<p />';
	prnMsg( _('Vote Head Accounts have not yet been defined. You must first') . ' <a href="' . $RootPath . '/VoteHeadsMaintenance.php">' . _('define the vote head accounts') . '</a> ' . _(' to be affected'),'warn');
	include('includes/footer.inc');
	exit;
} else {
	//echo '<option value=""></option>';
	while ($myr=DB_fetch_array($resulti)){
	/*list the bank account names 
		if (isset($_POST['VoteHead']) AND $_POST['VoteHead']==$myr['Votecode']){
			echo '<option selected="selected" value="' . $myr['Votecode'] . '">' . $myr['Votecode'] . ' - ' . $myr['Votehead'] . '</option>';
		} else {
			echo '<option value="' . $myr['Votecode'] . '">' . $myr['Votecode'] . ' - ' . $myr['Votehead'] . '</option>';
		}
	}
	echo '</select></td>*/
		echo'</tr>';
...
################################################################################################333
		$sql_check = "SELECT * ,chequeNo FROM payment_voucher WHERE voucherid = '" .$_SESSION['PaymentDetail' . $identifier]->VoucherID. "' ";
	$result_check = DB_query($sql_check);
	while ($check=DB_fetch_array($result_check)){
	$Session_chequeNo = $check['chequeNo'];
	}
####################################################################################################
echo '<tr>
		<td>' . _('Cheque Number') . ':</td>
		<td><input type="text" name="ChequeNum" maxlength="20" size="15" value="' . $Session_chequeNo . '" /> ' . _('(if using pre-printed stationery)') . '</td>
	</tr>';
...
echo '<tr>
		<td>' . _('Narrative') . ':</td>
		<td colspan="2"><input type="text"  required="required" name="Narrative" maxlength="200" size="52" value="' . stripslashes($_POST['Narrative'] ) . '" />  ' . _('Narrative on General Ledger Account Inquiry') . '</td>
	</tr>
...
// End select Tag
	...
	if (isset($_POST['GLManualCode'])) {
		echo '<tr>
				...
				<td><input type="text" name="GLManualCode" maxlength="12" size="12" onchange="return inArray(this, GLCode.options,\'' . _('The account code') . ' \' + this.value + \' ' . _('doesnt exist') . '\')" value="'. $_SESSION['PaymentDetail' . $identifier]->PV_Votehead .'"   /></td>
			</tr>';
	} else {
		echo '<tr>
				<td>' . _('Enter GL Account Manually') . ':</td>
	...
	echo '<tr>
			...
			echo'<td>';
			echo'<input type="hidden" disabled="true" name="GLCode" maxlength="65" size="65" value=' .$_SESSION['PaymentDetail' . $identifier]->PV_Votehead. ' /><input type="text" disabled="true" name="GLCode" maxlength="65" size="65" value="' .$_SESSION['PaymentDetail' . $identifier]->VoteCode. '" /></td>';
	...
	echo '<tr>
			<td>' .  _('Finance Voucher Number')  . '</td>
			<td><input type="text" required="required" name="Cheque" maxlength="12" size="12"value="' . $_POST['Cheque'] . '" /></td>
		</tr>';
	...
	echo '<tr>';	
		while ($myrowq = DB_fetch_array($results)) {
			...
			echo '<tr>
				...
				if($myrowq['percentage']==2){
		$TaxTotals6 += (round(($_SESSION['PaymentDetail' . $identifier]->Amount_Total*(14/114) * (2/14)),2));
...
/*$tax_rate = $_SESSION['PaymentDetail' . $identifier]->tax;
$tax = $tax_rate+100;
$Total_Pay = $_SESSION['PaymentDetail' . $identifier]->PV_Total-$tax;*/
$_SESSION['PaymentDetail' . $identifier]->PAID=$_SESSION['PaymentDetail' . $identifier]->Amount_Total - $_SESSION['PaymentDetail' . $identifier]->TOT_TAX;
echo '<tr>
<td>' . _('Amount') . ' (' . $_SESSION['PaymentDetail' . $identifier]->Currency . ')( Amount Paid.):</td>
<td><input type="text" required="required" name="GLAmount" maxlength="12" size="12" class="number" value="' .  $_SESSION['PaymentDetail' . $identifier]->PAID . '" /></td>
			</tr>';
	
	echo '<tr>';	
	$sqlsttt = "SELECT *,b.tax_name,a.tax_percentage FROM  tax_deducted a 
	         INNER JOIN pv_tax b ON a.tax_percentage=b.ptid WHERE a.Vid=" .  $_SESSION['PaymentDetail' . $identifier]->VoucherID . "";
	$resultsuuu = DB_query($sqlsttt);
	echo '<tr>';	
		  echo'<td>' . $_SESSION['PaymentDetail' . $identifier]->taxname6 . '</td>
		      <td>' . $_SESSION['PaymentDetail' . $identifier]->TaxTotals6 . '</td>
			  </tr>
			   <tr>
			   <td>' . $_SESSION['PaymentDetail' . $identifier]->taxname3 . '</td>
				<td>' .  $_SESSION['PaymentDetail' . $identifier]->TaxTotals3. '</td>
			 </tr>
			 <tr>
			   <td>' . $_SESSION['PaymentDetail' . $identifier]->taxname10 . '</td>
				<td>' .  $_SESSION['PaymentDetail' . $identifier]->TaxTotals10. '</td>
			 </tr>
			  <tr>
			    <td>' . $_SESSION['PaymentDetail' . $identifier]->taxname5 . '</td>
				<td>' .  $_SESSION['PaymentDetail' . $identifier]->TaxTotals5. '</td>
			 </tr>
			  <tr>
			   <td>' . $_SESSION['PaymentDetail' . $identifier]->taxname8 . '</td>
				<td>' .  $_SESSION['PaymentDetail' . $identifier]->TaxTotals8. '</td>
			</tr>';

	echo '<tr>';	
	echo'<td>' . _('Tax3434') . ' (' . $_SESSION['PaymentDetail' . $identifier]->tax . '%) (Posted to Account):</td>
	<td><input type="text"  name="tax" maxlength="12" size="12" class="number" value="' .   $_SESSION['PaymentDetail' . $identifier]->TOT_TAX . '" /></td>
			</tr>';
	} else {
	$headers45 = "SELECT *,a.voucherid,
		                  a.votehead,
						  a.invoice_no,
						  c.VoteHead,
						  a.particulars,
	...
	$res34 = DB_query($headers45);
	while ($my33=DB_fetch_array($res34)){
	$amount23 = unserialize($my33['amount']);
	$lpono = unserialize($my33['lpo_no']);
	$amount = unserialize($my33['amount']);
	}
	for($i=0;$i<=count($lpono);$i++)
	{
	$amnt355 = $amount23[$i];
...
//$Total2 = $_SESSION['PaymentDetail' . $identifier]->PV_Total;
...
echo '<tr>
	...
	echo '<tr>';
	echo'<td>' . _('Tax22') . ' (' . $_SESSION['PaymentDetail' . $identifier]->tax . '%) (Posted to Account):</td>
	<td><input type="text"  name="tax" maxlength="12" size="12" class="number" value="' .  $_SESSION['PaymentDetail' . $identifier]->TOT_TAX . '" /></td>
			</tr>';
	}
...
the fields for entry of receipt amt and disc */
	...
	echo '<table class="selection">
		<tr>
			<td>' . _('Amount of Payment') . ' ' . $_SESSION['PaymentDetail' . $identifier]->Currency . '(Amount Paid):</td>
			<td><input class="number" type="text" name="Amount" maxlength="12" size="13" value="' . $_SESSION['PaymentDetail' . $identifier]->AmountPaid . '" /></td>
		</tr>';
		echo '<tr>
	<td>' . _('Tax12121') . ' (' . $_SESSION['PaymentDetail' . $identifier]->tax . '%) (Posted to Account):</td>
	<td><input type="text" required="required" name="GLAmount" maxlength="12" size="12" class="number" value="' .  $_SESSION['PaymentDetail' . $identifier]->TaxTotals . '" /></td>
			</tr>';
