...
for($i=0; $i < $N; $i++)
{
$sql = "UPDATE payment_voucher SET process_level=4 WHERE voucherid = '". $pv[$i] . "'";

		$result = DB_query($sql);
$SQL = "SELECT voucherno FROM  payment_voucher_approval where voucherno='".$pv[$i]."' and process_level=4";
$resu=DB_query($SQL);
...
$sql = "UPDATE payment_voucher SET process_level=12, comment='$comment' WHERE voucherid = '". $_POST['PV'] . "'";
...
echo '<a href="' . $RootPath . '/Process_IA_PV.php">' . _('Back to Payment Voucher') . '</a>';
...
if (!isset($SelectedUser)) {
	$sql = "SELECT voucherid,
					...
					process_level,
					total
				FROM payment_voucher WHERE process_level =3 OR process_level = 12
