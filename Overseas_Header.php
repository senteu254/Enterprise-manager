...
if (isset($_GET['ModifyOrderNumber'])) {
	$Title = _('Modify Oversea Order') . ' ' . $_GET['ModifyOrderNumber'];
} else {
	$Title = _('Oversea Order Entry');
}
...
$ViewTopic= 'OverseasOrdering';
$BookMark = 'OverseasOrdering';
...
if ((isset($_POST['UpdateStatus']) AND $_POST['UpdateStatus'] != '')) {
	...
	if ($_SESSION['ExistingOrder'] == 0) {
		...
		if ($OKToUpdateStatus == 1) {
			...
			$SQL = "UPDATE overseaoders SET status='" . $_POST['Status'] . "',
							stat_comment='" . $_SESSION['PO' . $identifier]->StatusComments . "',
							allowprint='" . $AllowPrint . "'
					WHERE  overseaoders.orderno ='" . $_SESSION['ExistingOrder'] . "'";
			...
			if ($_POST['Status']=='Completed' OR $_POST['Status']=='Cancelled' OR $_POST['Status']=='Rejected') {
				$SQL = "UPDATE overseaorderdetails SET completed=1 WHERE orderno='" . $_SESSION['ExistingOrder'] . "'";
				$UpdateResult =DB_query($SQL,$ErrMsg);
			} else {//To ensure that the overseaorderdetails status is correct when it is recovered from a cancelled orders
				$SQL = "UPDATE overseaorderdetails SET completed=0 WHERE orderno='" . $_SESSION['ExistingOrder'] . "'";
				$UpdateResult = DB_query($SQL,$ErrMsg);
			}
		} //$OKToUpdateStatus == 1
...
if (isset($_POST['EnterLines']) OR isset($_POST['AllowRePrint'])) {
	...
	if(!isset($_POST['PONo']) or $_POST['PONo'] == ""){
	prnMsg(_('Purchase Order Number cannot be empty') . '. ', 'warn');
	$error = 1;
	}
	$sql = "SELECT orderno FROM  overseaoders
				WHERE  overseaoders.orderno='" . $_POST['PONo'] . "'";
	$ErrMsg = _('An error occurred checking the purchase order existance') . '. ' . _('The error says');
	$Result = DB_query($sql, $ErrMsg);
	
	if(DB_num_rows($Result) >0){
	prnMsg(_('Purchase Order Number entered already exist in the system. Please try again') . '. ', 'warn');
	$error = 1;
	}else{

	$_SESSION['PO'.$identifier]->OrderNo = $_POST['PONo'];
	...
	$_SESSION['PO' . $identifier]->Ordertype = $_POST['Ordertype'];

	if (isset($_POST['RePrint']) AND $_POST['RePrint'] == 1) {
		$_SESSION['PO' . $identifier]->AllowPrintPO = 1;

		$sql = "UPDATE  overseaoders
				SET  overseaoders.allowprint='1'
				WHERE  overseaoders.orderno='" . $_SESSION['PO' . $identifier]->OrderNo . "'";
	...
	if (!isset($_POST['AllowRePrint'])) { // user only hit update not "Enter Lines"
		echo '<meta http-equiv="Refresh" content="0; url=' . $RootPath . '/Oversea_Items.php?identifier=' . $identifier . '">';
		echo '<p>';
		prnMsg(_('You should automatically be forwarded to the entry of the purchase order line items page') . '. ' . _('If this does not happen') . ' (' . _('if the browser does not support META Refresh') . ') ' . '<a href="' . $RootPath . '/Oversea.php?identifier=' . $identifier . '">' . _('click here') . '</a> ' . _('to continue'), 'info');
		include('includes/footer.inc');
		exit;
	} // end if reprint not allowed

} // end of checking the order no
} //isset($_POST['EnterLines']) OR isset($_POST['AllowRePrint'])

/* end of if isset _POST'EnterLines' */

echo '<span style="float:left"><a href="' . $RootPath . '/PO_SelectOSPurchOrderOveseas.php?identifier=' . $identifier . '">' . _('Back to Overseas Orders') . '</a></span>';
...
if (isset($_GET['ModifyOrderNumber'])) {
	include('includes/PO_ReadInOrderOverseas.inc');
}
...
if ((!isset($_POST['SearchSuppliers']) or $_POST['SearchSuppliers'] == '') AND (isset($_SESSION['PO' . $identifier]->SupplierID) AND $_SESSION['PO' . $identifier]->SupplierID != '')) {
	...
	$_POST['Ordertype'] = $_SESSION['PO' . $identifier]->Ordertype;
...
if (isset($_POST['Select'])) {
	...
	if (($AuthRow = DB_fetch_array($AuthResult) and $AuthRow['cancreate'] == 0)) {
		...
		prnMsg(_('You do not have the authority to raise Overseas Orders for') . ' ' . $myrow['suppname'] . '. ' . _('Please Consult your system administrator for more information.') . '<br />' . _('You can setup authorisations') . ' ' . '<a href="PO_AuthorisationLevels.php">' . _('here') . '</a>', 'warn');
		include('includes/footer.inc');
		exit;
	}
...
if ($_SESSION['RequireSupplierSelection'] == 1 OR !isset($_SESSION['PO' . $identifier]->SupplierID) OR $_SESSION['PO' . $identifier]->SupplierID == '') {
	echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/supplier.png" title="' . _('Purchase Order') . '" alt="" />' . ' ' . _('Oversea Order: Select Supplier') . '</p>';
	...
	if ($_SESSION['ExistingOrder']) {
		echo _(' Modify Oversea Order Number') . ' ' . $_SESSION['PO' . $identifier]->OrderNo;
	}
	...
	if (isset($Purch_Item)) {
		...
		/* the link */
		echo '<a href="' . $RootPath . '/Oversea_Items.php?NewItem=' . $Purch_Item . '&identifier=' . $identifier . '">' . _('Enter Line Item to this purchase order') . '</a>';
	...
	/*******************************************************type of order*****************************************************************************
	echo'<tr>';
	echo'<td>Type of Order</td><td><select name="Ordertype">';
	
	echo'<option value="1">Local order</option>';			   
    echo'<option value="2">Overseas Order</option>';
	echo '</select></td>
	
	<tr>';
	/********************************************************end*********************************************************************************/
	echo '<tr>
	<td>' . _('Contract Number.') . ':</td>
	<td><input type="text" required="required" autofocus="autofocus" name="PONo" class="text" size="30" value="'.$_POST['PONo'].'" /></td>
	</tr>';
	}
	echo '<tr>
			<td>' . _('Contract Date') . ':</td>
			<td>';
	if ($_SESSION['ExistingOrder'] != 0) {
	...
	if (isset($_POST['AllowRePrint'])) {
		$sql = "UPDATE  overseaoders SET allowprint=1 WHERE orderno='" . $_SESSION['PO' . $identifier]->OrderNo . "'";
		$result = DB_query($sql);
	}
	...
	if ($_SESSION['PO' . $identifier]->Status == '') { //then its a new order
		echo '<tr>
				<td><input type="hidden" name="Status" value="NewOrder" />' . _('New Oversea Order') . '</td>
			</tr>';
	} else {
		echo '<tr>
				<td>' . _('Status') . ' :  </td>
